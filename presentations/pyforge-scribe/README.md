...
`pixi run -e local-recipes deck-trio pyforge-scribe --head --deck` at tree `a407cd03f6`
...
numbered-section/closing-band slides for the deck): `PyForge Scribe - Infographic.dc.html` now
145,087 B, `PyForge Scribe - Infographic Deck.dc.html` now 150,893 B. A second `--head --deck`
run changed nothing on disk (verified).

`pixi run -e local-recipes deck-facts pyforge-scribe --refresh` then `--check` at the same tree
brought poster, head and Infographic Deck current: **39** stale `data-fact` literals rewritten
...
reads `0 unmarked, 0 mismatch, 0 drifted, 0 unsourced, 1 unshown; facts 285/285`.
...
`a1e42dac-7cee-438b-9acc-2523985b5253`, then read back and SHA-256-compared against disk:

| Artifact | Bytes | Design etag | Read-back |
|---|---|---|---|
| `PyForge Scribe - Infographic.dc.html` | 145,087 | `1789635957432168` | identical ✓ |
| `PyForge Scribe - Infographic Deck.dc.html` | 150,893 | `1789635959056382` | identical ✓ |

Both files are under the 256 KiB `read_file` cap, so a single call plus entity-decoded SHA-256
comparison is the read-back proof (no windowing needed). `deck-facts pyforge-scribe --check`

...
def main():
    parser = argparse.ArgumentParser(
        description="Install bmad-manticore skills into .claude/skills/",
        add_help=True,
    )
    parser.add_argument(
        "dest",
    ...
    source = os.path.join(prefix, "share", "bmad-manticore", "skills")
    if not os.path.isdir(source):
        print(f"Error: bmad-manticore not found at {source}", file=sys.stderr)
        sys.exit(1)

...
def main():
    parser = argparse.ArgumentParser(
        description="Install superpowers into .claude/skills/",
        add_help=True,
    )
    parser.add_argument(
        "dest",
    ...
    source = os.path.join(prefix, "share", "superpowers", "skills")
    if not os.path.isdir(source):
        print(f"Error: superpowers not found at {source}", file=sys.stderr)
        sys.exit(1)

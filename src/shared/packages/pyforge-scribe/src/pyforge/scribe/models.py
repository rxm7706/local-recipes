...
class CaptureRecord(BaseModel):
    ...
    def from_frontmatter(cls, frontmatter: str, *, text: str = "") -> "CaptureRecord":
        ...
        `description`, `metadata.type`) — the isolated swap point the spec
        calls for, not a general-purpose YAML parser. Prefer
        `parse_capture_file()` when reading a whole file from disk: it
        locates this block's boundaries correctly even when the captured
        body itself contains a bare `---` line.
        """
        name = ""
        description = ""

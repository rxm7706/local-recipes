"""Station-owned CAP-16 persona contract for bmad-agent-scribe (Story 5.1).

Steward 29.2 remains the fleet proof. This suite owns the same contract
under pyforge-scribe so Path B is not steward-only leftover.
"""
...
CONTENT_SKILL = f"pyforge-{PROOF_STATION}"
...
MCP_PATH = f"/stations/{PROOF_STATION}/mcp"
...
def validate_transcript(events: list[dict]) -> None:
    ...
    for event in events:
        ...
        if kind == "consult_content_skill":
            ...
            if len(argv) < 3 or argv[0] != "pyforge" or argv[1] != PROOF_STATION:
...
def test_golden_transcript_is_grammar_and_mcp_only():
    root = _repo_root()
    path = _persona_dir(root) / "transcripts" / "scribe-recall-e2e.json"
...
def test_station_binary_without_pyforge_grammar_fails():
    events = [
        ...
        {"kind": "grammar", "argv": ["scribe", "recall", "why did we pin typer"]},
    ]
    try:
        validate_transcript(events)
    except PersonaContractError:
        return
    raise AssertionError("bare scribe binary was accepted as FR-13 grammar")
...
def test_consult_of_graph_file_is_not_cap15():
    events = [
        {
            "kind": "consult_content_skill",
            "skill": CONTENT_SKILL,
            "path": ".claude/data/pyforge-scribe/graph.json",
        },
        {"kind": "grammar", "argv": ["pyforge", "scribe", "recall", "x"]},
    ]
    try:
        validate_transcript(events)
    except PersonaContractError:
        return
    raise AssertionError("consult of graph.json was accepted as CAP-15")


def test_mcp_other_station_is_not_scribe_face():
    events = [
        {
            "kind": "consult_content_skill",
            "skill": CONTENT_SKILL,
        ...
        {"kind": "mcp", "method": "POST", "path": "/stations/atlas/mcp"},
    ]
    try:
        validate_transcript(events)
    except PersonaContractError:
        return
    raise AssertionError("atlas MCP was accepted as the scribe face")
...
def test_live_persona_skill_forbids_filesystem_and_adhoc_http():
    ...
    assert "pyforge scribe" in skill
...
def test_persona_is_bmad_launcher_not_skf_compiled():
    ...
    assert "pyforge scribe" in customize
...
def test_does_not_mint_01_02_persona():
    root = _repo_root()
    skills = root / ".claude" / "skills"
    assert not (skills / "bmad-agent-chrome-probe").exists()
    assert not (skills / "bmad-agent-infra-probe").exists()

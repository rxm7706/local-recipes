...
def test_entry_cap_is_enforced_within_a_single_pathologically_wide_directory(
    ...
    than scanning that whole directory's filename listing first. A tiny
    monkeypatched cap keeps this test fast without creating tens of
    thousands of real files."""
    from pyforge.warden import hygiene as hygiene_module

    monkeypatch.setattr(hygiene_module, "_ADJACENT_PYTHON_SOURCE_ENTRY_CAP", 10)
    for i in range(200):
        (tmp_path / f"file{i}.txt").write_text("", encoding="utf-8")
    # Cap-exhaustion is inconclusive -> True (never silently skip hygiene).
    assert has_adjacent_python_source(tmp_path) is True

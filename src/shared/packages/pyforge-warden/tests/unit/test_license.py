...
def test_conda_about_license_alias_bomb_degrades_to_unknown_not_expanded(tmp_path, component_factory):
...
def test_meta_yaml_v0_about_license_also_resolves(tmp_path, component_factory):
    (tmp_path / "meta.yaml").write_text("about:\n  license: BSD-3-Clause\n", encoding="utf-8")
    component = component_factory(
        name="mypkg",
        version="1.0.0",
        ecosystem=Ecosystem.CONDA,
...
def test_recipe_yaml_neutralize_pass_tolerates_a_jinja_comment(tmp_path, component_factory):
    ...
    (tmp_path / "recipe.yaml").write_text("{# a comment #}\nabout:\n  license: MIT\n", encoding="utf-8")
    component = component_factory(
        name="mypkg",
        version="1.0.0",
        ecosystem=Ecosystem.CONDA,
...
def test_conda_recipe_yaml_wins_over_meta_yaml_when_both_present(tmp_path, component_factory, provenance):
...
def test_pypi_uninstalled_is_unknown_with_warn_finding(tmp_path, component_factory):
    ...
    assert finding.license == LicenseInfo(expression="unknown", family=None, verdict=LicenseVerdict.UNKNOWN)
...
def test_pypi_empty_identity_name_degrades_to_unknown_never_crashes(tmp_path, component_factory):
...
def test_pypi_installed_resolves_via_pep639_license_expression(monkeypatch, tmp_path, component_factory):
    _patch_metadata(monkeypatch, _fake_metadata(license_expression="MIT"))
    component = component_factory(name="fake-pkg", version="1.0.0")
    assert license_findings([component], tmp_path) == ()


def test_pypi_installed_falls_back_to_legacy_license_field(monkeypatch, tmp_path, component_factory):
    _patch_metadata(monkeypatch, _fake_metadata(license="Apache-2.0"))
    component = component_factory(name="fake-pkg", version="1.0.0")
    assert license_findings([component], tmp_path) == ()


def test_pypi_installed_falls_back_to_trove_classifiers(monkeypatch, tmp_path, component_factory):
    _patch_metadata(
        monkeypatch,
...
def test_pypi_generic_bsd_classifier_alone_is_unknown(monkeypatch, tmp_path, component_factory):
...
def test_pypi_conflicting_classifiers_degrade_to_unknown_not_first_pick(monkeypatch, tmp_path, component_factory):
...
def test_pypi_installed_prefers_pep639_over_legacy_and_classifiers(monkeypatch, tmp_path, component_factory):
...
def test_pypi_installed_unresolvable_metadata_degrades_to_unknown(monkeypatch, tmp_path, component_factory):
...
def test_pypi_installed_full_license_text_never_parsed_as_an_expression(monkeypatch, tmp_path, component_factory):
...
def test_both_flags_overlapping_match_deny_wins(tmp_path, component_factory):
    ...
    (finding,) = license_findings([component], tmp_path, allow_licenses=("MIT",), deny_licenses=("MIT",))
    assert finding.license.verdict is LicenseVerdict.DENIED
...
def test_deny_list_order_independent_compound_match(monkeypatch, tmp_path, component_factory):
    ...
    component = component_factory(name="fake-pkg", version="1.0.0")
    (finding,) = license_findings([component], tmp_path, deny_licenses=("MIT OR Apache-2.0",))
    assert finding.license.verdict is LicenseVerdict.DENIED


def test_allow_list_or_decomposition_all_branches_covered(monkeypatch, tmp_path, component_factory):
    ...
    component = component_factory(name="fake-pkg", version="1.0.0")
    assert license_findings([component], tmp_path, allow_licenses=("MIT", "Apache-2.0")) == ()


def test_allow_list_or_decomposition_partial_coverage_is_denied(monkeypatch, tmp_path, component_factory):
...
def test_deny_list_and_expression_any_symbol_denies(monkeypatch, tmp_path, component_factory):
    ...
    _patch_metadata(monkeypatch, _fake_metadata(license_expression="MIT AND GPL-3.0-only"))
    component = component_factory(name="fake-pkg", version="1.0.0")
    (finding,) = license_findings([component], tmp_path, deny_licenses=("GPL-3.0-only",))
    assert finding.license.verdict is LicenseVerdict.DENIED
...
def test_non_string_license_value_degrades_to_unknown_never_crashes(tmp_path, component_factory):
    (tmp_path / "recipe.yaml").write_text("about:\n  license:\n    - MIT\n    - Apache-2.0\n", encoding="utf-8")
    component = component_factory(
        name="mypkg",
        version="1.0.0",
...
def test_manifest_invalid_utf8_degrades_to_unknown_never_crashes(tmp_path, component_factory):
...
def test_deps_assessed_equals_deps_total_regardless_of_verdict_mix(tmp_path, component_factory):
    (tmp_path / "recipe.yaml").write_text("about:\n  license: MIT\n", encoding="utf-8")
...
def test_conda_manifest_is_read_at_most_once_per_manifest(tmp_path, component_factory, monkeypatch):
    (tmp_path / "recipe.yaml").write_text("about:\n  license: MIT\n", encoding="utf-8")
    calls: list[object] = []
...
def test_license_rung_verdict_absent_from_policy_degrades_to_indeterminate():
    ...
    status, _driver = license_rung(finding, policy={LicenseVerdict.DENIED: Status.POLICY_VIOLATION})
    assert status is Status.INDETERMINATE
...
def test_classify_verdict_no_flags_resolvable_is_allowed():
    resolution = ("MIT", "MIT")
    assert _classify_verdict(resolution, allow=frozenset(), deny=frozenset()) is LicenseVerdict.ALLOWED


def test_classify_verdict_unresolvable_is_unknown_regardless_of_flags():
    assert _classify_verdict(None, allow=frozenset({"MIT"}), deny=frozenset({"MIT"})) is LicenseVerdict.UNKNOWN
...
def test_classify_verdict_deny_matches_any_symbol_of_a_compound_expression():
    ...
    assert _classify_verdict(resolution, allow=frozenset(), deny=frozenset({"MIT"})) is LicenseVerdict.DENIED
...
def test_classify_verdict_allow_requires_every_symbol_of_a_compound_expression():
    ...
    resolution = ("Apache-2.0 OR MIT", None)
    assert _classify_verdict(resolution, allow=frozenset({"MIT"}), deny=frozenset()) is LicenseVerdict.DENIED
    assert (
        _classify_verdict(resolution, allow=frozenset({"MIT", "Apache-2.0"}), deny=frozenset())
...
def test_parse_spdx_long_valid_compound_expression_resolves():
    ...
    assert (
        _classify_verdict(resolution, allow=frozenset(), deny=_normalize_tokens(("GPL-3.0-only",)))
...
def test_classify_verdict_deny_of_base_license_taints_with_exception_variant():
    ...
    assert (
        _classify_verdict(resolution, allow=frozenset(), deny=_normalize_tokens(("GPL-2.0-only",)))
        is LicenseVerdict.DENIED
    )
...
def test_classify_verdict_allow_of_base_license_does_not_auto_allow_with_variant():
    ...
    resolution = _parse_spdx("GPL-2.0-only WITH Classpath-exception-2.0")
    assert (
        _classify_verdict(resolution, allow=_normalize_tokens(("GPL-2.0-only",)), deny=frozenset())
        is LicenseVerdict.DENIED
    )
...
def test_conda_license_ref_resolves_and_deny_matches(tmp_path, component_factory):
    ...
    assert license_findings([component], tmp_path) == ()
    (finding,) = license_findings([component], tmp_path, deny_licenses=("LicenseRef-Proprietary",))
    assert finding.license.verdict is LicenseVerdict.DENIED
    assert finding.license.expression == "LicenseRef-Proprietary"
    assert finding.id == "license:LicenseRef-Proprietary:mypkg@1.0.0"


def test_conda_empty_paren_license_value_degrades_to_unknown_never_crashes(tmp_path, component_factory):
...
def test_deeply_nested_manifest_flow_collections_degrade_never_crash(tmp_path, component_factory):
...
def test_pypi_unmapped_license_naming_classifier_alongside_mapped_degrades(monkeypatch, tmp_path, component_factory):
...
def test_pypi_generic_parent_approval_classifier_is_not_a_conflict(monkeypatch, tmp_path, component_factory):

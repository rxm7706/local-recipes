"""Story 10.1 — SKF domain skill compiled from pyforge-warden (CAP-15)."""
...
def test_compiled_skill_is_agentskills_compliant_with_provenance():
    ...
    assert "pyforge warden" in body
    assert "scan --doctor" in body


def test_cli_is_the_sole_pr_gate_publisher():
    """Content skill must not teach agents to invent a second PR-gate."""
    root = _repo_root()
    skill = (_skill_package(root) / "SKILL.md").read_text(encoding="utf-8")
    lowered = skill.lower()
    assert "second pr-gate" in lowered or "second pr gate" in lowered
    assert "sole pr-gate" in lowered or "sole publisher" in lowered or "do not invent" in lowered

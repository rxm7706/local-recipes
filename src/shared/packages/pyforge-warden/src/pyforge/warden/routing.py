...
environment.yml's exactly, for the ``environment.yaml`` spelling (shares
``EnvironmentYmlExtractor``, so the same 2 section tokens apply). Unknown
pairs raise ``ValueError`` (fail-loud; already caught at ``cli.py``'s
extract-stage seam — the ``except (SystemExit, Exception)`` handler around
``extractor.extract(...)`` — and converted to a typed ``internal-error``
record with the report still emitted, ratified by Story 1.7). The
kind/section tokens are imported from their owning modules (single

"""Story 2.5 -- `recipe.py::validate()`: numerically the first `recipe`
verb, even though `build()`/`diagnose()`/`optimize()`/`scan()`/`submit()`/
`update()` (Stories 2.6-2.10) landed first in this worktree (see
`recipe.py`'s own module docstring for the reconciliation rationale).
Mirrors `diagnose()`'s composition-test shape below exactly (argv/timeout
passthrough, verbatim-return, `ensure_import_floor` never called,
...
from pyforge.mason.recipe import build, diagnose, optimize, scan, submit, update, validate

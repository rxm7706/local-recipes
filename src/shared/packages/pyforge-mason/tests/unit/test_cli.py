...
Story 2.5 registers the first real verb, `recipe validate`, numerically --
even though `build`/`diagnose`/`optimize`/`scan`/`submit`/`update` (Stories
2.6-2.10) landed first in this worktree (see `cli.py`'s own module
docstring for the reconciliation rationale) -- and its own test block below
mirrors `recipe diagnose`'s established pattern: `recipe.validate` is
mocked (`patch("pyforge.mason.cli.recipe.validate", ...)`) to return a
fixed `CfeResult`, text/JSON happy paths, flags/environ/cwd passthrough,
`--cfe-timeout` resolution, and the two dedicated error-projection tests
(`CfeUnresolvedError`/`CfeTimeoutError`). Unlike every other verb in this
file, `validate` also gets a differentiating pair proving `main()`'s one
non-`EXIT_OK`-always dispatch branch: a passing canned result projects to
`EXIT_OK`, a failing one to `EXIT_FAILED`, in both text and `--format json`
modes -- the JSON envelope's own `status` field stays `"ok"` regardless
(spec I/O matrix), only the process exit code carries the signal.
...
def test_recipe_diagnose_verb_metavar_reflects_the_registered_verb(capsys):
    ...
    `{validate,build,diagnose,optimize,scan,submit,update}`. Story 2.5
    registers the first verb, `validate`, numerically -- even though
    `build`/`diagnose`/`optimize`/`scan`/`submit`/`update` (Stories
    2.6-2.10) were registered first in this worktree (see `cli.py`'s own
    module docstring for the reconciliation rationale) -- so `validate`
    leads this metavar, in registration order, with the other six
    following."""
    with pytest.raises(SystemExit) as exc:
        build_parser().parse_args(["recipe", "bogus-verb"])
    assert exc.value.code == 2
    err = capsys.readouterr().err
    assert "{validate,build,diagnose,optimize,scan,submit,update}" in err

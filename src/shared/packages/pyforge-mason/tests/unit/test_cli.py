...
call `cfe.ensure_import_floor`. Their real end-to-end fixture tests
therefore also fake the import floor via `cfe.probe_import_floor`
(`monkeypatch.setattr`), not `subprocess.run` wholesale -- mirrors
`test_recipe.py`'s identical Design Notes rationale.
"""

...
def test_version_exists_returns_true_on_a_200_response():
    with patch(
        "pyforge.mason.pypi_index.urllib.request.urlopen", return_value=MagicMock(),
    ) as mock_urlopen:
...
def test_version_exists_returns_false_on_a_404_response():
    error = urllib.error.HTTPError(
        url="https://pypi.org/pypi/pkg/0.1.0/json", code=404, msg="Not Found",
        hdrs=None, fp=None,
    )
...
def test_version_exists_returns_none_on_a_non_404_http_error():
    error = urllib.error.HTTPError(
        url="https://pypi.org/pypi/pkg/0.1.0/json", code=500, msg="Server Error",
        hdrs=None, fp=None,
    )
    with patch("pyforge.mason.pypi_index.urllib.request.urlopen", side_effect=error):
        result = pypi_index.version_exists("pkg", "0.1.0")
...
def test_version_exists_returns_none_on_a_bare_oserror():
    with patch(
        "pyforge.mason.pypi_index.urllib.request.urlopen", side_effect=OSError("boom"),
    ):
        result = pypi_index.version_exists("pkg", "0.1.0")
...
def test_version_exists_returns_none_on_a_timeout_error():
    with patch(
        "pyforge.mason.pypi_index.urllib.request.urlopen", side_effect=TimeoutError("timed out"),
    ):
        result = pypi_index.version_exists("pkg", "0.1.0")

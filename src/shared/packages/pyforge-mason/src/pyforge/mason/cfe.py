...
`validate_recipe`/`submit_pr`/`build_native`'s filenames are relative to a
resolved CFE root's `.claude/scripts/conda-forge-expert/`; `build_docker`
is the one entry outside that standard subdirectory -- its script lives at
the CFE root's own top level (spec Always boundary), so `build_docker`
below builds that path itself rather than reusing `_invoke_captured`'s
standard-subdirectory join.

Four entries as of Story 2.6 -- `validate_recipe.py`/`submit_pr.py`
(Story 1.9's fixture stubs) plus the two build scripts this story adds;
which specific script backs each of the remaining Stories 2.4/2.5/2.7-2.10
is still open (epic context); adding a table entry ahead of that decision

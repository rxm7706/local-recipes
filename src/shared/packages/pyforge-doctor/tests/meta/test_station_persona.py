"""Story 18.1 — CAP-16 doctor persona acts only through grammar and MCP."""
...
def test_golden_transcript_is_grammar_and_mcp_only():
    root = _repo_root()
    path = _persona_dir(root) / "transcripts" / "doctor-monitor-e2e.json"
...
def test_freelance_filesystem_in_transcript_fails():
    root = _repo_root()
    events = json.loads(
        (_persona_dir(root) / "transcripts" / "doctor-monitor-e2e.json").read_text(
            encoding="utf-8"
        )
    )
...
def test_station_binary_without_pyforge_grammar_fails():
    events = [
        ...
        {"kind": "grammar", "argv": ["doctor", "monitor", "--fleet"]},
    ]
    try:
        validate_transcript(events)
    except PersonaContractError:
        return
    raise AssertionError("bare doctor binary was accepted as FR-13 grammar")
...
def test_mcp_other_station_is_not_doctor_face():
    ...
    try:
        ...
        return
    raise AssertionError("scribe MCP was accepted as the doctor face")
...
def test_live_persona_skill_forbids_filesystem_and_adhoc_http():
    ...
    assert "pyforge doctor" in skill
...
def test_persona_is_bmad_launcher_not_skf_compiled():
    ...
    assert "pyforge doctor" in customize
    assert MCP_PATH in customize
    assert "bmad-build" not in customize
    assert "advisory" in customize.lower()

...
def _github_owner_repo(target: Path, package: str) -> str | None:
    ...
    ``recipe.yaml`` (``OSError``), malformed YAML (``yaml.YAMLError``), a
    non-mapping document or a missing/non-mapping ``extra``/
    ``cfe-upstream-name`` (``KeyError``/``TypeError``/``AttributeError``),
    or a non-github registry all fold to ``None`` -- the caller
    (``_gather_suite_findings``) treats that identically to "GitHub
    fallback not attempted" (Boundaries)."""
    try:
...
def _fetch_latest_github_release(
    *, owner_repo: str, timeout: float | None = None
    ...
    try:
        with urllib.request.urlopen(tags_url, timeout=resolved_timeout) as response:
            entries = json.loads(response.read())
        parsed = [
            triple
            for triple in (
                _parse_release_triple(_strip_leading_v(str(entry["name"])))

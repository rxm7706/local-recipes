...
def list_linked_github_items(
    *, config: SyncConfig, credential: HostScopedCredential, transport: TransportFn
    ...
    a null entry inside `items.nodes`, or a non-advancing pagination cursor
    -- never a raw `KeyError`/`TypeError`/`AttributeError` escaping to the
    caller, and never an infinite loop against a malformed `pageInfo`.
    """
    ...
    while True:
        ...
        for node in nodes:
            # A null entry (e.g. a partial GraphQL error for one node) is
            # skipped, not fatal -- one bad node must not drop every other
            # candidate on the page, matching this story's own "one item's
            # failure never aborts the batch" spirit.
            if node is None:
                continue
            item_id = node.get("id")
            if item_id is None:
...
def reconcile_schedule_batch(
    *, config: SyncConfig, dry_run: bool = False, transport: TransportFn | None = None
    ...
    return DutyResult(
        ok=not failed,
        summary=(
            f"sync reconcile --schedule: {len(entries)} candidates, "
            f"{len(entries) - len(failed)} ok, {len(failed)} failed"
        ),

...
def reconcile(
    *,
    ...
    if gh_assignee_base is _MISSING and gh.baseline:
        gh_assignee_base = gh.assignee
    jira_assignee_base = jira.baseline.get(_ASSIGNEE_FIELD, _MISSING)
    if jira_assignee_base is _MISSING and jira.baseline:
    ...
    if (
        ...
        return DutyResult(
            ...
            details={
                ...
                "target_value": None,
                "baseline": None,
                "assignee": {"decision": "no_op", "target_value": None},
                "link_repairs": [],
            },
        )
    ...
    details: dict[str, object] = {
        ...
        "baseline": None,
        "assignee": {"decision": assignee_decision, "target_value": assignee_target_value},
    ...
    # then link repairs; a `SyncError` from any step returns immediately
    # (never attempts a later step after an earlier one failed).
    try:
        ...
        if assignee_decision == "push_to_jira":
            ...
            update_github_assignees(
                ...
                # Remove ONLY the login this round observed as GH's own
                # tracked assignee -- never an assignee this module didn't
                # itself add.
                remove=gh.assignee,
                credential=github_credential,
                transport=transport,
    ...
    # or assignee) diverged from ITS OWN prior baseline on EITHER side,
    # because every future reconcile of this pair would otherwise see it as
    # "changed" forever (AD-5's zero-loop property). Deliberately NOT gated
    # on the link-repair flags -- the identity link is never baselined
    # (Boundaries & Constraints), so a link-repair-only round makes exactly
    # the link write(s) above and nothing else. Each side's baseline records
    # that side's own ACTUALLY-RESULTING value for both fields together (one
    # write call per side, unchanged call count) -- this is how a pre-8.7
    # item's first post-upgrade reconcile backfills its missing "assignee"
    # baseline key for free the moment any OTHER field's divergence already
    # triggered a write (Design Notes).
    if gh_changed or jira_changed or gh_assignee_changed or jira_assignee_changed:
        try:
            new_gh_baseline = _serialize_baseline(
                {
                    **gh.baseline,
                    _TRACKED_FIELD: github_status_result,
                    _ASSIGNEE_FIELD: github_assignee_result,
                },
                side="github",
            ...
            new_jira_baseline = _serialize_baseline(
                {
                    **jira.baseline,
                    _TRACKED_FIELD: jira_status_result,
                    _ASSIGNEE_FIELD: jira_assignee_result,
                },
                side="jira",

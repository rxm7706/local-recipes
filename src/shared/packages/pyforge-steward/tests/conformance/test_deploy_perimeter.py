...
def test_multi_worker_django_redis_cache_variant_also_passes():
    """The allowlist covers both Django's own built-in Redis cache backend
    and the widely-deployed third-party django-redis alternative -- see
    `_SHAREABLE_CACHE_BACKENDS`'s own comment for why both are allowlisted."""
    topology = DeploymentTopology(

"""Story 33.1: steward CAP-16 persona acts only through grammar and MCP."""
...
GOLDEN = "steward-provision-list-e2e.json"
...
def test_station_binary_without_pyforge_grammar_fails():
    events = [
        ...
        {"kind": "grammar", "argv": ["steward", "provision", "--list"]},
    ]
    try:
        validate_transcript(events)
    except PersonaContractError:
        return
    raise AssertionError("bare steward binary was accepted as FR-13 grammar")
...
def test_mcp_other_station_is_not_steward_face():
    events = [
        {
            "kind": "consult_content_skill",
            "skill": CONTENT_SKILL,
    ...
    try:
        ...
        return
    raise AssertionError("scribe MCP was accepted as the steward face")
...
def test_live_persona_skill_forbids_filesystem_and_adhoc_http():
    ...
    assert "pyforge steward" in skill
...
def test_persona_is_bmad_launcher_not_skf_compiled():
    ...
    assert "pyforge steward" in customize
...
def test_story_does_not_add_pyforge_under_src_platform():
    root = _repo_root()
    changed = changed_paths_since(root, pathspec="src/platform", include_untracked=True)

...
# -- the other names' routing lines are each a DIFFERENT, not-yet-landed
# station story named directly in the register's own § 2 rows 43-44
# (`mcp-builder` -> atlas 24.1, `slides-generator` -> herald 18.3; epics.md
# Epic 46's own boundary text: "Station-side halves are their own stories
# ... and are named in each story's acceptance, never restated here"). Their
# directories now exist on disk from 46.5's own provisioning, so the
# single-station routing assertion below would otherwise fire against them
# -- this narrow, story-cited carve-out skips ONLY the positive "the owning
# station mentions it" half for these remaining names; the exclusivity half
# (no OTHER, wrong station mentions it) keeps running unconditionally for
# every name, carved out or not (review finding: an earlier draft's
# `continue` skipped both halves, silently widening the carve-out past what
# it needed to cover). Remove a name from this set the same day its own
# cited story lands the persona mention, never before. `multi-repo-git-ops`
# was removed 2026-09-07 by marshal 31.6, which landed its persona mention
# in `bmad-agent-marshal`.

...
# tests/meta/test_exception_root_sole_ownership.py -> parents[3] is
# src/shared/packages/ (mirrors test_atomic_write_sole_ownership.py's
...
def _sibling_station_dirs() -> list[Path]:
    return sorted(
        ...
        and p.name not in _OUT_OF_SCOPE_STATIONS
    )

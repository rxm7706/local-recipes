"""Unit tests for ``pyforge.core.process`` (Story 14.4, SPEC-pyforge-core
CAP-6) -- ``PosixProcess`` against REAL, trivial, fast subprocesses, adapted
from Marshal's own ``tests/unit/test_process_posix.py`` (Story 2.1/3.4),
which this file supersedes -- import paths only, assertion content
unchanged.
"""
...
def test_run_raises_process_error_for_empty_argv(process, tmp_path):
    ...
    ProcessError only" contract holds for every caller."""
    with pytest.raises(ProcessError):
...
def test_run_wraps_missing_executable_launch_failure(process, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(process_module.subprocess, "run", _raise_not_found)
...
def test_run_wraps_a_generic_launch_oserror(process, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(process_module.subprocess, "run", _raise_eacces)
...
def test_run_wraps_a_hung_process(process, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(process_module.subprocess, "run", _raise_timeout)
    with pytest.raises(ProcessError, match="timed out"):
        process.run(["whatever"], cwd=tmp_path)
...
def test_run_wraps_an_embedded_null_byte(process, tmp_path):
    """subprocess.run raises a plain ValueError (not an OSError) for a NUL
    byte in argv -- e.g. a TOML `\\u0000` escape survives shlex.split as an
    ordinary character. Uncaught, this would violate `run`'s own "raises
    ProcessError only" contract."""
    with pytest.raises(ProcessError, match="cannot launch"):
...
def test_run_does_not_block_on_stdin(process, tmp_path):
    """An unattended invocation has no terminal to answer a prompt -- a
    command that unexpectedly reads stdin must see EOF immediately
    (DEVNULL), never hang waiting on input that can never arrive."""
    result = process.run(
...
def test_run_never_interprets_shell_metacharacters(process, tmp_path):
    """This adapter's entire security posture rests on `subprocess.run`
    never being called with `shell=True` -- true today by construction, but
    nothing previously exercised a shell-metacharacter-bearing argument to
    prove it stays inert. A command containing `;`/`&&`/a backtick must be
    passed to the target program as ONE literal argument, never split or
...
# --- is_alive: never raises, existence-only ------------------------------------
...
def test_is_alive_true_when_permission_denied(process, monkeypatch):
    """A process that EXISTS but this invocation lacks permission to signal
    (``EPERM``) still answers ``True`` -- existence, not ownership, is the
    question. Exercised via monkeypatch since provoking a real ``EPERM``
    needs a process owned by a different user, not reliably available in a
    test environment."""
    import pyforge.core.process as process_module
    ...
    monkeypatch.setattr(process_module.os, "kill", _raise_eperm)
    assert process.is_alive(1) is True


def test_is_alive_false_for_a_pid_outside_c_int_range(process):
    """``os.kill`` raises a bare ``OverflowError`` -- NOT an ``OSError`` --
    for a pid larger than C ``int``, so it must not escape this method's
...
def test_is_alive_false_on_an_unexpected_oserror(process, monkeypatch):
    ...
    monkeypatch.setattr(process_module.os, "kill", _raise_other)
...
def test_spawn_detached_child_does_not_import_from_its_cwd(process, tmp_path):
    """``cwd`` sets the child's working directory, but WITHOUT
    ``PYTHONSAFEPATH`` it also lands on the child's ``sys.path[0]`` -- and a
    caller of this method may hand it a ``cwd`` whose contents it does not
    own (an arbitrary project checkout). A stdlib-shadowing module there
    would otherwise choose what the detached child ran, while the caller had
    already been handed a pid and reported success.
...
def test_spawn_detached_quotes_the_log_path_in_its_error_message(process, tmp_path):
    """This message is interpolated verbatim into some callers' own
    report text, printed UNQUOTED by design -- so a raw path here would
    reopen a report-forgery hole. Every other message in this module already
...
def test_spawn_detached_wraps_an_embedded_null_byte_in_the_log_path(process, tmp_path):
    """The ``open(log_path, "wb")`` that precedes the ``Popen`` had only an
    ``OSError`` clause originally, but ``open()`` raises a plain
    ``ValueError`` for a path carrying an embedded NUL -- the SAME CPython
    split already guarded at the sibling ``Popen`` call above, at ``run``'s
    own call."""
    with pytest.raises(ProcessError, match="cannot open log"):
        process.spawn_detached(
...
def test_spawn_detached_calls_popen_with_the_detach_recipe(process, tmp_path, monkeypatch):
    """Proves this generic primitive uses a fixed, deterministic ``Popen``
    kwargs shape rather than a second, divergent detach recipe."""
    import pyforge.core.process as process_module
    ...
    monkeypatch.setattr(process_module.subprocess, "Popen", _fake_popen)
    log_path = tmp_path / "spawned.log"
    ...
    # PYTHONUNBUFFERED=1: a child's stdout is fully block-buffered once
    # redirected to a regular file -- omitting it here would silently
    # reintroduce that bug for any future caller of this generic primitive.
    assert kwargs["env"]["PYTHONUNBUFFERED"] == "1"
    # PYTHONSAFEPATH=1: `python -m <pkg>` puts `cwd` on `sys.path[0]`, and a
    # caller of this method may hand it a `cwd` whose contents it does not
    # own. Without this, a stdlib-shadowing module or a stray same-named
    # package at that root decides what the detached child imports.
...
def test_spawn_detached_reports_a_generic_message_for_a_bad_cwd_or_missing_executable(process, tmp_path, monkeypatch):
    ...
    alone, so its message must not overclaim "executable not found" for a
    failure that could just as well be a bad ``cwd``."""
    import pyforge.core.process as process_module

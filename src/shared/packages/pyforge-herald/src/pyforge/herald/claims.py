...
def create(
    ...
    *,
    ...
    No network step, so the whole body below runs inside ``locking.locked``
    on a sidecar ``<claims_path>.lock`` file (Story 13.1) -- see the module
    ...
    *,
    ...
    with locking.locked(lock_path):
        ...
        for claim in fresh_claims:
            entry_map = validated_by_claim.get(claim.id, {})
            revalidated_evidence = tuple(

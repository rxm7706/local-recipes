...
**Concurrency (Story 13.1, resolving `DW-1-4-2`).** ``create`` locks its
whole body (no network I/O, so nothing to carve out) around a sidecar
``<claims_path>.lock`` file. ``publish``/``revalidate``/``revalidate_all``
each call ``evidence_mod.validate_link`` per evidence entry -- a real HTTP
request -- so the lock must never span it: locking the whole function
(fine for ``create``) would mean one slow validation call blocks every
other concurrent claims writer for its duration, a liveness regression
worse than the lost-update bug this story fixes. Each of the three instead
runs evidence validation UNLOCKED first, then acquires the lock, re-reads
the FRESH claims state (not the pre-validation read -- another writer may
have changed it while validation was running), and applies the
already-computed results only to each evidence entry that still matches
what that fresh read shows; an entry a concurrent writer changed in the
interim has its now-stale validation result silently dropped rather than
clobbering the newer data, while every other entry's write still lands.
...
def create(
    ...
    *,
    ...
    No network I/O, so the whole body runs under ``locking.locked`` on a
    sidecar ``<claims_path>.lock`` file (Story 13.1, resolving
    ``DW-1-4-2``) -- unlike ``publish``/``revalidate``/``revalidate_all``,
    which validate evidence links before acquiring the lock (see module
...
def publish(
    ...
    *,
    ...
    Evidence validation runs UNLOCKED, before any lock is taken (Story
    13.1, module docstring's Concurrency section) -- only the subsequent
    fresh-read/apply/write span is locked, so a slow or hanging HTTP
    validation call never blocks a concurrent claims writer."""
    if validate is None:
    ...
    # Validation is over -- everything below is the locked
    # load/apply/write span. `validated_map` keys the ORIGINAL
    # (pre-validation) evidence entries to their validated replacements; a
    # fresh entry that no longer equals its pre-validation counterpart (a
    # concurrent writer changed it while validation was running) simply
    # misses the map and is carried through unchanged -- its stale
    # validation result is dropped rather than clobbering the newer data.
    lock_path = claims_path.with_name(claims_path.name + ".lock")
    with locking.locked(lock_path):
        claims = read_all(claims_path)  # fresh -- not the pre-validation read above
        index = next((i for i, c in enumerate(claims) if c.id == claim_id), None)
        if index is None:
            raise errors.ClaimNotFoundError(f"no claim found with id {claim_id!r}")
        claim = claims[index]
        ...
        validated_evidence = tuple(validated_map.get(e, e) for e in claim.evidence)
        edit_history = claim.edit_history
        if claim.thesis is not None and claim.thesis != final_thesis:
            edit_history = (
                *edit_history,
                ThesisVersion(thesis=claim.thesis, edited_at=timestamp_iso),
            )
        updated = replace(
            claim,
            status="published",
...
def revalidate(
    ...
    *,
    ...
    Evidence validation runs UNLOCKED, before any lock is taken -- see
    ``publish``'s docstring and the module docstring's Concurrency section;
    only the subsequent fresh-read/apply/write span is locked."""
    if validate is None:
    ...
    # Validation is over -- see `publish`'s equivalent comment on what
    # `validated_map.get(e, e)` protects against below.
    lock_path = claims_path.with_name(claims_path.name + ".lock")
    with locking.locked(lock_path):
        claims = read_all(claims_path)  # fresh -- not the pre-validation read above
        index = next((i for i, c in enumerate(claims) if c.id == claim_id), None)
        if index is None:
            raise errors.ClaimNotFoundError(f"no claim found with id {claim_id!r}")
        claim = claims[index]
        revalidated_evidence = tuple(validated_map.get(e, e) for e in claim.evidence)
        updated = replace(
            claim, evidence=revalidated_evidence, updated_at=timestamp_iso
        )
        claims[index] = updated
        _write_all(claims_path, claims)
    return updated
...
def revalidate_all(
    ...
    *,
    ...
    Evidence validation runs UNLOCKED, before any lock is taken -- see
    ``publish``'s docstring and the module docstring's Concurrency section.
    A claim created concurrently, after this run's unlocked snapshot was
    taken, was never part of the batch that got validated -- it is left
    untouched by the locked write rather than stamped with an
    ``updated_at``/evidence state this run never actually checked."""
    if validate is None:
        validate = evidence_mod.validate_link
    timestamp = now()
    timestamp_iso = timestamp.isoformat()
    ...
    lock_path = claims_path.with_name(claims_path.name + ".lock")
    with locking.locked(lock_path):
        fresh_claims = read_all(
            claims_path
        )  # fresh -- not the pre-validation read above
        updated_claims = []
        for claim in fresh_claims:
            ...
            revalidated_evidence = tuple(
                validated_map.get(e, e) for e in claim.evidence
            )

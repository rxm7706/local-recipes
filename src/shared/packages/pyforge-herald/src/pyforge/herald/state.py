...
def write(state_path: Path, slug: str, state: DeckState) -> None:
    ...
    guarantee this module makes. The load-mutate-replace span is also held
    under ``locking.locked`` (Story 13.1, resolving ``DW-1-4-2``): a sidecar
    ``<state_path>.lock`` file, so a second concurrent ``write`` for a
    different slug in the same file blocks and serializes instead of
    racing this one -- atomic replacement alone was crash-safety, not
    concurrency-safety, and two concurrent writers each loading the same
    starting document would otherwise silently drop whichever's
    ``os.replace`` lost the race.

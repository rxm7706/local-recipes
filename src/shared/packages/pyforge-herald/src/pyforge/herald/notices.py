...
today. And ``_write_markdown`` is a plain ``write_text`` (truncate-then-write,
not the temp-file + ``os.replace`` used for the index), so a concurrent
*reader* can still observe a partially written markdown file; serializing
writers does not make a non-atomic write atomic.
...
def author_notice(
    ...
    *,
    ...
    The whole read-modify-write body below runs inside ``locking.locked`` on
    a sidecar ``<index_path>.lock`` file (Story 13.1) -- see the module
    docstring's Concurrency section. ``notice_type``/``component`` are pure,
    no-I/O argument checks validated BEFORE the lock is acquired -- a call
    that is going to fail on basic validation fails fast without first
...
def publish_notice(
    ...
    *,
    ...
    The whole body below runs inside ``locking.locked`` -- see
...
def archive_rename(
    ...
    *,
    ...
    The whole read-modify-write body below runs inside ``locking.locked`` --
    see ``author_notice``'s docstring and the module docstring's Concurrency
    section. The component-name and self-redirect checks just below are
    pure, no-I/O argument validation, so they run BEFORE the lock is
    acquired -- a call that is going to fail on basic validation fails fast
    without first contending on the lock. The "already redirects" / "no
    notice exists yet" checks below depend on the fresh on-disk document,
    so they stay inside the lock."""

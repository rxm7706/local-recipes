"""Story 17.1: station-owned SKF skill + CAP-16 persona gates."""
...
GOLDEN = "herald-deck-status-e2e.json"
...
def test_compiled_skill_exists_with_provenance():
    ...
    assert "herald deck seed" in body
    assert "herald deck pull" in body
    assert "herald deck status" in body
    assert "pyforge herald" in body


def test_skf_validators_pass():
    root = _repo_root()
    pkg = _skill_package(root)
    skill_md = pkg / "SKILL.md"
    fm = subprocess.run(
...
def test_station_binary_without_pyforge_grammar_fails():
    events = [
        ...
        {"kind": "grammar", "argv": ["herald", "deck", "status", "pyforge-warden"]},
    ]
    try:
        validate_transcript(events)
    except PersonaContractError:
        return
    raise AssertionError("bare herald binary was accepted as FR-13 grammar")
...
def test_mcp_other_station_is_not_herald_face():
    events = [
        {
            "kind": "consult_content_skill",
            "skill": SKILL_NAME,
    ...
    try:
        ...
        return
    raise AssertionError("scribe MCP was accepted as the herald face")
...
def test_live_persona_skill_forbids_filesystem_and_adhoc_http():
    ...
    assert "pyforge herald" in skill
...
def test_persona_is_bmad_launcher_not_skf_compiled():
    ...
    assert "pyforge herald" in customize

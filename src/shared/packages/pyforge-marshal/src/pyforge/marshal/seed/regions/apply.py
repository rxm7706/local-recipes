...
def insert_region(
    ...
    *,
    ...
    ``InsertionOutcome.ALREADY_PRESENT`` immediately -- untouched, no write
    at all (unlike ``substitute_region``, which defers idempotence to a
    future plan layer per AD-60, THIS primitive's own epics AC names
    "already-present is a no-op" as its own behavior; see the module
    docstring). Otherwise resolves ``anchor`` via ``resolve_anchor`` and
    writes the rendered region through exactly one ``fs.replace_span(path,
    offset, offset, payload, ...)`` call -- a zero-width span, satisfying
    ``fs.replace_span``'s own ``0 <= start <= end <= len(original)`` contract
    with ``start == end``, so insertion needs no new primitive in ``fs.py``.
    A matched-anchor insertion carries no leading blank line; the
    append-fallback (``resolve_anchor`` returns ``matched=None``) payload is
    preceded by exactly one blank line (``"\\n"``), separating the appended
    region from whatever content precedes it.
    ...
    if any(span.name == name for span in existing):
        return InsertionOutcome.ALREADY_PRESENT

...
class RunStatusSnapshot:
    ...
    own ``Phase.DEFERRED``-only scope. Both ``deferred`` and ``tasks`` are
    trailing and defaulted so a caller constructing this value for a
    pause-only concern need not supply either."""

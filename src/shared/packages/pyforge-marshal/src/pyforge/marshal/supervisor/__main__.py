    ...
    *,
    ...
    try:
        ...
        while watched_alive and not deferred:
            ...
            if session_name is not None and harness_log_path is not None:
                ...
                if rung_index(rung) > rung_index(last_acted_rung):
                    if rung is LadderRung.NUDGE:
                        ...
                        try:
                            ...
                            # window (this story's own Always bullet): the
                            # new engine attempt gets a clean sample history
                            # and the ladder resets to its resting position.
                            watched_pid = new_pid
                            samples.clear()
                            rung = LadderRung.NONE
                    elif rung is LadderRung.DEFER:
                        intent_id = _append_intent(_DEFER_KIND, {"watched_pid": watched_pid})
                        _append_outcome(_DEFER_KIND, intent_id, {"watched_pid": watched_pid})
                        deferred = True

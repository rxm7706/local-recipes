...
def gather_conformance_findings(
    ...
    *,
    ...
    if plan.platform_mechanism is None:
        # Mirrors MRS-ADP-005's own message shape from `run_adapters_sync`,
        # reused verbatim (AD-31's own MRS-DEPLOY-003 precedent: same code,
        # same tier, a second call site) -- never a fabricated pass for a
        # platform with no declared mechanism-table row.
        if plan.unsupported_trees:
            findings.append(
                Finding(
                    code="MRS-ADP-005",
                    ...
                    message=(
                        ...
                        f"{', '.join(plan.unsupported_trees)}"
                    ),
                )
            )
        data["checks"] = []
        data["unevaluated_trees"] = list(plan.unsupported_trees)
        return data, findings

    desired_adapters_by_tree = {action.tree: action.adapters for action in plan.to_project}
    all_trees = sorted(set(desired_adapters_by_tree) | previously_projected)
    ...
    data["unevaluated_trees"] = list(report.unevaluated_trees)

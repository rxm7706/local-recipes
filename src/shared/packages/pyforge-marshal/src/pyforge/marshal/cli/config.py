...
def materialize(effective_policy: policy.EffectivePolicy, target_dir: Path) -> Path:
    ...
    try:
        ...
        if target_path.exists():
            ...
            return target_path
        # Story 14.2 deviation (see the story's Spec Change Log): the shared
        # primitive's temp file is mkstemp-based (private 0600), unlike the
        # PREVIOUS os.open(..., 0o666) approach that let the KERNEL apply the
        # process umask onto the final artifact. Reproducing that
        # umask-respecting permission via an explicit `mode=` -- a
        # os.umask(0)/restore probe reintroduces the very process-GLOBAL
        # toggle (briefly zeroing the umask for every other thread) the
        # prior implementation deliberately avoided, but it is the only way
        # to keep the materialized artifact's permission bits
        # umask-respecting (tests/unit/test_cli.py::
        # test_materialized_artifact_gets_umask_respecting_permissions)
        # while still routing the write through the shared mkstemp-based
        # primitive.
        current_umask = os.umask(0)
        os.umask(current_umask)
        atomic_write_bytes(target_path, expected_bytes, mode=0o666 & ~current_umask)
        return target_path

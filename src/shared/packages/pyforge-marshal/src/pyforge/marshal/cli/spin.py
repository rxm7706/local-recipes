...
def _render_text(data: Mapping[str, object], findings: tuple[Finding, ...], command: str = "factory spin") -> str:
    ...
    if "escalated" in data:
        # Story 3.12 (retry escalation, AD-26) -- review finding: every
        # other `run_resume`-only field here (`story_key`,
        # `resolution_reference`) has a dedicated line, but this one was
        # missed, leaving the floor-raise decision invisible in the
        # DEFAULT (non-JSON) output -- the exact surface an unattended

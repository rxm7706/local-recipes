...
#: How long the detached campaign supervisor waits between cycles. A cycle
#: is cheap (ledger reads + git/process facts) and a story takes minutes to
#: hours, so this matches the per-story supervisor's own 60 s tick.
...
def add_factory_drain_subparser(factory_subparsers: argparse._SubParsersAction) -> None:
    ...
    parser.add_argument(
        "--campaign",
        default=None,
        help="Reuse an existing campaign run id instead of minting one.",
    )
    parser.add_argument(
        "--loop",
        action="store_true",
        help=(
            "Run cycles until the campaign completes. This is what the "
            "DETACHED campaign supervisor runs; a foreground invocation "
            "never waits (no foreground path busy-waits on a session)."
        ),
    )
    parser.add_argument(
        "--format",
...
def _journal_fleet_cycle(
    ...
    base = cycle_index * 2
    intent = build_entry(
        id=JournalEntryId(writer_id, base),
        ts=_format_entry_ts(_now_utc()),
        run_id=run_id,
        kind=dispatch_fleet.KIND_FLEET_CYCLE,
        phase=Phase.INTENT,
        payload={"cycle": cycle_index, "mode": report.data.get("mode")},
    )
    outcome = build_entry(
        id=JournalEntryId(writer_id, base + 1),
        ts=_format_entry_ts(_now_utc()),
        run_id=run_id,
        kind=dispatch_fleet.KIND_FLEET_CYCLE,
        phase=Phase.OUTCOME,
    ...
    try:
        ...
        findings.append(
            Finding(
                code="MRS-DRAIN-009",
                severity=Severity.WARN,
                message=f"fleet cycle {cycle_index} ran but could not be journaled: {exc}",
            )
        )
...
def _spawn_campaign_supervisor(
    *,
    ...
    the operator's foreground invocation NEVER waits on a session, so the
...
def run_fleet_drain(
    ...
    *,
    ...
    campaign_blocked: dict[str, dict[str, str]] = {}
    cycles = 0
    report: FleetCycleReport | None = None
    while True:
        ...
        _journal_fleet_cycle(fs, run_dir, run_id, cycles, report, findings)
        cycles += 1
        if not loop or report.complete:
            break
        if max_cycles and cycles >= max_cycles:
            break
        # Only the DETACHED supervisor ever reaches this sleep -- a
        # foreground `marshal factory drain` runs one cycle and returns.
    ...
    if not loop and not once and not report.complete:
        try:
            ...
            findings.append(
                Finding(
                    ...
                    severity=Severity.WARN,
                    message=(
                        f"cycle {cycles - 1} completed but the campaign "
                        f"supervisor could not be spawned: {exc} -- re-run "

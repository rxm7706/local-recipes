...
``plan.to_promote`` (durable, not yet promoted, valid spec) or
``plan.missing_spec_keys`` (durable, no Tier-3 spec at all). Computed fresh
on every call -- no caching, mirroring ``run_promote``'s own "no
promotion-state flag is cached anywhere" rule and AD-29's teardown-side
"reachability computed at teardown time, never a journal flag". A candidate
whose Tier-3 spec exists but fails ``is_valid_spec_text`` (``MRS-DEPLOY-002``,
zero-byte/truncated) is deliberately NOT included -- the story's own Always
bullet names only "durable-but-unpromoted" and "durable-with-no-spec-at-all"
as unreachable; a broken-but-present spec is a distinct paper-trail gap
``deploy promote`` already reports on its own next run.
...
class _PromotionScan:
    ...
    __slots__ = ("plan", "findings", "combined_subjects", "template", "already_promoted")
...
def unreachable_promotions_for_slug(
    ...
    *,
    ...
    that is durable-but-unpromoted (``plan.to_promote``) or
    durable-with-no-spec-at-all (``plan.missing_spec_keys``) -- via the SAME
    ``_scan_promotions`` pipeline ``run_promote`` uses, computed fresh on
    every call (no caching, AD-29: "reachability computed at teardown time,
    never a journal flag"). A durable story whose Tier-3 spec exists but
    fails validation (``MRS-DEPLOY-002``) is deliberately NOT included --
    the story's own Always bullet names only these two cases.

    Returns ``()`` -- never raises -- for a malformed/empty slug, or when
    ``_scan_promotions`` could not determine durability at all (its own
    ``plan is None`` case, e.g. local ``main``'s history could not be read):
    this function's signature carries no findings channel (mirrors the
    docstring promise the stub it replaces made -- "the call site and
    contract are permanent"), so an undeterminable state degrades to "name
    nothing unreachable" rather than raising past a caller with no way to
    surface a finding. Teardown's OTHER refusal channels (dirty worktree,
    unmerged branch) are unaffected by this degrade -- only the AD-29 check
    itself goes silent when it genuinely cannot be answered."""
...
def _run_snapshot_candidates(root: Path, project_slug: str, story_key: identity.StoryKey) -> list[dict[str, object]]:
    ...
    try:
        matches = sorted(runs_dir.glob(f"*/{stem}*.md"))
    except OSError:
        matches = []

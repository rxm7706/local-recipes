...
def _gather_claimed_commits(
    ...
    for task in snapshot.tasks:
        try:
            ...
            continue
        claims.append(
            status.ClaimedCommit(story_key=story_key, claimed_commit_sha=task.commit_sha)
        )
    return tuple(claims)
...
def _run_resync_commands(
    ...
    arguments), run via ``ProcessPort.run``, classify via
    ``core.status.classify_resync_outcome`` (this story's own
    MRS-DEPLOY-019/020 codes -- never ``core.gate.classify_outcome``'s
    hardcoded ``MRS-GATE-*`` codes, a different policy key's own area).
    Every failure is reported, never silently swallowed."""

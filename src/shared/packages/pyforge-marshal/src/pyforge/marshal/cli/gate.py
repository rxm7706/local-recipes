...
when ``--story`` is missing, the active project cannot be resolved, the
story key does not parse, or ``VcsPort.changed_files`` fails against the
project's loop home. The frozen set comes from ``core.journal.FoldResult.

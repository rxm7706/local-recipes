...
resolvable active project, the binding check is skipped too -- there is no
``specs/`` directory to look in, mirroring ``--scope-check``'s own
``MRS-GATE-009`` "no resolvable active project" precondition. When
``--run <id>`` was requested but its fold is unavailable, the binding
check is ALSO skipped, for the identical reason ``--scope-check``'s own
result is omitted then: ``MRS-GATE-005`` already reports the one root
cause in the same envelope, and a second finding here would be a redundant
symptom of it. ``core.spec_binding.parse_success_signal`` extracts the
spec's declared commands (pure); ``core.gate.check_spec_binding`` compares
them against ``effective.verify_commands`` (pure) -- the commands this
invocation's own policy declares, independent of whether this particular
run actually executed them.
"""
...
def run_evaluate(
    ...
    *,
    ...
    # story key AND the active project resolved (so a specs/ lookup was
    # even possible), confirm the story's own tracked spec Success signal
    ...
    if (
        ...
        and project_slug
        and policy._is_valid_project_slug(project_slug)
        and not (args.run_id is not None and fold_result is None)
    ):
        declared_commands = (
            spec_binding.parse_success_signal(spec_text) if spec_text is not None else None
        )

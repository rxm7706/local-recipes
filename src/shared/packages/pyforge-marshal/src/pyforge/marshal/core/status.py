        ...
        if claimed_sha is not None and not durable:
            findings.append(
                Finding(
                    ...
                    message=(
                        f"story {key}: the harness claims commit "
                        f"{claimed_sha!r} landed (RunStatusSnapshot's own "
                        "per-task commit_sha), but git's own "
                        "merged_story_keys does not confirm it -- reported "
                        "only; the durable answer stays git's, per AD-33"
                    ),
                )
            )

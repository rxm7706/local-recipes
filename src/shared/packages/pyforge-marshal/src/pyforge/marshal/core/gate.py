...
Three real codes register here (the registry's seventh real caller, joining
...
def classify_outcome(
    ...
    *,
    ...
    ``failure_code`` (a registered ``core.findings`` code, e.g.
    ``MRS-GATE-002``/``MRS-GATE-003``) and ``failure_reason`` (the
    human-readable message) are then REQUIRED (``ValueError`` otherwise):
    this function has no other way to know why the command is missing an
    outcome, and a silent default would hide a real caller bug. The
    returned report carries ``resolvable: False`` and ``returncode: None``,
    with no ``stdout``/``stderr`` keys (there is no captured output for a

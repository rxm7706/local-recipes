...
def _read_bearer(path: str) -> str | None:
    if not path:
        return None
    try:
        token = path if path.startswith("ey") else open(path, encoding="utf-8").read()
    except OSError:
        return None
    token = token.strip()
...
def fetch_story_tasks(
    ...
    *,
    ...
    if bearer is None:
        return None
    client = PyForgeStationClient(station=_STATION, base_url=host, transport=transport)
    mint = HostMintClient(f"{host}/assertion/mint/", transport=client._urllib)  # noqa: SLF001
    try:
        assertion = mint.emit(idp_bearer=bearer, station=_STATION)
    except Exception:  # noqa: BLE001 -- best-effort read degrades
        return None
    mcp_url = f"{host}/stations/{_STATION}/mcp"
    body = json.dumps(
        _jsonrpc_call(
            _LIST_TOOL,
    ...
    headers = PyForgeStationClient(
        ...
        base_url=host,
        assertion=assertion,
        transport=transport or client._urllib,  # noqa: SLF001
    ).build_headers(
        extra={"Content-Type": "application/json", "Accept": "application/json"},
    )
    try:
        raw = (transport or client._urllib)("POST", mcp_url, headers, body)  # noqa: SLF001

    ...
    def merge_branch(self, repo_root: Path, branch: str, *, into: str, subject: str) -> str:
        ...
        try:
            ...
            if remove_result.returncode != 0:
                # Best-effort fallback (e.g. `add` above never completed, so
                # there is nothing registered to `remove`): never raises --
                # a cleanup failure must not mask the real outcome (success
                # or the original VcsCommandError) above.

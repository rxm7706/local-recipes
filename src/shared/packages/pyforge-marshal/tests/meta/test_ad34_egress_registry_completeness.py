...
def _token_shape_pattern_references(tree: ast.Module) -> list[int]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            ...
            # A COPIED regex literal (review finding). Importing the private
            # name was the only thing the guard could see, yet the threat its
            # own docstring names -- "no call site can hand-roll its own
            # redaction against a COPY of this vocabulary" -- is served far
            # more naturally by pasting `re.compile(r"ghp_[A-Za-z0-9]{36,}")`
            # into another module, which was completely invisible. The
            # signature is a known token prefix immediately followed by a
            # regex character class, so ordinary prose mentioning `ghp_` or
            # a test fixture containing a literal token does not trip it.
            if _COPIED_TOKEN_REGEX.search(node.value):
                violations.append(node.lineno)

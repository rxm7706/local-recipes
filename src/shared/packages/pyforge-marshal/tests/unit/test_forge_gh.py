...
def test_find_open_pr_returns_pr_info(forge, monkeypatch):
    ...
    assert result == PrInfo(number=42, url="https://example/pr/42", state="open")
...
def test_create_pr_creates_then_looks_up_the_new_pr(forge, monkeypatch):
    created = json.dumps([{"number": 7, "url": "https://example/pr/7", "state": "open"}])
    ...
    assert result == PrInfo(number=7, url="https://example/pr/7", state="open")
...
def test_update_pr_edits_then_views(forge, monkeypatch):
    viewed = json.dumps({"number": 9, "url": "https://example/pr/9", "state": "OPEN"})
    ...
    assert result == PrInfo(number=9, url="https://example/pr/9", state="open")

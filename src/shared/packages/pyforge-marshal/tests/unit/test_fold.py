...
def test_entries_tie_break_by_counter_numerically_at_equal_ts_and_writer():
    """The "then counter" half of the AD-28 tie-break (review finding): the
    test above differs in `writer_id`, so the comparison resolves before
    `counter` is ever consulted. Counters 2 and 10 order numerically here;
    were the sort key ever to stringify the counter -- a plausible refactor,
    since `ts` is already sorted as text -- `10 < 2` lexicographically and
    the rest of the suite would still pass."""
    second = build_entry(
    ...
    tenth = build_entry(
        id=_valid_id(writer_id="cli-1", counter=10),
        ts=_ts(0),
        run_id="run-1",
        kind="k",
        phase=Phase.OBSERVATION,
    ...
    result = fold([_line(tenth), _line(second)])
    assert result.entries == (second, tenth)
...
def test_small_inline_payload_without_a_sidecar_ref_key_is_untouched():
    """A payload naturally small enough to inline, and NOT shaped like the
    placeholder, never goes through sidecar resolution at all."""
    entry = build_entry(
        id=_valid_id(),
        ts=_ts(0),
        run_id="run-1",
...
def test_small_payload_shaped_exactly_like_the_placeholder_round_trips():
    """Review finding, verified live: the read side can only tell a
    placeholder from real data by SHAPE, so a small payload that is itself
    exactly ``{"sidecar_ref": <str>}`` used to be inlined by
    `prepare_for_write` and then read back as a placeholder -- `fold` found
    no blob for it and quarantined a perfectly well-written entry as
    MRS-JOURNAL-002, making its whole (story, kind) domain unevaluable.

    The predecessor of this test claimed to cover exactly this case in its
    name and docstring while constructing `payload={"a": 1}` -- a payload
    with no `sidecar_ref` key at all -- so it asserted a proposition nothing
    in its body exercised and green-lit the broken behavior.

    `prepare_for_write` now routes the collision to a real sidecar however
    small it is, which makes the shape test true by construction.

    The `sidecar_ref` VALUE here is deliberately not this entry's own
    derived blob path (review finding, verified live): when it was, the
    payload was byte-identical to the placeholder `prepare_for_write`
    emits, so `entries[0].payload == payload` held whether the blob was
    resolved or the placeholder was passed straight through as data -- the
    read half of this round-trip asserted nothing. A distinct value makes
    the two outcomes observably different."""
    payload = {"sidecar_ref": "some/other/entrys/blob.json"}
    entry = build_entry(
        id=_valid_id(counter=0),
        ts=_ts(0),
        run_id="run-1",
...
# --- review-pass regressions: recovery must never itself abort the fold ----


def test_invalid_utf8_bytes_line_is_quarantined_not_a_crash():
    """Review finding, verified live: `json.loads` accepts `bytes`, and the
    sequence guard rejects only a bare `bytes` for `lines` ITSELF, never a
    per-element one. An element carrying invalid UTF-8 raised
    UnicodeDecodeError -- a ValueError subclass that is NOT a
    JSONDecodeError -- from `_best_effort_recover`, which runs INSIDE
    `fold`'s own except handler, so it escaped `fold` entirely and aborted
    the whole run: the exact opposite of "one bad line never aborts the
    fold", and every other entry was lost with it."""
    good = build_entry(
        id=_valid_id(),
        ts=_ts(0),
        run_id="run-1",
    ...
    result = fold([b"\xff\xfe not json", _line(good)])
    assert good in result.entries
    assert len(result.quarantined) == 1
    assert result.quarantined[0].finding.code == "MRS-JOURNAL-001"


def test_invalid_utf8_sidecar_blob_is_quarantined_as_a_sidecar_failure():
    """Same ValueError-is-not-JSONDecodeError gap on the blob decode path:
    a `bytes` blob with invalid UTF-8 escaped to `fold`'s outer catch and
    was misreported as MRS-JOURNAL-001, a LINE parse failure, defeating the
    discrimination the two codes exist to make."""
    entry = build_entry(
        id=_valid_id(counter=9),
        ts=_ts(0),
...
def test_over_long_story_key_digits_are_quarantined_not_a_crash():
    """Review finding, verified live: the same class of gap on the OTHER
    catch in `_best_effort_recover`. `normalize` documents
    MalformedStoryKeyError, but it ends in `int(match.group("epic"))`, and
    CPython >= 3.11 caps integer-string conversion (default 4300 digits,
    lowerable via `sys.set_int_max_str_digits`/`PYTHONINTMAXSTRDIGITS`) with
    a PLAIN ValueError. Caught only as MalformedStoryKeyError, that escaped
    `_best_effort_recover` -- which runs INSIDE `fold`'s except handler --
    and therefore escaped `fold` itself, losing every other entry in the
    run."""
    good = build_entry(
        id=_valid_id(),
        ts=_ts(0),
    ...
    over_long = json.dumps({"story": "9" * 5000 + ".1", "kind": "k", "ts": "nope"})
    result = fold([over_long, _line(good)])
    assert good in result.entries
    assert len(result.quarantined) == 1
    record = result.quarantined[0]
    assert record.finding.code == "MRS-JOURNAL-001"
    # `story` never recovered, so AD-30 widens the scope to the whole run
    # rather than leaving an asymmetric (None, kind) pair behind.
    assert (record.story, record.kind) == (None, None)


# --- review-pass regressions: FoldResult's own construction invariants -----


def test_fold_result_rejects_a_generator_for_a_tuple_field():
    """Review finding, verified live: a generator passed for a declared
...
def test_fold_result_rejects_a_list_for_a_tuple_field():
    with pytest.raises(ValueError, match="quarantined must be a tuple"):
        FoldResult(
            entries=(),
            open_intents=(),
            orphaned_outcomes=(),
...
def test_fold_result_rejects_entries_out_of_ad28_total_order():
    """Review finding, verified live: AD-28's total order is a property of
    this TYPE -- both `by_kind` and `for_story` advertise results "in
    `entries`'s own AD-28 total order", so a consumer reading

...
class TestFormatRollupByHarness:
    """Story 46.5 (CAP-193): ``_format_rollup_by_harness``'s text rendering
    -- one line per harness, each under its own currency, never a single
    blended/summed total."""

    def test_renders_each_harness_under_its_own_currency(self) -> None:
        rollup = {
            "status": "ok",
            "harnesses": {
                "claude": {
                ...
                "cursor": {
                    "currency": "quota-burn",
                    "silent": {"output_compression_saved": ["caveman-skill-not-deployed"]},
                    "configured": {},
                    "runs": 1,
                },
            },
        ...
        assert "claude (usd)" in text
        assert "cursor (quota-burn)" in text
        assert "total" not in text

    def test_empty_or_no_data_status_renders_nothing(self) -> None:
...
class TestRunStatus:
    ...
    def test_savings_rollup_by_harness_absent_for_whole_fleet_status(self, tmp_path, capsys):
        """Story 46.5 (CAP-193): whole-fleet status (no ``--project``)
        computes no cross-project rollup -- each project's dispatch-runs
        are scoped to that project only."""
    ...
    def test_savings_rollup_by_harness_splits_two_harnesses_by_currency(
        self, tmp_path, capsys
    ):
        """Story 46.5 (CAP-193): a project scoped by ``--project`` whose
        ``dispatch-runs/`` span two distinct ``harness_profile`` values
        renders each harness's savings under its OWN ``currency``, never a
        single summed-across-harnesses total."""
        slug = "acme"
        runs_dir = (
            tmp_path
        ...
        for run_id, profile, savings in (
            ("run-claude", "claude", {"wire_compression_saved": 500}),
            ("run-cursor", "cursor", {"output_compression_saved": 100}),
        ):
        ...
        assert "total" not in rollup

        # And the text projection renders each under its own currency too.
        text_exit_code = status_cli.run_status(
            _args(project=slug, format="text"),
            vcs=vcs,
            fs=LocalFs(),
            harness=_FakeHarness(),
            process=_FakeProcess(),
        ...
        assert "claude (usd)" in out
        assert "cursor (quota-burn)" in out

...
class TestQuickDevCompletions:
    def test_corroborated_non_native_present_in_ledger_not_done_is_advanced(self):
        """The headline case: a quick-dev-completed, corroborated story
        with a live ledger row not yet done."""
        result = status.quick_dev_completions(
            ledger_done_keys=frozenset(),
            ledger_all_keys=frozenset({"5.9"}),
            corroborated_keys=frozenset({"5.9"}),
            marshal_native_keys=frozenset(),
        )
        assert result == frozenset({"5.9"})

    def test_marshal_native_key_is_excluded_even_if_corroborated(self):
        """A key landed via `deploy land-story`/bmad-loop -- Story 5.4's
        own sync already owns this case."""
        result = status.quick_dev_completions(
            ledger_done_keys=frozenset(),
            ledger_all_keys=frozenset({"4.3"}),
            corroborated_keys=frozenset({"4.3"}),
            marshal_native_keys=frozenset({"4.3"}),
        )
    ...
    def test_uncorroborated_key_is_excluded_even_if_present_in_ledger(self):
        ...
        collision -- never triggers a write on its own."""
        result = status.quick_dev_completions(
            ledger_done_keys=frozenset(),
    ...
    def test_key_absent_from_ledger_is_excluded_never_invented(self):
        ...
        the ledger map entirely" -- corroborated and non-native, but no
        ledger row at all."""
        result = status.quick_dev_completions(
            ledger_done_keys=frozenset(),
            ledger_all_keys=frozenset(),
            corroborated_keys=frozenset({"9.1"}),
            marshal_native_keys=frozenset(),
        )
        assert result == frozenset()
    ...
    def test_mixed_batch_advances_only_the_eligible_keys(self):
        ...
        eligible key, in a single call -- proves the four conditions
        combine correctly, not just in isolation."""
        result = status.quick_dev_completions(
            ledger_done_keys=frozenset({"1.1"}),  # already done
            ledger_all_keys=frozenset({"1.1", "5.9"}),  # 9.1 has no row
            corroborated_keys=frozenset({"1.1", "4.3", "5.9", "9.1"}),
            marshal_native_keys=frozenset({"4.3"}),  # Marshal-driven
        )
        assert result == frozenset({"5.9"})

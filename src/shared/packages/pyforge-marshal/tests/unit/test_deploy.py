...
def test_reconcile_completions_missing_ledger_row_reports_mrs_deploy_026(
    ...
    ledger advance (review fix, patch 2, 2026-08-12): `to_promote_scoped`
    no longer depends on the ledger's own advance set at all, so the spec
    IS still promoted here even though the ledger row is missing."""
    monkeypatch.setattr(deploy_module, "repo_root", lambda: tmp_path)
    _write_tier3_spec(tmp_path, "acme", "5-9-title", _VALID_SPEC)
...
def test_reconcile_completions_already_done_key_is_a_clean_no_op(
    ...
    system (ledger already `done` AND the spec already promoted -- since
    patch 2's review fix decouples promotion scope from the ledger's own
    advance set, a key that is merely `done` in the ledger but NOT yet
    promoted is no longer a no-op for promotion; see the dedicated
    `test_reconcile_completions_already_promoted_spec_still_advances_the_
    ledger`/promotion-retry tests for that independent axis) produces zero
    changes and no finding on re-run."""
    monkeypatch.setattr(deploy_module, "repo_root", lambda: tmp_path)
    _write_tier3_spec(tmp_path, "acme", "5-9-title", _VALID_SPEC)
...
def test_reconcile_completions_ledger_commit_failure_still_attempts_promotion(
    ...
    outcome of the write+commit sequence (review fix, patch 1, 2026-08-12,
    high-severity): empty here, even though the key was ELIGIBLE, because
    the commit itself never durably landed -- the finding explains why."""
    monkeypatch.setattr(deploy_module, "repo_root", lambda: tmp_path)
    _write_tier3_spec(tmp_path, "acme", "5-9-title", _VALID_SPEC)
    ledger_path = _write_ledger(tmp_path, "acme", _ledger_text(("5-9-title", "backlog")))
...
def test_reconcile_completions_ledger_commit_failure_recovers_on_a_later_run(
    tmp_path, capsys, monkeypatch
):
    """Proves patch 1 + patch 2 compose correctly for full recovery: a
    ledger commit failure this run reports an empty `advanced` (patch 1),
    and -- once the transient failure is no longer simulated -- a LATER
    run still finds the key eligible (the durable ledger state git/the
    harness report never actually advanced, since the first run's commit
    never landed) and successfully retries the ledger advance. The spec
    itself was already promoted on the first run (patch 2's decoupling),
    so the second run has nothing left to promote."""
    monkeypatch.setattr(deploy_module, "repo_root", lambda: tmp_path)
    _write_tier3_spec(tmp_path, "acme", "5-9-title", _VALID_SPEC)
    ledger_path = _write_ledger(tmp_path, "acme", _ledger_text(("5-9-title", "backlog")))
    ...
    first_exit = deploy_module.run_reconcile_completions(
        _args(), vcs=failing_vcs, fs=LocalFs(), harness=harness
    )
    first_payload = json.loads(capsys.readouterr().out)
    ...
    # A later run: the failure is no longer simulated. The tracked ledger
    # is reset to `backlog` (the first run's own local write succeeded but
    # its commit never landed -- `render_ledger_advancements` is
    # idempotent, so re-applying it against the SAME already-"done" text
    # would be a byte-identical no-op; resetting here models the durable,
    # actually-committed state the harness also reports: genuinely still
    # `backlog`) and the harness still reports `backlog` too.
    _write_ledger(tmp_path, "acme", _ledger_text(("5-9-title", "backlog")))
    non_failing_vcs = _FakeVcs(main_subjects=(_quick_dev_subject("5-9-title"),))
    second_exit = deploy_module.run_reconcile_completions(
        _args(), vcs=non_failing_vcs, fs=LocalFs(), harness=harness
    )
    second_payload = json.loads(capsys.readouterr().out)
...
class _SpecsDirLockRaisingFs(LocalFs):
    ...
    proves the ledger's own advisory lock (Story 5.9's review-fix pass,
    ``ledger_path.parent``, a SEPARATE resource) is independently
    acquirable even while ``_execute_promotion_plan``'s own ``specs_dir``
    lock is contended."""
...
def test_reconcile_completions_promotion_lock_failure_is_retried_on_a_later_run(
    tmp_path, capsys, monkeypatch
):
    """Proves patch 2 (high-severity): a promotion failure (lock
    contention on `specs_dir`) for a key whose ledger advancement
    SUCCEEDED in the SAME run used to permanently orphan that key's
    promotion, because `quick_dev_completions`'s own eligibility condition
    requires absence from `ledger_done_keys` -- once the ledger says
    `done`, the key could never reappear in a later run's own `advanced_
    dot_keys` set, and `to_promote_scoped` used to be filtered by exactly
    that set. `to_promote_scoped` is now scoped to `quick_dev_candidates`
    directly, independent of the ledger's own advance set, so a later run
    retries the promotion regardless of the ledger's already-`done` state."""
    monkeypatch.setattr(deploy_module, "repo_root", lambda: tmp_path)
    _write_tier3_spec(tmp_path, "acme", "5-9-title", _VALID_SPEC)
    ledger_path = _write_ledger(tmp_path, "acme", _ledger_text(("5-9-title", "backlog")))
    vcs = _FakeVcs(main_subjects=(_quick_dev_subject("5-9-title"),))
    ...
    first_exit = deploy_module.run_reconcile_completions(
        _args(), vcs=vcs, fs=promotion_locked_fs, harness=harness
    )
    first_payload = json.loads(capsys.readouterr().out)
    ...
    # `quick_dev_completions` correctly excludes it from `advanced_dot_
    # keys` (AD-21 convergence, already done) -- but the spec is STILL
    # promoted, proving `to_promote_scoped` no longer depends on the
    # (now-empty) `advanced_dot_keys` set.
    harness_second = _FakeReconcileHarness(ledger_statuses=(("5-9-title", "done"),))
    second_exit = deploy_module.run_reconcile_completions(
        _args(), vcs=vcs, fs=LocalFs(), harness=harness_second
    )
    second_payload = json.loads(capsys.readouterr().out)
...
def test_reconcile_completions_ledger_lock_contention_reports_warn_and_skips_the_write(
    tmp_path, capsys, monkeypatch
):
    """Proves patch 3 (medium-severity): the ledger's own read-modify-
    write sequence now acquires an advisory lock on the ledger's OWN
    parent directory (a SEPARATE resource from `_execute_promotion_plan`'s
    own `specs_dir` lock) -- proven here against a GENUINE, real
    `fcntl.flock` held by another thread (mirrors `test_promote_hits_the_
    real_contention_path_when_another_holder_has_the_lock`'s own real-
    contention style), not a simulated one. The second (foreground) run
    reports the WARN and never touches the ledger; spec promotion for the
    same key still proceeds independently."""
    monkeypatch.setattr(deploy_module, "repo_root", lambda: tmp_path)
...
def test_reconcile_completions_dirty_ledger_skips_the_write_but_promotion_still_proceeds(
    tmp_path, capsys, monkeypatch
):
    """Proves patch 4 (medium-severity): a pre-existing uncommitted edit to
    the tracked ledger (an ordinary, expected case -- other writers,
    including bmad-loop itself, touch this file) must never be silently
    folded into this command's own commit -- the write is skipped, WARN
    reported, and spec promotion for the SAME eligible key still proceeds
    ...
    before = ledger_path.read_text(encoding="utf-8")
    vcs = _FakeVcs(
        main_subjects=(_quick_dev_subject("5-9-title"),),
    ...
    finding = next(
        ...
        if f["code"] == "MRS-DEPLOY-024" and "uncommitted changes" in f["message"]
    )
    assert finding["severity"] == "warn"
    assert payload["data"]["advanced"] == []
    assert payload["data"]["advanced_count"] == 0
...
def test_reconcile_completions_already_promoted_spec_still_advances_the_ledger(
    tmp_path, capsys, monkeypatch
):
    """The scenario the review flagged as missing coverage: a key
    corroborated via `scan.already_promoted` (a valid, git-committed
    tracked copy already exists in the archive -- `_write_tracked_spec`)
    rather than `scan.plan.to_promote`, with the ledger row still
    `backlog`. Corroboration only requires SOME durable, valid spec
    (`to_promote UNION already_promoted`, `_scan_promotions`'s own
    established convention), so the ledger still advances to `done`;
    there is nothing left to promote for this key."""
    monkeypatch.setattr(deploy_module, "repo_root", lambda: tmp_path)
    _write_tier3_spec(tmp_path, "acme", "5-9-title", _VALID_SPEC)
    _write_tracked_spec(tmp_path, "acme", "5-9-title", _VALID_SPEC)
    ledger_path = _write_ledger(tmp_path, "acme", _ledger_text(("5-9-title", "backlog")))

...
def test_help_lists_gate_subcommand(capsys):
    ...
    assert exit_code == 0
    assert (
        "{config,init,homes,preflight,teardown,gate,factory,deploy,land,retire,status,check,adapters,upstream,seed,refresh,chain,planning,context,login,benchmark}"

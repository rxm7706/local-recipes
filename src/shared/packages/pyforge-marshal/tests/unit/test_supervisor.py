...
class FakeHarness:
    ...
    ``fail_stop``/``fail_resume`` to exercise the ``HarnessError`` path."""
...
def test_normal_attach_journals_attach_then_heartbeats_then_detach():
    ...
    # each tick, against the REAL session/log target (Story 3.5, closing
    # Story 3.4's own placeholder gaps) -- the non-advancing FakeClock means
    # no ladder action ever fires (elapsed is always 0), so the observation
    # shape above is unaffected.
    assert observer.pane_content_calls == [_SESSION_NAME] * 3
    assert observer.mtime_calls == [_harness_log_path()] * 3
...
def test_first_threshold_crossing_fires_a_nudge_intent_then_outcome():
    ...
    # threshold_s = 150 (2.5min) against a 60s tick: ticks 1-3 stay under
    # one threshold, tick 4 crosses it (240s / 150s = 1.6 -> rung 1), tick 5
    # stays at the SAME rung (no re-fire).
    rc = run_supervisor(
...
def test_second_threshold_crossing_fires_stop_and_retry():
    ...
    harness = FakeHarness()
    # threshold_s = 90 (1.5min): tick3 (180s) crosses NUDGE(1), tick4 (240s)
    # crosses STOP_AND_RETRY(2) -- 240/90=2.67 -> 2.
    rc = run_supervisor(

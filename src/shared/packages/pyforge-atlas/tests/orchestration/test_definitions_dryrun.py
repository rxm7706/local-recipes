...
def test_evaluate_raw_scan_tolerates_malformed_cursor(bad):
    ...
    Regression: `set(seen)` on a nested-container cursor used to raise TypeError."""
    from pyforge.atlas.orchestration.wiki_events import evaluate_raw_scan

    d = evaluate_raw_scan(["a.md"], bad)  # must not raise
    # 'nothing seen' -> the current doc reads as new -> a run (the safe, idempotent degrade).
    assert d.run and d.new_docs == ("a.md",)


def test_compile_sensor_survives_a_corrupt_cursor(defs):
    """End-to-end: a corrupt cursor must not crash the sensor generator (it degrades to a run,

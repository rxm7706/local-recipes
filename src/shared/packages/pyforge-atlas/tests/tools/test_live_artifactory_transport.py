...
def _load_transport_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "_live_artifactory_transport_test", path
    )
...
def test_api_key_sets_jfrog_header(transport_mod, monkeypatch):
    ...
    transport = transport_mod.live_transport(
        "https://artifactory.example.com", api_key="tok123"
    )
    response = transport(
        AqlRequest(method="POST", url="/api/search/aql", body={"q": 1})
    )
...
def test_username_password_sets_basic_auth(transport_mod, monkeypatch):
    ...
    monkeypatch.setattr(transport_mod.requests, "request", _fake_request)
    transport = transport_mod.live_transport(
        "https://artifactory.example.com", username="alice", password="s3cret"
    )

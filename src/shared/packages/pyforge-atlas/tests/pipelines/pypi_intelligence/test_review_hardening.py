...
def test_match_source_urls_skips_malformed_list_conda_name():
    base = pd.DataFrame(
        {"pypi_name": ["a"], "conda_name": ["a"], "match_source": ["parselmouth"]}
    )
    ...
    cand = pd.DataFrame(
        {"pypi_name": ["b", "c"], "conda_name": [["not", "scalar"], "c-conda"]}
    )
...
def test_flag_cross_channel_skips_malformed_list_conda_name():
    df = pd.DataFrame(
        {"conda_name": [["x"], "numpy"], "channel": ["bioconda", "bioconda"]}
    )
...
def test_phase_r_upsert_one_missing_key_replaces_prior_missing_row():
    ...
    out = phase_r_upsert_one(
        base, {"pypi_name": None, "packaging_shape": "pure-python"}
    )
...
def test_notes_survive_phase_s_rerun_via_enriched_carry():
    ...
    assert (
        enriched.iloc[0]["notes"] == "operator: hold - license review"
    )  # R carries it

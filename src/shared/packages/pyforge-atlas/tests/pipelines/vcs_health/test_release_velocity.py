...
def test_matched_version_lag_and_ms_to_seconds_boundary():
        ...
        [
            {
                "pypi_name": "foo",
                "version": "1.2.3",
                "upload_time_iso_8601": _iso(upload_s),
            }
        ]
    )
    repo = _repo(
        [{"conda_name": "foo", "version": "1.2.3", "timestamp": avail_s * 1000}]
    )  # ms
    ...
    assert (
        row["release_lag_qualifies"] is True
        or bool(row["release_lag_qualifies"]) is True
    )
...
def test_seconds_magnitude_timestamp_is_not_divided():
    ...
    pcv = _pcv(
        [{"pypi_name": "foo", "version": "9.9", "upload_time_iso_8601": _iso(upload_s)}]
    )
    repo = _repo(
        [{"conda_name": "foo", "version": "9.9", "timestamp": avail_s}]
    )  # seconds
    out = derive_release_velocity(
        pcv, repo, _map([{"pypi_name": "foo", "conda_name": "foo"}]), now=_NOW
    )
...
def test_ninety_day_guard_stale_release_does_not_qualify():
    ...
    pcv = _pcv(
        [{"pypi_name": "old", "version": "1.0", "upload_time_iso_8601": _iso(upload_s)}]
    )
    repo = _repo([{"conda_name": "old", "version": "1.0", "timestamp": avail_s * 1000}])
    out = derive_release_velocity(
        pcv, repo, _map([{"pypi_name": "old", "conda_name": "old"}]), now=_NOW
    )
    assert len(out) == 1
...
def test_ninety_day_boundary_recent_release_qualifies():
    ...
    repo = _repo(
        [{"conda_name": "recent", "version": "2.0", "timestamp": avail_s * 1000}]
    )
    out = derive_release_velocity(
        pcv, repo, _map([{"pypi_name": "recent", "conda_name": "recent"}]), now=_NOW
    )
...
def test_rebuild_inside_window_does_not_shift_lag():
    ...
    pcv = _pcv(
        [{"pypi_name": "pkg", "version": "3.1", "upload_time_iso_8601": _iso(upload_s)}]
    )
    repo = _repo(
        [
            {
                "conda_name": "pkg",
                "version": "3.1",
                "timestamp": rebuild * 1000,
            },  # later row first
            {"conda_name": "pkg", "version": "3.1", "timestamp": first_avail * 1000},
        ]
    )
    out = derive_release_velocity(
        pcv, repo, _map([{"pypi_name": "pkg", "conda_name": "pkg"}]), now=_NOW
    )
    assert len(out) == 1
...
def test_unmatched_version_produces_no_row():
    ...
    upload_s = _NOW - _DAY
    pcv = _pcv(
        [{"pypi_name": "foo", "version": "2.0", "upload_time_iso_8601": _iso(upload_s)}]
    )
    repo = _repo(
        [{"conda_name": "foo", "version": "1.0", "timestamp": upload_s * 1000}]
    )
    out = derive_release_velocity(
        pcv, repo, _map([{"pypi_name": "foo", "conda_name": "foo"}]), now=_NOW
    )
...
def test_unmapped_pypi_name_produces_no_row():
    upload_s = _NOW - _DAY
    pcv = _pcv(
        [{"pypi_name": "foo", "version": "1.0", "upload_time_iso_8601": _iso(upload_s)}]
    )
    repo = _repo(
        [{"conda_name": "foo", "version": "1.0", "timestamp": upload_s * 1000}]
    )
...
def test_pypi_conda_name_differ_via_mapping():
    ...
    repo = _repo(
        [{"conda_name": "pillow", "version": "10.0", "timestamp": avail_s * 1000}]
    )
    out = derive_release_velocity(
        pcv, repo, _map([{"pypi_name": "Pillow", "conda_name": "pillow"}]), now=_NOW
    )
    assert len(out) == 1
...
def test_malformed_upload_time_yields_nan_lag_and_false_qualifies():
    avail_s = _NOW - _DAY
    pcv = _pcv(
        [{"pypi_name": "foo", "version": "1.0", "upload_time_iso_8601": "not-a-date"}]
    )
...
def test_malformed_conda_timestamp_yields_nan_lag_and_false_qualifies():
    ...
    assert math.isnan(out.iloc[0]["release_lag_hours"])
    assert (
        out.iloc[0]["release_lag_qualifies"] is False
        or out.iloc[0]["release_lag_qualifies"] == False
    )  # noqa: E712
...
def test_empty_frame_has_bool_and_float_dtypes():
    ...
    out = derive_release_velocity(
        pd.DataFrame(), pd.DataFrame(), pd.DataFrame(), now=_NOW
    )
    assert out.empty and list(out.columns) == _COLS

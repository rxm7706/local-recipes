...
def _fresh_universe(universe_pypi=()):
    ...
    return {
        "metadata": {
            "properties": [
                {"name": "cfe:atlas_built_at", "value": str(int(time.time()))}
            ]
        },
        "components": comps,
    }
...
def _core():
    ...
    return pd.DataFrame(
        [
            ...
            {
                "conda_name": "noversion",
                "latest_version": None,
                "upstream_version": None,
            },  # UNKNOWN (no cf version)
        ]
    )
...
def test_all_six_buckets_reproduced_on_a_fixture_inventory():
    ...
    components = [
        ...
        },  # UPDATE-FEEDSTOCK
        {
            "name": "rich",
            "version": "13.0.0",
            "purl": "pkg:conda/rich@13.0.0?channel=conda-forge",
        },  # UPDATE-PIN
        {
            "name": "noversion",
            "version": "1.0",
            "purl": "pkg:conda/noversion@1.0?channel=conda-forge",
        },  # UNKNOWN
        {
            "name": "flask",
            "version": None,
            "purl": "pkg:pypi/flask",
        },  # ADD (pypi in universe, unmatched)
        {
            "name": "left-pad",
            "version": "1.0.0",
    ...
    components = [
        {
            ...
            "version": None,
            "purl": "pkg:pypi/totally-unknown-pkg",
        }
    ]
    report = match_against_universe(
        _bom(components),
...
def test_g10_bare_match_guard_rejects_a_name_coincidence():
    ...
    mapping = pd.DataFrame(
        [{"pypi_name": "wasmtime-py", "conda_name": "wasmtime"}]
    )  # different pypi project
...
def test_matched_conda_row_carries_channel_qualifier_purl_and_version_comparison():
    ...
    report = match_against_universe(
        _bom(components), _core(), _mapping(), _fresh_universe(), _pypi_universe([]), {}
    )
    row = report["components"][0]
...
def test_stale_field_is_truthful_under_allow_stale():
    ...
    stale_bom = {
        "metadata": {
            "properties": [
                {"name": "cfe:atlas_built_at", "value": str(int(now - 30 * 86400))}
            ]
        },
        "components": [],
    }
    params = {
        "freshness": {"stale_after_days": 14},
        "sbom": {"now": now, "allow_stale": True},
    }
    report = match_against_universe(
        _bom([]), _core(), _mapping(), stale_bom, _pypi_universe([]), params
    )
    assert report["stale"] is True
    # a missing built_at bypassed by allow_stale is also reported stale
    nostamp = {"metadata": {}, "components": []}
    params["sbom"].pop("now")
    report2 = match_against_universe(
        _bom([]), _core(), _mapping(), nostamp, _pypi_universe([]), params
    )
...
def test_ad12_matcher_produces_an_input_not_a_compliance_report():
    ...
    report = match_against_universe(
        _bom([]), _core(), _mapping(), _fresh_universe(), _pypi_universe([]), {}
    )
...
def test_classify_bucket_verbatim_decision_tree():
    assert (
        classify_bucket({"ecosystem": "pypi", "pinned": None}, None, None, True)
        == "ADD"
    )
    assert (
        classify_bucket({"ecosystem": "pypi", "pinned": None}, None, None, False)
        == "UNKNOWN"
    )
    assert (
        classify_bucket({"ecosystem": "conda", "pinned": None}, None, None, False)
        == "UNKNOWN"
    )
    assert (
        classify_bucket({"ecosystem": "cargo", "pinned": None}, None, None, False)
        == "ADD-NONPYPI"
    )
    assert (
        classify_bucket({"pinned": "1.0"}, {"cf_latest": None}, None, False)
        == "UNKNOWN"
    )
    assert (
        classify_bucket({"pinned": "1.0"}, {"cf_latest": "1.0"}, "2.0", False)
        == "UPDATE-FEEDSTOCK"
    )
    assert (
        classify_bucket({"pinned": "1.0"}, {"cf_latest": "2.0"}, "2.0", False)
        == "UPDATE-PIN"
    )
    assert (
        classify_bucket({"pinned": "2.0"}, {"cf_latest": "2.0"}, "2.0", False)
        == "CURRENT"
    )

...
| [`mcp-host-real-station-tools.md`](mcp-host-real-station-tools.md) | dreamt | The mcp-host sidecar (`spec-mcp-era-isolation` slice 1) proxies every station's `/stations/<name>/mcp` call correctly but only ever answers with a generic `station_face()` identity stub — it never hosts any station's real MCP tools (marshal's `publish_loop_run` included), and no existing spec names closing that gap. Found 2026-09-12 chasing CAP-3's live-run proof: a real minted assertion still could not publish a run. |

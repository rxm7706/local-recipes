...
| [`pyforge-unifying-strategy.md`](pyforge-unifying-strategy.md) | realized | The Canopy and the 8-station hub-and-spoke estate — one Django host at `src/platform/` mounting every station's portal, MCP on that same ASGI process, a unified CLI and a Redis Streams event backbone. **Rescoped 2026-08-24** as an extension of [`python-agent-platform`](python-agent-platform.md). SPEC `shipped` 2026-08-26: Lane 1 `/` **200**, `/cms/` 302 to IdP, CAP-9 `platform_app` DML-only. Isolated `mfa` sqlmigrate stays fake. Q5 parked on [`build-league-scorecard.md`](build-league-scorecard.md). Query plane captured here — no HTAP CAP minted. No `services/` microservice tier. Optional: Story 12.9 CI when Actions minutes return. |

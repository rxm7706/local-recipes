...
| [`suite-scaffold-and-mybmad-sidecar.md`](suite-scaffold-and-mybmad-sidecar.md) | dreamt | 2026-09-06 skip rows flip: `bmad-module-template` is an authoring tool beside `bmad-builder` (never provisioned into `.claude/skills/`); `mybmad-dashboard` is an isolated opt-in sidecar, never `/console/` — own Postgres + auth must not replace Keycloak or the django-pyforge host. Steward Epic 52 / Story 52.1. |

...
| [`pyforge-unifying-strategy.md`](pyforge-unifying-strategy.md) | dreamt | The Canopy and the 8-station hub-and-spoke estate — one Django host at `src/platform/` mounting every station's portal, a FastAPI/MCP service tier, a unified CLI and a Redis Streams event fabric. **Rescoped 2026-08-24** to an extension of [`python-agent-platform`](python-agent-platform.md) after an audit found the host already shipped (steward epics 10/11/12/16 `done`); the chain covers only the residual — Wagtail Lane 1, `django-pyforge`, seven portals, `services/`, `pyforge.core.client`, RFC-1..5 and BS-1..8. |

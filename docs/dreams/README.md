...
| [`pyforge-unifying-strategy.md`](pyforge-unifying-strategy.md) | specified | Evergreen Foundry Dream (hub-and-spoke, eight stations). Read **How to read** + Grounding first. Tree, phases 0–6, `python-<role>-<class>` names, and fleet-convention table live **in this file**. Host work is **`pap:CAP-1`..`6`** (`extends` `spec-python-agent-platform`); Unifying **CAP-1..19** are a different set. CAP-1..18 closeout stands. **CAP-19** / Epic 34 is the live residual. Q5 parked on [`build-league-scorecard.md`](build-league-scorecard.md). |

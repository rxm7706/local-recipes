...
| [`pyforge-unifying-strategy.md`](pyforge-unifying-strategy.md) | specified | The Canopy and the 8-station hub-and-spoke estate — one Django host at `src/platform/` mounting every station's portal, MCP on that same ASGI process, a unified CLI and a Redis Streams event backbone. **Rescoped 2026-08-24** to an extension of [`python-agent-platform`](python-agent-platform.md); Spec `in-progress`, steward Epics 18–32 landed, 12-7 `/ht/` 200. Residual: RFC-5 leftover DDL; pip-layer fold is sibling [`platform-image-one-pixi-env.md`](platform-image-one-pixi-env.md). Q5 parked on [`build-league-scorecard.md`](build-league-scorecard.md). `lane1-serves-dw-h3` answered no. No `services/` microservice tier. |

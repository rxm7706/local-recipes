...
| [`cursor-native-tier-map.md`](cursor-native-tier-map.md) | specified | The campaign runs in Cursor. Re-point `model_tier_map` to explicit `{ harness = "cursor", model = "…" }` tables (easy composer-2.5, medium grok-4.6, heavy composer-2.5-fast) before any unattended drain. Fail-safe unchanged. Wave driven from this IDE. Story 33.15. |
...
| [`coverage-gate-independence.md`](coverage-gate-independence.md) | dreamt | **A check that can red a station's PR must not ship inside that station's package.** Found 2026-09-14 by an investigation aimed elsewhere: marshal's verdict lattice was *cleared* (it judges the story's work — `GATE_FAILED` is documented as "a project's own gate failed" against `ERROR`'s "an internal Marshal operation failed"), and `doctor/sources/marshal.py` already holds marshal's row with the estate's strongest independence. The violation was one directory up: `pyforge/marshal/coverage_gate.py` and `coverage_thresholds.toml` ship **inside** marshal while `coverage-gates.yml` runs that gate over all eight stations with no `continue-on-error`, so a three-line `[stations.marshal]` edit lowers marshal's own blocking floor with no Doctor verdict — Charter §6's prohibition verbatim, whose scope the operator ruled **broad** the same day. **Not a file move:** `pyforge-core` and `pyforge-testing-kit` are both marshal-governed, so relocating there moves the violation rather than ending it, and the module is named in an AD-3/AD-4 import-linter contract. Five open questions; `spec-coverage-gate-independence` seeded `draft`. |
| [`self-hosted-bmad-marketplace.md`](self-hosted-bmad-marketplace.md) | dreamt | The estate hosts its own BMAD module/skill catalog (air-gapped App Store), not Claude's public marketplace. Seeded 2026-09-14: fork/mirror `bmad-plugins-marketplace`, point installer + `extraKnownMarketplaces` at our clone, Builder/template/SKF as publish, trust review of our own. Live inventory the same day: no store on A, none at ownyourintelligence.ai (Layer 3 still thesis), none in openteams-ai; nebari-dev has skillsctl (Claude skills, consume), nebari-frames (Frames/SQLite — parked), catalog-pack/Harbor/nebi (OCI packs). Three SKUs — do not collapse. Seven open questions; `spec-self-hosted-bmad-marketplace` seeded `draft`. |
| [`work-passports-dated-extracts.md`](work-passports-dated-extracts.md) | dreamt | Internal agile team sees Jira + Internal GH + vendor-facing External GH + last extracts without logging into a forbidden board. Corridor is dated lists both ways (they extract/send; we send a signed slice they load) and a passport UUID in the existing Postgres app — not a fifth Kanban, not Epic 8 into their private GH. Sibling of [[jira-github-projects-sync]]. Seeded 2026-09-14 from `bmad-brainstorming` converge; `spec-work-passports-dated-extracts` `draft`. |
...
| [`vocabulary-one-name-one-job.md`](vocabulary-one-name-one-job.md) | dreamt | PyForge speaks three vocabularies at once — BMAD-METHOD's upstream terms, our own Lexicon and its derived artifact statuses, and the Intelligence Hub / Frame vocabulary adopted 2026-09-13 — plus a fourth taught only in the Design tier. First three-source comparison (2026-09-14) found the Lexicon's own rule (*every noun does exactly one job; every job has exactly one noun*) holding inside the seven nouns and nowhere else: four status vocabularies of which one is declared, `in-progress` meaning three different things, one job with four names (`done`/`shipped`/`realized`/`review`), and `shipped` — our most-used Spec status, 67 live — being upstream's canonical **illegal** value. Three corrections to our own records fell out: the Hub has **six** shared abstractions and the Charter names seven (Tier-0 amendment owed); the Guard categories are right but Source-Grounding ranks **second**, not sixth; and `extension-point` is a documented parked-Spec state, not a category error. **No adoption decided** — eleven operator questions carried; `spec-vocabulary-one-name-one-job` seeded `draft` and registered in `DEFERRED_SPECS`. |

...
> Live facts decay — re-verify before acting.

**langflow-suite** = ONE multi-output recipe, **4 outputs** (langflow-sdk + lfx + langflow-base + langflow), langflow **1.10.1**, python_min 3.11.
...
**🟡 FRONTEND FIX APPLIED — verifying (G73):** the langflow web UI (`langflow/frontend/`, ~1,874 built files: index.html + the Vite/React bundle) is generated at release time and otherwise exists **only in the PyPI wheel**; the GitHub tag carries `src/frontend/` React *source*, not the built bundle. A tag build that only `pip install`s ships **0 frontend files** — `import langflow` + `pip check` PASS but `langflow run` serves **no UI** (verified: built `.conda` = 0 `index.html`/`.js`/`.css`; the wheel = 1,874). **Fix (applied, in `langflow-base` output):** node-build the frontend from `src/frontend` **at build time** — `nodejs >=20.19` build dep, `npm ci && npm run build` (Vite → `src/frontend/build`), copy into `src/backend/base/langflow/frontend/` before `pip install` (hatchling `packages=["langflow"]` ships it). This is the conda-forge **idiom** for a wheel-only frontend (build jobs HAVE network; only tests are offline) — precedents: **gradio** (exact no-sdist analogue, `scripts/build_frontend.sh`), **chainlit** (pnpm via hatchling hook), **mlflow** (`yarn build` of `server/js`), **agentsview/nebi**. The earlier wheel-graft plan was dropped (no cf precedent). Guarded by an env-free **`package_contents`** test (`langflow/frontend/index.html` + `assets/*.js`).

**NEXT ACTION (to make langflow-suite submittable):**

1. ✅ **Frontend fix applied** (node-build, above).
2. **Verify** the built `langflow-base` `.conda` contains `langflow/frontend/index.html` + `assets/*.js` (the `package_contents` test enforces this in CI; confirm locally too — the green import/pip_check do NOT exercise the UI).
3. THEN submit `langflow-suite` as one multi-output staged-recipes PR — see the risk list below. ⚠️ A **competing** submission exists: pb01ka's **#33853 langflow-base (draft, ships no frontend)** + **#33875 langflow-sdk (open)**; coordinate or note the overlap.
...
| Field            | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Status           | **IN PROGRESS — see the ## CURRENT STATE block (top) for the authoritative state.** Headline: langflow-suite (4-output, v1.10.1) builds+tests GREEN locally against PURE conda-forge (langchain/langflow-sdk/litellm gates ALL cleared, G68) **frontend fix APPLIED** — the GitHub-tag build shipped no prebuilt frontend (G51/G73; `langflow/frontend/` is wheel-only), now node-built at build time in langflow-base (`nodejs`+`npm`; gradio/chainlit/mlflow precedent), verified by a `package_contents` test (index.html + `assets/*.js`). Verify the built `.conda`, then submit. Closure PRs: ~13 merged / ~38 open-green / ~10 draft-red / 3 closed (per-recipe = Appendix). |
| Owner            | rxm7706                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| 3  | langflow-sdk                              | folded into langflow-suite (#33856 CLOSED)        | noarch        | MIT                        | —                                                                                                                                     |
...
> **⚠️ staged-recipes vs feedstock (VERIFIED on PR #33972, build `de2f9814`):** `noarch_platforms` controls the **feedstock** CI matrix **post-merge** — it does **NOT** change the **staged-recipes PR** matrix. staged-recipes runs **3 fixed Azure jobs (linux/osx/win)** and builds the recipe on each, so the **win job still runs + fails** in the PR (the PR build had a `win_64` leg and **no** `osx_arm64` leg, despite the conda-forge.yml). Especially here, `noarch` is **per-output** (no top-level `noarch`), so the matrix generator doesn't treat the recipe as noarch. → To go green on win **in the PR**, `noarch_platforms` is not enough: land the magika-feedstock fix, or add `build: skip: true  # [win]` (verify lint-clean). The conda-forge.yml is still correct + kept — it configures the **feedstock** (drops win, adds `osx_arm64` validation) post-merge.
>
> **✅ Artifacts ARE downloadable from the PR.** `workflow_settings.store_build_artifacts` **does** work in the staged-recipes PR (verified: build `de2f9814` published `conda_pkgs_linux` / **`conda_pkgs_noarch`** / `conda_pkgs_osx`, even with the win leg red). langflow-suite is `noarch:python`, so the **`conda_pkgs_noarch`** artifact is the **universal** package — one `.conda` (built on `linux_64`) that installs + runs on **macos-arm64** and every platform (no per-arch build). The red win **build** leg does NOT block artifact publishing on the other legs. Fetch via the CFE `download_pr_artifacts` tool or the Azure build's Artifacts tab.

**Why Windows fails — three distinct win-only failures surfaced on PR #33972 (first two fixed; the third is the blocker):**

1. **Bash-only frontend build → ✅ FIXED (G73, skill v8.52.1).** The `langflow-base` node-build script was unix/bash-only (`set -ex`/`pushd`/`rm -rf`/`cp -r`/bare `npm`). staged-recipes builds noarch on **all** platforms and runs `package_contents` on each; the win leg runs the script through **cmd.exe** → bash-isms fail + a bare `npm` `.cmd` shim without `call` terminates the script → empty frontend → `package_contents` guard fails. Fixed with a cross-platform build (`if: win` → `call npm --prefix … && if errorlevel 1 exit 1`; copy + `pip install` via Python `shutil`).
2. **gunicorn Unix-only hard dep → ✅ FIXED (G76, skill v8.53.0).** `gunicorn` is Unix-only on conda-forge (`linux-*`/`osx-*` subdirs only — no `win-64`, no `noarch`). As a hard run dep of the noarch `langflow-base`, the win test env couldn't **solve** (`nothing provides gunicorn`). noarch bakes `depends` at build time, so `if: unix` can't make a hard dep platform-conditional. Fixed by making gunicorn **optional**: stripped from the wheel `[project.dependencies]` (patch 0001) + moved to conda `run_constraints` (advisory).
3. **magika → onnxruntime transitive conda-vs-wheel win skew → 🔴 EXTERNAL, NOT FIXED (the reason win is excluded).** `lfx → markitdown → magika 0.6.3`, whose **wheel** metadata caps `onnxruntime<=1.20.1; sys_platform=="win32"`. But the conda **magika-feedstock does not encode that win-only cap** (conda deps can't carry environment markers), so the solver installs the latest `onnxruntime 1.26.0` on win → `pip check` (which staged-recipes runs **regardless of the recipe's `pip_check:` setting**, G76) fails:

   ```
   magika 0.6.3 has requirement onnxruntime<=1.20.1; sys_platform == "win32", but you have onnxruntime 1.26.0.
   ```

   This is an **upstream magika-feedstock metadata gap**, NOT a langflow-suite bug — and it can't be cleanly worked around in a noarch recipe (a win-only onnxruntime cap is impossible; an unconditional cap penalizes linux/osx).

**To RE-ENABLE Windows (future task — out of scope for the initial unix submission):**

1. **Land a `magika-feedstock` PR** adding the win onnxruntime cap (`onnxruntime <=1.20.1  # [win]` — the conda equivalent of magika's wheel marker) so the solver picks a win-compatible onnxruntime; then rebuild-to-channel (G66).
2. **Re-check for further win moles** — langflow's dep tree is deeply unix-oriented; expect more transitive win skews after this one clears (triage each like #2/#3: is it a Unix-only conda dep → `run_constraints`, or a conda-vs-wheel marker gap → upstream-feedstock PR).
3. **Add `win_64` back to `noarch_platforms`** in `recipes/langflow-suite/conda-forge.yml` and confirm the win leg goes green.
4. Until then, Windows users can still `conda install` the noarch package (it installs); only the Unix-only `gunicorn` server is absent — langflow falls back to `uvicorn` on Windows.
...
- CFE-skill retro (Rule 2) folding in whatever the feedstock-convergence + submission work
  ...
  has since driven the skill to **v8.51.0** (through G73 — notably G54/G55, the wheel→GitHub-source
...
- **D. SPECIAL (updated 2026-06-27 — langflow-suite now SUBMITTABLE):** `langflow-suite` is **no longer gated** —
  **langflow-sdk was FOLDED INTO the suite as a 4th output** (`src/sdk`; lfx pins it via `pin_subpackage`, so the
  ...
  (and #33856 CLOSED, superseded). Still gated (5): `lfx-arxiv`/`lfx-docling`/`lfx-duckduckgo`/`lfx-ibm` (cannot solve
...
**NO workaround langchain/litellm needed (updated 2026-06-27).** The suite now resolves the whole
langchain stack from **pure conda-forge**: cf `langchain 1.3.11 pyhcf101f3_1` carries the fixed
`aiosqlite >=0.19.0,<0.23` (the #276 rebuild landed on-channel) and `litellm 1.89.4` is de-flattened.
**Purge any stale local `langchain` / `litellm` builds** from `build_artifacts/<subdir>/` + re-index
(`python -m conda_index …`) **before building** — a leftover local copy shadows cf under strict channel
priority and gives a false result (G68). If the suite then fails to solve on langchain/aiosqlite, that's
the live signal the cf channel regressed — re-check it (do NOT re-introduce a workaround).
...
## Worked Example (HISTORICAL LOG — superseded by § CURRENT STATE; dated entries below are a record, not current status)
...
gated until that rebuild uploads **AND** `langflow-sdk 0.2.1` lands (#33856 CLOSED — langflow-sdk folded into the suite; not on the
...
| 3  | langflow-sdk                                                       | B1      | [recipe.yaml](https://github.com/rxm7706/local-recipes/blob/main/recipes/langflow-sdk/recipe.yaml)            | FOLDED into langflow-suite (4th output, pin_subpackage) — superseded                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | [#33856](https://github.com/conda-forge/staged-recipes/pull/33856) CLOSED |
| 4  | jigsawstack                                                        | B1      | [recipe.yaml](https://github.com/rxm7706/local-recipes/blob/main/recipes/jigsawstack/recipe.yaml)             | submitted — open draft, CI-green (in review)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | [#33857](https://github.com/conda-forge/staged-recipes/pull/33857) DRAFT  |

...
**Status (2026-06-27):** `recipes/langflow-suite/conda-forge.yml` sets `noarch_platforms: [linux_64, osx_64, osx_arm64]` — **`win_64` is intentionally excluded.** linux + osx (incl. **osx-arm64**) build + test GREEN; Windows is deferred. langflow-suite is `noarch:python`, so the single noarch `.conda` (built on `linux_64`) is the universal package — it still *installs* on Windows; win is dropped only from the **CI test matrix** because of a transitive upstream skew (failure #3 below). `workflow_settings.store_build_artifacts` publishes the built `.conda` for download.

...
>
> ```
> run quick-dev — implement the intent in docs/specs/kedro-cf-atlas-migration.md
> ```
...
| Field | Value |
|---|---|
| Status | **Draft v1 — ready for `bmad-quick-dev` intake.** 5 open questions (§ 11), none v1-blocking. |
| Owner | rxm7706 |
| Track | BMAD Quick Flow (tech-spec only, no separate PRD/architecture phase) |
...
### 5.8 Data Quality Guardrails (`kedro-great-expectations`)
...
The seven implementation waves (A–G) decompose into the stories below. Each wave depends on the prior wave's deliverables. Stories within a wave may proceed in parallel where noted.
...
#### Story F2 — Inject `kedro-great-expectations` validation nodes

**Goal**: Wire Great Expectations data contracts into Kedro nodes for upstream API ingestions; halt on violation.

**Acceptance criteria**:
- A contract violation (e.g., PyPI JSON missing a version field) halts the node and raises an A2A alert before persistence.
- Maps to FR-10.
...
**Default**: treat § 4.12 / § 7 (Wagtail CMS + Karpathy Wiki) as a follow-on; this migration delivers Waves A–G (Kedro core through WASM), with the CMS layer as forward-looking context.
...
| Wagtail CMS / LA SUITE DOCS / Karpathy Wiki knowledge-base build-out | Forward-looking factory layer (§ 4.12, § 7); follow-on effort pending Q5. |
...
- Kedro + `kedro-viz` + `kedro-dagster` + `kedro-mcp` + `kedro-great-expectations` plugins (all conda-forge-sourced per FR-15).
...
```
@bmad-quick-dev — implement the intent in docs/specs/kedro-cf-atlas-migration.md.

Wave A first (A1 nebi scaffold → A2 catalog → A3 IncrementalParquetDataset).
...
Proceed wave by wave (C orchestration+viz, D semantic layer+dashboards,
E A2A+observability, F DuckDB singularity, G WASM+sensors). Resolve Q2/Q3/Q4
at the start of their gating wave; default to the recommendations in § 11.
Treat § 7 / § 4.12 (Wagtail CMS) as follow-on per Q5 unless the operator
expands scope.

Per CLAUDE.md Rule 1, invoke the conda-forge-expert skill for any work that
touches recipe code or atlas tooling. Per Rule 2, close with a CFE-skill retro
+ CHANGELOG entry.
```

...
sources:
  - https://github.com/bmad-code-org/BMAD-METHOD/blob/main/docs/_STYLE_GUIDE.md
  - .claude/skills/bmad-os-diataxis/SKILL.md
verified: 2026-09-20
---

:::note[Provenance]
Vendored 2026-09-20 from BMAD-METHOD's `docs/_STYLE_GUIDE.md` (MIT License, © 2025 BMad Code, LLC) so this repo's `bmad-os-diataxis` skill reads the same rules the upstream skills were written against. Kept as close to upstream as possible on purpose: the only edits replace "BMAD" with "PyForge" where the sentence is about the reader's product, and paths that assume upstream's `docs-site/` layout stay as written until this repo's docs site matches it (herald Dream, 2026-09-20). Re-vendor rather than fork.
:::
...
- Use specialized terms only when readers need them to use PyForge. Define an unfamiliar term at first use.

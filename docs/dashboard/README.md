...
not authored documentation (see `docs/MAP.md` § Outside this map). The
convention is **one subfolder per published dashboard board**. Today the
only board publishing here is atlas Kedro-Viz, under `kedro-viz/`; that name
stays as-is (it is not renamed to match any other product). Rebuild with

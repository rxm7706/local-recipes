...
- Five-tier is 03-only (brief; PRD glossary + canopy:FR-37/38/39 + SM-5).
- Registration carries owner / `work_class` / promotion date; SLA not an AppConfig field (canopy:FR-1).
- No 01/02 station tiles (canopy:FR-2).
- Hooks/plugins glossary + canopy:AD-21 pointer (PRD §3).
- Path B ≠ Tachyon; Lane 2 HTMX implied by stack/SPEC; Q4 identity on canopy:FR-17 area.
...
- **high** OQ-4 zombie — Brief: URL scheme closed (canopy:FR-9a). PRD §11 answers it; PRD §10 and §14 still speak as if it is open.

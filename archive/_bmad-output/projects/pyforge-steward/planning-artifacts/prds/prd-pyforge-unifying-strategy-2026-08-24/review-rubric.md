...
FRs 1–42 carry testable consequences. canopy:FR-37/38/39 are 03-scoped. canopy:FR-12 names `start`/`get`. Vague “gracefully” language is not the pattern here.

### Findings
- **low** UJs do not mention `work_class` or Golden Path (§2.3) — Journeys still describe the 03 estate only, which is correct enough; OM rules live in glossary + canopy:FR-2. *Fix:* optional one clause on UJ-1 that 01/02 work is not a switcher tile.
...
- Glossary: `work_class`, Path B/Tachyon, hooks/plugins present and used in canopy:FR-1/2/17/37–39.
- FR IDs include canopy:FR-9a/9b/21a; contiguous enough; no duplicate FR numbers spotted.

...
`DETECTOR = {...}` marker). Renamed from `spec_surface_check.py` in
review pass 2 for the same reason: `scripts/detectors.py::discover()`
globs `scripts/*_check.py` and hard-fails on ANY match with no valid
...
invokes the dispatcher below instead, which does not understand this flag):
        python scripts/spec_surface_baseline.py --write-baseline [--spec NAME ...]

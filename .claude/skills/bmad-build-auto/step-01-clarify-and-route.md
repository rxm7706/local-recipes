...
- `done` → read `{spec_file}` frontmatter `followup_review_recommended` (missing means `false`).
  - `false` → **HALT** with status `done` and blocking condition `done spec — follow-up not recommended`. Do not reset `review_loop_iteration`. Do not edit the spec. Do not commit. This is not a resumption and not a fresh review (CAP-11 / steward 41.2, mason 13.2).
  - `true` → write `followup_review_recommended: false` now (this invocation is the single allowed follow-up), then **EARLY EXIT** to `[[bmad-snapshot:step-04-review.md]]`. Step-04 must leave the flag `false` at HALT even if its patch-score would have set `true`.
...
- **If exactly one matches**, set `spec_file` to that path.
  ...
  - Any other recognized `status`: **EARLY EXIT** using the same routing as above, including the `done` + `followup_review_recommended` HALT-or-one-follow-up rule (never an unconditional `review_loop_iteration` reset). One difference: a `blocked` story HALTs with blocking condition `story already blocked`, not `blocked spec supplied` — the caller did not supply this file; build-auto found it by id.

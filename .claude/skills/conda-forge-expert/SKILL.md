...
# tick every "- [ ]" except the "knowledge base" line, then:
...
**Case study**: qianfan #33935 (Jun 26, 2026) — first opened with a custom `--body-file` description that dropped the checklist; fixed by re-fetching the live template and ticking the boxes.
...
**Fix**: request review **only after every check is green**, **exactly once**, with the team matching the recipe's language. **Before pinging, check the PR's labels** — if `review-requested` (and the language label) are already present, a prior ping already took effect → **skip**:
```bash
gh pr view <pr> --repo conda-forge/staged-recipes --json labels --jq '[.labels[].name]'
# only ping if "review-requested" is absent AND all checks are SUCCESS:

...
4.  **Security Scan**: Use `scan_for_vulnerabilities` to check all dependencies against the local CVE database. Report and resolve any findings.
...
| `check_dependencies` | Verifies that all dependencies exist on conda-forge. |
| `optimize_recipe` | Lints for quality and best practices (e.g., misplaced dependencies). |

...
**Fix (G51 prebuilt-asset path)**: add the package's PyPI **wheel as a secondary `source`** and **force-include** its `<pkg>/frontend/` into the output. Building the frontend from `src/frontend` is NOT the idiomatic path — it needs `npm ci` to fetch the `node_modules` tree from the registry at build time, which conda-forge builds are not meant to do ([G45](#g45-a-browser-spa-vuereactsvelte--vite-is-usually-not-conda-forge-submittable--run-the-viability-gate-first-then-build-local-only-as-a-static-site--launcher) / G6 network note); the wheel's prebuilt assets sidestep it. (`npm run build` itself isn't "forbidden" — the build-time *network fetch* is. Building from source would require vendoring `node_modules` offline, a heavy/uncommon setup.)

**Case study**: `langflow-suite` (langflow-base + langflow built from the `langflow-ai/langflow` v1.10.1 tag), surfaced Jun 27 2026 during an adversarial spec review: the built `langflow-base`/`langflow` `.conda` shipped **0** frontend files while the `langflow-base` PyPI wheel ships **~1,874** (`langflow/frontend/index.html` + the Vite/React assets bundle). `import langflow`/`pip_check` were green throughout — the UI-less break was invisible to them. Recorded as the langflow-suite submission blocker (fix pending: force-include the wheel's `langflow/frontend/`).

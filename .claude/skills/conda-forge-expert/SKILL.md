...
**Upper-bound caps follow the same rule (v8.86.0).** G26 is not limited to `==` pins — any load-bearing cap in upstream's `pyproject.toml` that you mirror or loosen in `run:` must be patched in the wheel METADATA too. **Case study**: `dbgpt-client` (Mason 13.2, Sep 2, 2026). Upstream pins `sqlalchemy>=2.0.25,<2.0.29`; conda-forge has no `cp314` sqlalchemy below 2.0.29, blocking `dbgpt-app` on Python 3.14. Loosening the recipe run-dep to `>=2.0.25,<2.1` without patching `packages/dbgpt-client/pyproject.toml` would fail `pip check` the same way as an unpatched `==`. Fix: source patch `0003-loosen-dbgpt-client-sqlalchemy-cap.patch` at recipe `source.patches` time, before any output builds.

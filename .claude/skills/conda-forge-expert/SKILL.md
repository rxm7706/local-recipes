...
- **v8.90.3** (Sep 10, 2026) — **Mason Story 16.1 Rule-2 retro: no skill changes; verified existing guidance held for mason's own `pyforge-mason` env satisfying `cfe.py`'s `CFE_IMPORT_FLOOR` (PATCH; docs/config only).** `[feature.pyforge-mason.dependencies]` was missing `truststore` and `conda-forge-metadata`, two of `cfe.py`'s six-entry `CFE_IMPORT_FLOOR`, so `mason doctor` reported `cfe_import_floor_satisfied: False` and `unavailable_verbs: ('recipe',)` even though `CFE_IMPORT_FLOOR` itself (`cfe.py:180-187`) was exactly correct throughout — a consumer-side gap in mason's own declaration, not a skill defect. Fixed entirely in `pyforge-mason`'s own Story 16.1 spec and code map; no correction, refinement, or addition to land here. **Files**: `SKILL.md` (frontmatter version, history), `config/skill-config.yaml` (8.90.2 → 8.90.3), `MANIFEST.yaml`, `CHANGELOG.md`.

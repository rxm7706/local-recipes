...
**G26 extension — a loosened upper-bound CAP needs the same source patch as a loosened `==` pin (Sep 10, 2026).** The METADATA rule is not limited to an exact `==` pin: when the recipe loosens an upstream `<upper-bound>` **cap**, the wheel's `dist-info/METADATA` still carries the OLD ceiling unless the source `pyproject.toml` is patched to match, and `pip_check` fails against that stale ceiling exactly as it would against an unpatched `==`. **Case study**: DB-GPT `dbgpt-client` story 13.2 — the feedstock capped `SQLAlchemy>=2.0.25, <2.0.29` (`2.0.29` has no `cp314` build, while the platform's other DB-GPT outputs already run sqlalchemy 2.0.52); loosening the recipe run-dep alone to `>=2.0.25,<2.1` was insufficient — `pip_check` kept reading the wheel's unpatched `Requires-Dist: SQLAlchemy>=2.0.25, <2.0.29` until `patches/0003-loosen-dbgpt-client-sqlalchemy-cap.patch` rewrote the identical line in `packages/dbgpt-client/pyproject.toml`. Fix mirrors the `==` case exactly: patch the source cap to the same range as the loosened run-dep, not just the recipe.
...
- **v8.91.0** (Sep 10, 2026) — **Mason Story 16.3 retro: the CLI<->MCP-tool surface is now a gated parity number, not review (MINOR).** `pyforge.marshal.mcp.parity`'s FR-155 gate (`assert_cli_tool_parity`/`parity_findings`, explicit `CLI_ONLY_VERBS`/`TOOL_ONLY_NAMES` allowlists) was marshal-only — the CFE server's 46 `@mcp.tool()` registrations in `conda_forge_server.py` had no parity gate at all, so a pixi task's CLI wrapper could gain or lose a tool counterpart (or vice versa) and nothing reds. **Extends marshal's primitive rather than re-implementing it** (a second independent copy of a gate is exactly how the `_http.py` credential leak survived a "durable fix"): new `.claude/tools/mcp_cli_parity.py` imports `pyforge.marshal.mcp.parity` unmodified via a `sys.path` insert and calls its generic `assert_cli_tool_parity`/`parity_findings` with CFE-derived arguments. **CLI-surface derivation**: `tomllib`-parses `pixi.toml` and scans every `[feature.*.tasks.*]` table (not `local-recipes` alone — `vuln-db` also invokes `.claude/scripts/conda-forge-expert/` wrappers, e.g. `scan-project` -> `scan_project.py`, the identical script the `scan_project` MCP tool references), keying the CLI verb on the **pixi task name** rather than the wrapper file's stem — a stem-anchored derivation (this story's own first-draft mistake, caught in review and reverted) cannot detect a task being renamed, added, or removed independent of its file. **Tool-surface derivation**: `ast`-parses (never imports) `conda_forge_server.py` for every `@mcp.tool()` registration, resolving each to a CLI verb via which `SCRIPTS_DIR / "<script>.py"` constant its body references. **35 CLI-only pixi tasks** and **9 tool-only MCP tools** (`generate_recipe_from_pypi`, `trigger_build`, `get_build_summary`, `lookup_feedstock`, `enrich_from_feedstock`, `get_feedstock_context`, `edit_recipe`, `migrate_to_v1`, `query_atlas`) are each declared with a one-line reason — never a silent skip; a missing or stale entry is itself a gate finding. **One case static script-reference analysis cannot disambiguate on its own**: `prepare_submission_branch` and `submit_pr` both invoke the identical canonical `submit_pr.py` script (only their argv differs — `--prepare-only`) — a `_VERB_OVERRIDE` hand-maps `prepare_submission_branch` to the separate `prepare-pr` pixi task explicitly. New `tests/meta/test_mcp_cli_tool_parity.py` proves the live inventory passes clean and that a fixture-injected mismatch fails the gate in each direction (an on-surface CLI verb with no tool; a tool with no CLI verb and no allowlist entry; a stale allowlist entry claimed by a real verb/tool). **Files**: `SKILL.md` (this entry, frontmatter version, history), `reference/mcp-tools.md`, `tests/meta/{test_mcp_cli_tool_parity.py (new), test_skill_md_consistency.py}`, `config/skill-config.yaml` (8.90.2 → 8.91.0), `MANIFEST.yaml`, `CHANGELOG.md`; `.claude/tools/mcp_cli_parity.py` (new) lands in a companion `feat(mason):` commit — a `.claude/tools/` library module (like `conda_forge_server.py` itself) is outside the three-place rule's `.claude/skills/conda-forge-expert/scripts/` scope, so it needs no CLI wrapper, pixi task, or `SCRIPTS`-list entry.
- **v8.90.2** (Sep 10, 2026) — **G26 gains an upper-bound-cap case study, distinct from Story 13.1's marker-split extension (PATCH).** Closes mason Story 15.2's Rule-2 retro for Story 13.2 (deferred since v8.86.0, tracked `DW-13-2-1`, unwritten across seven releases because mason's meta-test forbids an in-story CFE edit outside a sanctioned `retro:`/`retro(<scope>):` commit that also moves this CHANGELOG in the same commit — this commit is that). G26's wheel-METADATA rule is not limited to an exact `==` pin: a loosened upper-bound *cap* (`<X`) hits the same failure, since the wheel METADATA still carries the old ceiling until the source is patched to match. **Case study**: `dbgpt-client`'s `SQLAlchemy>=2.0.25, <2.0.29` cap (no `cp314` build) — loosening the recipe run-dep to `<2.1` alone was insufficient; `patches/0003-loosen-dbgpt-client-sqlalchemy-cap.patch` had to rewrite the identical line in `packages/dbgpt-client/pyproject.toml`. No script changed (docs + derived config only); both compiled slices' `brief_mirrored_through` advanced in a companion commit to keep `cfe-rebuild-guard-check` clause (b) clean. **Files**: `SKILL.md` (G26 case study, frontmatter version, history), `config/skill-config.yaml` (8.90.1 → 8.90.2), `MANIFEST.yaml`, `CHANGELOG.md`.

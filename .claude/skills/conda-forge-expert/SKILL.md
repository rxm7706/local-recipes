...
- **v8.91.0** (Sep 10, 2026) — **The CLI<->MCP-tool surface is now a gated parity number, not review (MINOR).** Marshal's `pyforge.marshal.mcp.parity` gate engine (`assert_cli_tool_parity`/`parity_findings`) was FR-155 marshal-only — the CFE server's 46 `@mcp.tool()` registrations had no equivalent, so a pixi task could gain or lose a tool counterpart and nothing reported it. New `.claude/tools/mcp_cli_parity.py` extends that same engine (never a forked second copy — a second independent copy of a gate is how the `_http.py` credential leak survived a "durable fix") by DERIVING both surfaces: the CLI verb set from every `[feature.*.tasks.*]` table across the full pixi task registry (not `local-recipes` alone — `vuln-db` invokes shared wrapper scripts too, e.g. `scan-project`), anchored at the pixi **task name** rather than the wrapper file's stem so a renamed/added/removed task can't drift silently; the tool set by `ast`-parsing `conda_forge_server.py` (never importing it) for which `SCRIPTS_DIR / "<script>.py"` constant each `@mcp.tool()` body references. 35 CLI-only pixi tasks (legacy/offline/maintenance scripts with no MCP read-tool) and 9 tool-only MCP tools (`generate_recipe_from_pypi`, `trigger_build`, `get_build_summary`, `lookup_feedstock`, `enrich_from_feedstock`, `get_feedstock_context`, `edit_recipe`, `migrate_to_v1`, `query_atlas`) are each declared with a one-line reason, never a silent skip. One case static analysis cannot disambiguate on its own: `prepare_submission_branch` and `submit_pr` both invoke the identical canonical `submit_pr.py` script, distinguished only by a `--prepare-only` flag — a `_VERB_OVERRIDE` hand-maps the former to the `prepare-pr` pixi task. New `tests/meta/test_mcp_cli_tool_parity.py` proves the live inventory passes clean AND that a fixture-injected mismatch in each direction (an on-surface CLI verb with no tool; a tool with no CLI verb and no allowlist entry) actually fails the gate. **Files**: `SKILL.md` (this entry, frontmatter version), `reference/mcp-tools.md`, `tests/meta/{test_mcp_cli_tool_parity.py (new), test_skill_md_consistency.py}`, `config/skill-config.yaml` (8.90.2 → 8.91.0), `MANIFEST.yaml`, `CHANGELOG.md`; `.claude/tools/mcp_cli_parity.py` (new) lands in a companion `feat(mason):` commit — it is a `.claude/tools/` library module, not a `.claude/skills/conda-forge-expert/scripts/` CI script, so the three-place rule does not apply to it (same precedent as `conda_forge_server.py`).

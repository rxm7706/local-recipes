...
| Channel composition by source ecosystem (Python vs. R vs. Perl vs. system libraries, etc.) | Aggregate `conda_name` naming-convention prefixes (`r-*`, `perl-*`, compiler/toolchain names) — **not** the existing cross-ecosystem columns | Phase B | Per-ecosystem package/feedstock counts and share | 🆕 recommended (2026-07-16). Live-validated: R/CRAN alone is 11.6% of the *entire* 32,765-package channel — the real second-largest ecosystem, not a rounding error. The atlas's existing `npm_name`/`cran_name`/`cpan_name`/`luarocks_name`/`maven_coord` columns were checked live and found almost entirely unpopulated at scale (136 of 11,602 non-Python packages matched `npm_name`; 0 for CRAN/CPAN) — a composition report needs the naming-convention approach, not those columns. |
...
| Conda-native vulnerability match (catches non-PyPI transitive deps Phase G structurally can't see) | Batch-query `api.basilisk.prefix.dev` `POST /v1/querybatch` (conda-forge PURL, CEP 63 form; 1,000/request cap) | New: `basilisk_vulns` table (candidate next phase after Phase T) | Per-package confirmed-advisory count, independent identity axis from Phase G's PyPI-keyed vdb match | 🆕 recommended (2026-07-16 — see `docs/specs/cfe-atlas-datapipeline-kedro-migration.md` § 15, FR-19). Live-validated: a full 21,163-package batch run found confirmed matches on `libuuid` (203M downloads), `libtiff`, `libarchive` — non-Python C libraries a PyPI-identity-only pipeline cannot match at all. |
| Is a matched CVE actually fixable right now, or genuinely blocked? | Cross-reference `basilisk_vulns` advisory `affected[].ranges[].events[].fixed` (name-matched, not ecosystem-tag-matched — the raw OSV data keeps its *original* ecosystem, typically `PyPI`) against the package's current installed version | Basilisk `GET /v1/vulns/{id}` + `packaging.version` compare | `fix_available: true / false / unknown` derived column | 🆕 recommended (2026-07-16 — FR-19). Live-validated on all 765 unique advisories surfaced by the full-population run: **85.3% of the 5,101 (package, advisory) matches already have a fix version that exists** — a scheduling problem, not an open-research problem, for the majority of matches. ~48% of advisories carry no structured fix data at all — must map to `unknown`, never collapse to `false`. |
...
| How long does conda-forge typically take to catch up after upstream releases? (time, not just behind/current) | Retain `upload_time_iso_8601` from Phase H's existing per-release PyPI JSON fetch (currently discarded after extracting `info.version`); diff against `latest_conda_upload` | Phase H extension (no new external source) | Release-to-availability lag distribution (hours/days); requires the ≤90-day-recency filter below | 🆕 recommended (2026-07-16 — see `docs/specs/cfe-atlas-datapipeline-kedro-migration.md` § 15, FR-20). Live-validated at full population (19,726 feedstocks): median **8.9 hours**, 72.4% within 24h, 83.7% within 72h. **Gotcha, found the hard way and must be encoded in the implementation:** a naive `latest_conda_upload − pypi_upload_time` delta is invalid without filtering to upstream releases ≤90 days old — conda-forge rebuilds long-stable versions for unrelated reasons (migrations, ABI bumps), and the raw metric showed a false 47%-behind headline before this filter was applied; 83.7% of that false-positive bucket had a PyPI release itself over a year old. Validated identically at two independent sample scales (5,000-package and 19,726-feedstock; both within 1 point of each other), so the threshold is not a sampling artifact. |
...
**🆕 recommended (2026-07-16, not yet counted above):** 4 new catalog rows added following a live multi-stage ecosystem analysis — Basilisk conda-native vulnerability matching + its derived fix-availability signal (Security & Compliance), the release-to-availability velocity signal + its required 90-day-recency filter (Environment Auditing), and channel composition by naming convention (Channel Health). Full detail, evidence, and candidate FR-19/FR-20 in `docs/specs/cfe-atlas-datapipeline-kedro-migration.md` § 15. These are proposals, not shipped or gated work — do not fold into the ✅/📋/❌ tallies until accepted at BMAD intake.

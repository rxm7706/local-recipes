...
**v8.11.4** (Jun 7, 2026, current) — G10 broadened to a four-form name-divergence check after a third confirmed PyPI→conda divergence pattern surfaced (PyPI `langfuse` → conda `langfuse-python`). **PATCH bump**, standalone: G10 rewrite + auto-memory + CHANGELOG entry; no code change. The G10 lookup checklist now covers bare PyPI name, hyphen↔underscore swap, `-py` suffix, AND `-python` suffix.

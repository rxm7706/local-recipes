...
**v8.11.1** (Jun 2, 2026, current) — Legacy-path cleanup + `recipe_editor.py` line-fold fix. Caps the npm-generator correction arc that ran v8.10.0 → v8.10.1 → v8.11.0 → v8.11.1 over 5 days (May 26 → Jun 2). **PATCH bump** — internal cleanup; default-path behaviour unchanged.
...
Per-release detail below. Auto-memory `canonical_npm_recipe_pattern.md` is the live reference for the v8.11.0+ npm recipe shape.

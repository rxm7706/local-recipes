...
**v8.11.3** (Jun 7, 2026, current) — Recipe-authoring gotchas G11 + G12 from the staged-recipes#33534 xorq CI-fix retro, plus a step 8b refinement on `prepare_submission_branch` vs. direct-git for follow-up commits. **PATCH bump**, standalone: two new SKILL.md gotcha sections + one step refinement + CHANGELOG entry; no code change. The v8.10 → v8.11.1 npm-generator correction arc summarized below is unchanged.
...
Per-release detail below. Auto-memory `canonical_npm_recipe_pattern.md` is the live reference for the v8.11.0+ npm recipe shape; auto-memory `feedback_pypi_conda_mapping_unreliable.md` is the live reference for the multi-form name-divergence check codified in v8.11.2's G10.

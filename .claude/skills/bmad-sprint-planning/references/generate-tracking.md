...
3. Read the JSON report and act on it — this is where judgment re-enters:
   ...
   - `dropped_orphans` are entries that existed in the old status file but whose story number (`N.M`) is gone from the epics — a deleted story, not a retitle. Each carries its old status; reconcile with the user, then transplant by rerunning with `--set <new-key>=<old-status>` only when the story number itself changed.
   - `kept_keys` are existing `N.M` rows whose current heading would mint a different slug (truncation, punctuation, a title edit). The old key and its status are kept; this is not drift.

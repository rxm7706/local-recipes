...
| 2 | `dashboard-project-path-derivation` | Marshal | 4 capabilities, single-file surface (`pyforge.doctor.sources.fleet_scan`) | **Ready** — a TODO already sits in the code (`generate.py:42`) naming this exact gap; edge cases (regen/genesis/CI) already verified against the live code | Small, contained story against `pyforge.doctor.sources.fleet_scan`; doesn't need a new epic, could be a maintenance PR |

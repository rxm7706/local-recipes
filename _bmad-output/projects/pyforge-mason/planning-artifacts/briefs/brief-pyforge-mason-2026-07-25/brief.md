...
**Status: BLOCKED.** This satellite brief was finalized 2026-07-25 and describes a genuinely
different product from `mason` itself: an air-gapped, conda-forge-native repackaging of the
third-party Presenton AI deck-generation app for Red Hat OpenShift, registered to the same station
(Mason repackages it) but sharing no architecture, code, or timeline with the `mason` CLI above. Its
owning Dream (`docs/dreams/presenton-pixi-image.md`) carries `status: archived`,
`archived-reason: blocked`, `blocked-on: Phase-0 decision gate (Epic 1)` — no story has entered
implementation, and the PRD's six Phase-0 exit criteria (most load-bearingly, whether Microsoft's
disconnected on-prem stack already ships a Copilot-for-PowerPoint-equivalent) remain unresolved.
...
absorbing it) reflects that difference honestly." Folding this brief into Mason's own brief, as done
...
narrative and the epics/blocked-status stay separate; only the planning-chain documents (this brief,
the PRD, the architecture spine, and the Spec) are consolidated into one per-station document.

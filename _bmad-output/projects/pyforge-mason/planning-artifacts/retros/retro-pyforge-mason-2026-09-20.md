---
title: 'pyforge-mason — Retrospective (2026-09-20: the fleet lint / type gate lands)'
...
evidence:
  - 'PR #1553 (2026-09-20): src/shared/packages/pyforge-mason/pyproject.toml, every .py under src/ and tests/'
  - 'pixi run -e pyforge-guild lint-types — exit 0 on the landed tree'
---

## What the gate found on its first run

- **Type baseline:** 4 modules baselined (3 codes). Every entry is `[[tool.mypy.overrides]]` per module with exactly the error codes that module trips — never `ignore_errors` — so the gate is live for every other module and code. Tighten or drop an entry whenever its module is touched; the baseline is a ratchet, not a floor.
- **Lint:** three unused `as mock_x` bindings in `tests/unit/test_package.py`; the mason meta-tests are per-commit (a commit touching a mason path must not also touch `CLAUDE.md`/`AGENTS.md`) — the landing commit had to be split for it.

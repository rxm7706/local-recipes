...
- `pixi run -e pyforge-mason pyforge-mason-test` -- expected: all unit + meta tests pass (default
  `-m "not slow"` loop); no real `twine` upload occurs anywhere in this suite (all `subprocess.run`
  calls mocked, matching `test_engines_pep517.py`/`test_engines_pixi.py`'s own convention).

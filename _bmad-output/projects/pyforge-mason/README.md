...
mason is the **packaging** station in the PyForge factory, responsible for
productizing the `conda-forge-expert` capability: recipe authoring, package
builds, and environment lockfiles (`mason recipe/package/environment`).
Never forks the craft — wraps the CFE skill + MCP surface.

## Structure

```
_bmad-output/projects/pyforge-mason/
├── planning-artifacts/
...
Real unit/integration/meta tests live at `src/shared/packages/pyforge-mason/tests/`,
not in this planning tree — this project has no `pytest.ini`, `playwright.config.ts`,
or `tests/` of its own.

## Tiers

- **Tier 0 (Dream)**: `docs/dreams/pyforge-mason*.md`
- **Tier 2 (Planning)**: `_bmad-output/projects/pyforge-mason/planning-artifacts/`
- **Tier 3 (Execution)**: `_bmad-output/projects/pyforge-mason/implementation-artifacts/` (gitignored)
...
1. Read the Dream (`docs/dreams/pyforge-mason*.md`) to understand the vision
...
5. Run the real test suite: `pixi run -e local-recipes pytest src/shared/packages/pyforge-mason/tests/`

## Testing

```bash
pixi run -e local-recipes pytest src/shared/packages/pyforge-mason/tests/ -v
```

See `src/shared/packages/pyforge-mason/README.md` for the full test/coverage setup.

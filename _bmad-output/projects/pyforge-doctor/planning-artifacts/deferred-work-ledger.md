...
- source_spec: `_bmad-output/projects/pyforge-doctor/planning-artifacts/specs/spec-deferred-work-visibility/SPEC.md`
  ...
  status: closed — Story 12.3 landed: INV-A now parses each open Spec's declared `CAP-N` ids from its own `## Capabilities` section against the `CAP-N` ids actually cited FOR THAT SPEC (per-Spec-scoped citation windows, anchored on each Spec's own slug text, closed at the next any-slug anchor or capped at the second `## ` heading following the anchor, and never opened by an anchor sitting before the prose's own first `## ` heading), reporting the specific uncovered ids instead of one project-wide substring pass/fail. A Spec with zero parseable CAP ids still falls back to the original bare-substring check, byte-identical.
  ...
  closed: 2026-08-21 — Story 12.3 (`spec-12-3-chain-completeness-parses-capability-ids-not-a-bare-substring-match.md`), Round 3 re-derive. Two prior code attempts were reverted via bad_spec loopback: Round 1's fix pooled cited CAP ids flat across a whole project, so two open Specs with overlapping declared ids cross-contaminated; Round 2's per-Spec-scoped fix (any-slug-occurrence anchoring, windows ended by the next any-slug anchor or capped at the second `## ` heading) closed that, but still let an anchor sitting in header-less preamble/changelog prose (live-reproduced at `pyforge-marshal/prd.md` line 8, before that file's first `## ` heading) open a window and sweep in an unrelated CAP id. Round 3 adds the one narrow fix — an anchor before the document's first `## ` heading never opens a window — plus deterministic glob-sorted/separator-joined prose concatenation and a single SPEC.md read reused for both frontmatter and CAP-id parsing. Verified live: the direct repro (`_cited_cap_ids_by_spec` against `pyforge-marshal`'s real concatenated prose) no longer credits `CAP-9` to `spec-surface-drift-reconciliation`, and `pixi run -e local-recipes python -m pyforge.doctor.sources chain-completeness` reports exactly 7 genuine INV-A `spec-not-decomposed` findings against this repo's real `_bmad-output/projects/` tree (pyforge-marshal x6, pyforge-steward x1), each naming its own specific uncovered CAP ids, with no reproducible cross-Spec leak — the same 7 as before this round's fix, since the preamble-anchor bug was latent (CAP-9 was never in `spec-surface-drift-reconciliation`'s own declared set) and never flipped an actual finding. NOTE: a follow-on `DW-CHAIN-COMPLETENESS-2` entry for those 7 genuine, out-of-this-story's-scope gaps was described during Round 2's review but never actually landed in this tracked ledger (it was part of that round's reverted diff) — not re-minted here, as minting a new entry is outside Story 12.3's own scope (Code Map names only closing this entry and flipping the hygiene-gap-catalog tag).

...
### DW-11-8-1: CAP-8 (backlog-intake check) split out of Epic 11, no follow-on Spec yet
- source_spec: `_bmad-output/projects/pyforge-doctor/planning-artifacts/specs/spec-deferred-work-resolution-sweep/SPEC.md`
  summary: former Story 11.8 ("backlog-intake surfaces deferred entries during story drafting," CAP-8) removed from Epic 11 2026-08-21. Its own Spec's Open Question — same story wave as CAP-1..7, or its own follow-on Spec, since it is write-adjacent to story-drafting rather than the read-only sweep the rest of the Spec is — resolved to "own follow-on Spec": Epic 11 shipped cleanly as Stories 11.1–11.7, a self-contained read-only pipeline, and bundling a write-adjacent capability into it after the fact would have muddied that result. `spec-deferred-work-resolution-sweep`'s own CAP-8 entry marked split-out; it now covers CAP-1..7 only.
  evidence: `epics.md` Epic 11 (Story 11.8 removed, replaced with a dated split-out note), `spec-deferred-work-resolution-sweep/SPEC.md` (CAP-8 entry + Open Questions), operator decision this session.
  status: open — no follow-on Dream/Spec exists yet for CAP-8. The capability itself (surface deferred-work entries naming an epic/story as candidate ACs during drafting) is real and not abandoned, just not yet re-scoped into its own effort.
  raised: 2026-08-21 — raised directly while resolving Epic 11's own closeout, tracked openly per this story's original text rather than silently dropped.

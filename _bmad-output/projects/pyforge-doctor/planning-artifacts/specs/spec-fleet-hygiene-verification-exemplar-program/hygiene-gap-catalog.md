...
`python -m pyforge.doctor.sources`'s 10 subcommands are the fleet's real always-on floor. Tonight's audit proved a mechanical detector can still have a real blind spot: `deferred-work`'s `tier3-only-deferral` finding (id-diff-based, misses headerless entries) (Status: shipped — spec-deferred-work-visibility CAP-4, doctor Epic 8 Story 8.1, done; verified 2026-08-21) and `chain-completeness`'s INV-A (bare substring test, misses partial decomposition — `DW-CHAIN-COMPLETENESS-1`, **this Spec's CAP-3**) (Status: shipped — doctor Epic 12 Story 12.3, done, landed; verified 2026-08-21). Each detector deserves the same scrutiny on its own schedule, not just when challenged.

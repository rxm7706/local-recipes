...
**Worked example.** A fresh audit reports: "`chain-completeness`'s INV-A check uses a bare substring test and misses partial decomposition." Scanning Category 1 first: it already names this exact defect as `DW-CHAIN-COMPLETENESS-1`. When Story 12.3 was still open, this line's `Status:` tag read `specced — this Spec's CAP-3, doctor Epic 12 Story 12.3, not yet landed` — that was answer 1, already known and already owned by a story, no new Spec or catalog entry needed, just point the finding at Epic 12 Story 12.3. (Story 12.3 has since landed — see Category 1's own tag below, now `partially shipped` — which is itself the maintenance discipline this section asks for: the tag moved in place rather than this example going stale silently, and stayed honest about the residual rather than rounding up to `shipped`.)

## Category 1 — Mechanically gated today
`python -m pyforge.doctor.sources`'s 10 subcommands are the fleet's real always-on floor. Tonight's audit proved a mechanical detector can still have a real blind spot: `deferred-work`'s `tier3-only-deferral` finding (id-diff-based, misses headerless entries) (Status: shipped — spec-deferred-work-visibility CAP-4, doctor Epic 8 Story 8.1, done; verified 2026-08-21) and `chain-completeness`'s INV-A (bare substring test, misses partial decomposition — `DW-CHAIN-COMPLETENESS-1`, **this Spec's CAP-3**) (Status: partially shipped — doctor Epic 12 Story 12.3, done (Round 4 re-derive, after three bad_spec loopbacks); the original bare-substring/flat-pooling defect is fixed and verified, but Round 4's own post-landing review found two further leaks in the same preamble-stripping heuristic, tracked as `DW-CHAIN-COMPLETENESS-4`/`-5` rather than a fifth loopback; verified 2026-08-21). Each detector deserves the same scrutiny on its own schedule, not just when challenged.
...
- `spec_surface_check.py --write-baseline`'s unlocked read-modify-write race (`DW-13-5-2`) → **this Spec's CAP-5.** (Status: SHIPPED — doctor Story 12.5 landed 2026-08-21, PR #604: advisory flock on a sidecar lockfile across the whole read→merge→write span + atomic tmp/os.replace write; race reproduced 4/20 pre-fix, 0/20 post. Id note: the atlas ledger records the race itself as `DW-13-5-3`; its `DW-13-5-2` is the adjacent staleness-safeguards entry, still open.)

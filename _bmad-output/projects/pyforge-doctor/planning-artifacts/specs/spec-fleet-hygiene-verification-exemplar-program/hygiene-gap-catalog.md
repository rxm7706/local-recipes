...
**Worked example.** A fresh audit reports: "`chain-completeness`'s INV-A check uses a bare substring test and misses partial decomposition." Scanning Category 1 first: it already names this exact defect as `DW-CHAIN-COMPLETENESS-1`, tagged `Status: specced — this Spec's CAP-3, doctor Epic 12 Story 12.3, not yet landed (as of 2026-08-21)`. That's answer 1 — already known and already owned by a story, with work still outstanding until Story 12.3 ships; no new Spec, no new catalog entry, just point the finding at Epic 12 Story 12.3.

## Category 1 — Mechanically gated today
`python -m pyforge.doctor.sources`'s 10 subcommands are the fleet's real always-on floor. Tonight's audit proved a mechanical detector can still have a real blind spot: `deferred-work`'s `tier3-only-deferral` finding (id-diff-based, misses headerless entries) (Status: shipped — spec-deferred-work-visibility CAP-4, doctor Epic 8 Story 8.1, done; verified 2026-08-21) and `chain-completeness`'s INV-A (bare substring test, misses partial decomposition — `DW-CHAIN-COMPLETENESS-1`, **this Spec's CAP-3**) (Status: specced — doctor Epic 12 Story 12.3, backlog, not yet landed; verified 2026-08-21). Each detector deserves the same scrutiny on its own schedule, not just when challenged.

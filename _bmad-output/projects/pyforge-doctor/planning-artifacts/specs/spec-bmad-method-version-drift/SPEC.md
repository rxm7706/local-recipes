...
- **CAP-1 — declared-vs-installed drift.**
  ...
  - **verified:** 2026-09-11 — PASS — mechanical re-verification at HEAD a0aba94b0a: `pixi.toml` declares `bmad-method = ">=6.12.0"`, `_bmad/_config/manifest.yaml` reports installed `version: 6.12.0` — both agree today, and no `bmad-method-version-drift` Finding appears in the live `fleet-picture` ATTENTION block, matching the "given both agree, no Finding fires" branch; `tests/unit/test_sources_bmad_method.py` (157 tests, incl. the drift-fires branch on synthetic disagreement) passing.
...
- **CAP-2 — installed-vs-upstream-latest drift.**
  ...
  - **verified:** 2026-09-11 — PASS — mechanical re-verification at HEAD a0aba94b0a: `_fetch_latest_upstream_version` present in `sources/bmad_method.py`, live per-check `urllib.request` call against npm's public registry, `_UPSTREAM_FETCH_TIMEOUT_SECONDS = 5.0` bounding it; `tests/unit/test_sources_bmad_method.py::test_installed_behind_latest_upstream_reports_second_warn_naming_both_versions` passing, plus fail-open coverage for the timeout/unreachable branch (same file, 157/157 green).
...
- **CAP-3 — ambient, non-gating surface.**
  ...
  - **verified:** 2026-09-11 — PASS — mechanical re-verification at HEAD a0aba94b0a: live `fleet-picture` ATTENTION block currently carries the sibling CAP-4 finding (`bmad-suite-upstream-drift: installed bmad-eval-quality 1.4.1 is behind the latest upstream release 3.0.0` — deliberately unresolved this session pending a win-64 build blocker, see PR #1205), proving the ambient surface fires and renders where operators already look; `Source.BMAD_METHOD_VERSION_DRIFT` confirmed `warn`-only in `models.py`.
...
- **CAP-4 — suite-vs-upstream drift** *(added 2026-08-21, superseding the former "core only" non-goal).*
  ...
  - **verified:** 2026-09-11 — PASS — mechanical re-verification at HEAD a0aba94b0a: live end-to-end — the watched set genuinely tracks `pixi.toml`'s `bmad-*` pins (this session's own PR #1205 bumped `bmad-eval-quality`/`bmad-method-test-architecture-enterprise` in `pixi.toml` and the very next `fleet-picture` run picked up the new floors with zero code change, firing exactly one still-live drift line for the recipe deliberately held back — `bmad-eval-quality 1.4.1 < 3.0.0` — proving derivation, not a hardcoded list); zero `DoctorStatus.FAIL` constructions anywhere in `sources/bmad_method.py` (warn-only holds); 157/157 unit tests passing.

...
**Outcome (2026-09-16, worktree dispatch — still `backlog`):** the re-derive half is done and on `main` (`9bafe966f51`, 2026-09-15: `deck-trio <slug> --head --deck` + `deck-facts --refresh` for all ten, recovered after a prior loop attempt never reached `main`). Re-verified this session: `deck-facts <slug> --check` reports **0 `mismatch`** for all eight PyForge station decks (atlas/doctor/herald/marshal/mason/scribe/steward/warden); the only findings are ordinary day-over-day `drifted` counters (`tree_commit_date`, `stories_done_total`, …) unrelated to this story. Every README's ledger already records "Design push/read-back: not performed — no working credential", so **"standalone ahead" is still present verbatim in all ten READMEs** (quoted while explaining it no longer applies to content, only to the Design mirror).

The push leg is blocked by **two independent, confirmed causes**, not one:
1. **`mcp` SDK compat break.** `pixi.lock` now resolves `mcp==2.2.0` for the `pyforge-herald` environment (some other environments still pin `1.28.1`/`1.26.0`); `herald deck push <slug>` fails before any network call with `TransportError: the mcp SDK is not importable (cannot import name 'streamablehttp_client' from 'mcp.client.streamable_http' ...)`. This reproduces on `main`, independent of any worktree or credential state, and is a cross-cutting dependency fix (own Dream/Spec territory per this repo's Dream-first rule, not a one-line patch inside this story).
2. **No working Design credential**, exactly as every README already recorded on 2026-09-15: this session's `claude-design` MCP connector reports `FIRST_PARTY_AUTH_REJECTED` (HTTP 403), and `~/.claude/.credentials.json` has no working `designOauth` block. `/design-login` is an interactive Claude Code slash command a human must complete in a real session; it cannot be run from an unattended dispatch.

Also newly noted: `.herald/bridge-state.json` is gitignored, per-checkout state — a fresh worktree has none, and `herald deck seed <slug>` refuses to recreate it for an already-linked deck (README-bootstrap fallback exists for `seed`'s own conflict check, not for populating state; `deck_pipeline.py::_require_seeded_state` has no "adopt" path). Reconstructed by hand for `pyforge-warden` from the README's recorded `project_id` (via `pyforge.herald.state.write`, `etags={}` — safe: push only uses local etags to skip already-pushed files, so an empty map just means "push everything") purely to exercise `push` and surface finding 1 above.

No PR opened this pass: nothing beyond the already-merged `9bafe966f51` local re-derive is completable without a working push, and 21.6–21.9 (the four chain decks) hit the identical pair of blockers for the same reason — their own local prepare (`e483288d54f`) is likewise on `main` with the Design push left `PENDING-PUSH`. Story stays `backlog` (mirrors 21.6–21.9's own unchanged status under the same blocker) pending an operator `/design-login` session and a separate fix/pin for finding 1.

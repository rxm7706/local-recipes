...
**Files changed:**
- `src/shared/packages/pyforge-herald/src/pyforge/herald/sync_all.py` — new module: `sync_all()`, `DeckSyncReport`/`SyncAllReport`, the `FactsRefresher`/`DeckDeriver`/`SitePublisher`/`LocalEditDetector` injectable seams and their real Pixi/Git implementations.
- `src/shared/packages/pyforge-herald/src/pyforge/herald/cli.py` — `herald deck sync-all --slug --dry-run [--repo-root]` wiring, incl. the empty-decks and publish-failure print lines.
- `pixi.toml` — `deck-sync-all` task.
- `docs/how-to/presentation-deck.md`, `presentations/README.md` — point at the one command instead of the manual step sequence.
- `src/shared/packages/pyforge-herald/tests/unit/test_sync_all.py`, `tests/unit/test_cli_sync_all.py`, `tests/meta/test_deck_sync_all_pixi_task.py` — new; `tests/unit/test_bridge.py` — registers `sync_all` in the determinism-boundary sweep.

**Review findings breakdown** (20 findings from 4 layers; verified against the code directly, not taken as filed):
- Patched (14, all applied and re-verified in the patched tree): dry-run crashed (`failed` instead of `unchanged`/`would-sync`) on any deck with a prior push, because the preview walked `export:*` etag keys `_remote_path_for_artifact` doesn't recognize [high ×2]; a tail-step failure (`refresh`/`derive`/`push`) discarded that deck's already-true `pulled`/`overwrote-local` facts [medium ×2]; a freshly-seeded-but-never-pulled deck (or one missing an artifact kind) silently reported plain `unchanged` [medium ×2]; an unguarded site-publish failure discarded every already-gathered per-deck report [medium]; missing test coverage for the standalone-bundle/marp-source pull branches [medium]; per-deck isolation silently swallowed `AuthError` contrary to the module's own docstring [medium ×2]; dry-run conflated a Design-unreachable "conflict" with a genuine pending change [low]; a bad `--repo-root` with no `--slug` silently reported success doing nothing [low]; `now()` was invoked twice (dirty-check vs. the pull's own write), able to disagree across a UTC-midnight boundary [low ×2].
- Deferred (1): the idempotency AC is proven over fakes plus one live smoke test that only exercised the `skipped` path (no live Design credentials in this environment) — `maybe-false`, recorded in frontmatter `deferred`.
- Rejected (5): "overwrote-local dead for 2 of 3 pull kinds" — false, the mechanism is accurate given the underlying kernel verbs' date-partitioned file naming, nothing was actually overwritten in the cases claimed; "no incremental progress output" — true but no named concrete harm, cosmetic; "no commit step" — false, the intent-contract's named steps stop at publish and every called kernel verb treats commit as opt-in by its own docstring; a pull/write TOCTOU on `is_dirty()` — real but a sub-second window needing a human editing the exact file at the exact moment, fix is more than a direct correction; dry-run's narrower report vocabulary vs. epics.md's "the same report" — the literal reading is infeasible without violating the intent-contract's own "do not re-implement kernel verbs" constraint, and the implemented, narrower scope is consistently documented everywhere (`--help`, pixi task description, the runbook).

**Follow-up review recommendation:** `true` — two `high`-verdict entries were patched this (first) pass. Specific unverified risk: `_dry_run_preview` now re-implements its own read-only etag-comparison loop (looping `existing.etags`, calling `_remote_path_for_artifact` + `transport.read_file` directly) in parallel with `deck_pipeline._status_for_slug`'s near-identical loop, rather than sharing it — a future change to `_remote_path_for_artifact`'s per-key resolution logic could silently diverge between the two copies with no shared test to catch the drift. Patched counts by entry verdict: high 2, medium 8, low 4.

**Verification performed:**
- `pixi run --frozen -e pyforge-herald pyforge-herald-test` (the spec's mandated command): 1465 passed / 4 skipped before the patch pass; 1476 passed / 4 skipped after (11 new tests from the 9 patches).
- Every patched fix was re-read directly in `sync_all.py`/`cli.py` against the patch subagent's own report (not trusted at face value) — all 9 confirmed present and correct: the export-key skip in `_dry_run_preview`, the conflict/would-sync split, `AuthError` re-raise in both the per-deck loop and `_sync_one_deck`'s tail try/except, the tail-failure `DeckSyncReport` preserving `pulled`/`overwrote_local`, the frozen `now()`, the empty-etags distinct `skipped_reason`, the `publish_error` field plus its `try/except` wrap and the CLI's two new print lines.

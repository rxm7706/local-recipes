...
**Summary:** A new `pull_request`-triggered workflow, `.github/workflows/docsite-check.yml`, path-filtered to `docsite/**`, `docs/dashboard/**`, `presentations/**`, `pixi.toml` and its own workflow file, runs two checks before merge: `docsite/build.py --check` with the same pip-installed deps `dashboard.yml` itself uses (predicting whether that build + check step would pass; it does not run `dashboard.yml`'s separate advisory `verify_claims.py` step), and `pixi run -e site site-check` (the existing task, reused not re-minted). `pixi.toml`'s `pr-preflight` gained a sixth leg, `{ task = "site-check", environment = "site" }`, so a local run predicts the lane. `dashboard.yml` is unchanged and stays the only `deploy-pages` caller. `cancel-in-progress` only cancels superseded PR runs, matching `platform-ci.yml`'s convention of never cancelling a push:main run.
...
- `docsite/README.md` — "Rebuilding" section gained a line naming the new CI lane and that `pr-preflight` predicts it.
- `_bmad-output/projects/pyforge-herald/planning-artifacts/deferred-work-ledger.md` — DW-FU-23-5 marked `status: done 2026-09-18` with a `verified:` note and a "SCOPE OF THIS CLOSURE" paragraph; new entry `DW-FU-24-2-1` (`status: open`) split off to track the still-missing `pyforge-herald-test` coverage of `docsite/`.
...
**Review findings breakdown:** Four independent layers (Blind Hunter, Edge Case Hunter, Verification Gap Reviewer, Intent Alignment Auditor) reviewed the story-scoped diff — 14 findings total, high 0, medium 3, low 7, false 4, maybe-false 0. See `## Review Triage Log` above for every finding, its verdict, and evidence.
- **Patched (8 entries: medium 2, low 6):** DW-FU-23-5 over-closure (split into a scoped closure + new `DW-FU-24-2-1`); `docsite-check.yml` header overclaim about `verify_claims.py` parity; unconditional `cancel-in-progress` (now PR-only, matching `platform-ci.yml`); path filter missing `presentations/**` (the actual source `collect_families` reads — closes a real gap where a broken deck asset could merge ungated); path filter missing `pixi.toml`; spec's self-contradicting `Status:` line; spec's "Files changed" list omitting `SPEC.md`; `docsite/README.md` not mentioning the new lane.
- **Rejected as low (1):** `pr-preflight`'s `site-check` leg doesn't byte-for-byte mirror CI's separate pip-installed-deps step — no demonstrated version drift, and matching the two provisioning paths exactly would add disproportionate complexity.
- **Rejected as false (4):** composite `pr-preflight` "never verified end-to-end" — ran it myself; it fails at a pre-existing, unrelated `detectors-ci` leg (tornado doc drift, a `PYTHONSAFEPATH=1` artifact, other stations' ledger-direction findings), never reaching the new leg, which independently passed. Two Intent Alignment divergences (fixture-as-prose vs. durable test; autonomous PR-open vs. "operator-run") — both resolved by the spec's own pre-existing `**Manual checks:**` section, which explicitly frames this as a manual check requiring a real PR's first green run. One Intent Alignment divergence (no proof the check is a required, merge-blocking status check) — `gh api .../branches/main/protection` returns 404; `main` has no branch protection repo-wide, true of every existing CI lane, not something this diff changed.

**Follow-up review recommendation:** false. Two `medium`-verdict entries were patched (DW-FU-23-5 ledger split; the `presentations/**`/`pixi.toml` path-filter gap), which meets the mechanical "2+ medium patched" trigger, but no specific unverified risk survives naming: the ledger split was independently confirmed via `pixi run -e local-recipes python -m pyforge.doctor.sources deferred-work` (clean), and the path-filter fix was independently confirmed via a live GitHub Actions run on the exact patched commit (`e34859244e`, run 35397320863, job "Docsite check" — SUCCESS), not just re-asserted by the same subagent that wrote it. Patched counts by verdict: medium 2, low 6.
...
- (Review pass, post-patch) `pixi run --frozen -e pyforge-steward pytest src/shared/packages/pyforge-steward/tests -k "path_filter" -q` — 1 passed (the `pull_request.paths`/`push.paths` identity meta-test, now covering the added `presentations/**`/`pixi.toml` entries).
- (Review pass, post-patch) `pixi run -e local-recipes python -m pyforge.doctor.sources deferred-work` — `ok — every Tier-3 deferral has a tracked twin` (confirms the DW-FU-23-5/DW-FU-24-2-1 ledger split is structurally sound).
- (Review pass, post-patch) `docsite/build.py --check` re-run against the patched tree — clean.
- (Review pass, post-patch) `pixi run --frozen -e pyforge-herald pyforge-herald-test` re-run against the patched tree — 1478 passed, 4 skipped.

**Live CI confirmation (PR #1472):** pushed, opened against `main`, `maintenance` label applied. First run, pre-patch commit `71c8c7a222`: `Docsite check` job SUCCESS in 26s (run 35389603779). After the review pass's 8 patches (commit `e34859244e`), the lane re-ran automatically on the same PR and is again SUCCESS (run 35397320863) — confirming the new `presentations/**`/`pixi.toml` path-filter entries and the conditional `cancel-in-progress` all work in real GitHub Actions, not just locally. At time of writing the full 26-job rollup has 0 failures with 7 unrelated jobs (`atlas-test`, `marshal-test`, both `container`/`guild-container` variants, `test`, `cfe-regression-net`) still `IN_PROGRESS` — normal for this repo's full matrix; `mergeable: MERGEABLE`.

**Residual risks:**
1. **`pixi run -e pyforge-guild detectors-ci` still exits 1 in this session's shell** (confirmed harmless — the real `detectors` GitHub Actions job on PR #1472 passed on both the pre- and post-patch commit), for findings verified unrelated to this story: (a) `llms_full_check` reports `tornado` as an undocumented dep in `docs/reference/library-llms-full.md` — pre-existing, this story added no dependency; (b) `scripts/pixi_version_check.py` crashes with `ModuleNotFoundError: No module named 'pixi_version_registry'` — reproduced as a `PYTHONSAFEPATH=1` session artifact (this shell has it set; `env -u PYTHONSAFEPATH python3 scripts/pixi_version_check.py --help` runs clean), the same root cause already on file as `feedback_pythonsafepath_breaks_render_skill.md`, not a repo bug; (c) `ledger-direction` reports four unrelated stories (three `pyforge-atlas`, one `pyforge-marshal/50-2`) as landed-but-unpromoted — unrelated fleet ledger-sync gaps, grown by one entry since the implementation subagent's original run as other stations' work continued landing on `main` concurrently. None of these are in this story's Surface, and the actual CI `detectors` job (which wraps this same sweep advisory) passed both times.
2. **`pixi run -e local-recipes test-ci` also showed 5 pre-existing failures in this session's shell, unrelated to this story and NOT present in the real `cfe-regression-net` CI job**: all `test_all_scripts_runnable.py::test_script_responds_to_help` for `add_handoff.py` / `inventory_match.py` / `library_futures.py` / `recommend_2027.py` / `universe_sbom.py`, all `ModuleNotFoundError` for sibling-module imports (`conda_forge_atlas`, `export_purls`) — the same `PYTHONSAFEPATH=1` session artifact as above.
3. **DW-FU-24-2-1 (new, `status: open`)**: `pyforge-herald-test` still has zero unit-test coverage of `docsite/build.py`'s own logic (`collect_families`, `collect_infographics`, the `check()` predicates) — the new CI lane catches a broken *build*, not a broken *function*. Deliberately left open, not this story's Surface (a real test suite for `docsite/` is a separate scoping question this story did not decompose) — tracked so it isn't lost.
4. **Not merged.** PR #1472 is open and green (both pre- and post-patch commits); merging is left as the next, deliberate step (not taken automatically in this session).
...
- findings:
  - `[medium]` `[patch]` DW-FU-23-5 marked fully `done` although its own `summary` names two problems (no PR-gating CI lane; `pyforge-herald-test` has zero `docsite/` coverage) and this diff resolved only the first — the second is now untracked, not actually fixed. Action: split the ledger entry per the Story 13.1 "SCOPE OF THIS CLOSURE" precedent — keep the CI-lane half `done`, add a residual entry for the missing `pyforge-herald-test` coverage of `docsite/`. **Applied:** kept DW-FU-23-5 `done` with a new "SCOPE OF THIS CLOSURE" paragraph, and split the untouched half into new entry `DW-FU-24-2-1` (`status: open`, severity medium).
  - `[low]` `[patch]` `docsite-check.yml`'s header comment claims the lane "predicts precisely what the real deploy step would do," but it never runs `docsite/tools/verify_claims.py` (`site-verify`), which `dashboard.yml` does run (advisory, `continue-on-error`) — verified via `grep -n "verify_claims" .github/workflows/dashboard.yml`. Action: correct the comment's scope claim. **Applied:** reworded to "predicts whether that build + check step would pass" and explicitly notes it does not run `verify_claims.py`.
  - `[low]` `[patch]` `spec-24-2-...md`'s `## Auto Run Result` top `Status:` line ("PR not yet opened/merged") contradicts its own later "Live CI confirmation (PR #1472)" paragraph (PR pushed, opened, green) — verified by reading both lines in the current file. Action: update the `Status:` line. **Applied:** `Status:` now reads "implemented, verified locally and in CI; PR #1472 open and green, not yet merged".
  - `[low]` `[patch]` `docsite-check.yml` sets `cancel-in-progress: true` unconditionally, including on `push: branches: [main]`, contradicting this repo's own established convention in `platform-ci.yml` (`cancel-in-progress: ${{ github.event_name == 'pull_request' }}`, "so main always keeps one full, uncancelled CI result per commit to bisect against") — verified by reading `platform-ci.yml:161-165`. Action: mirror that same conditional. **Applied:** changed to `cancel-in-progress: ${{ github.event_name == 'pull_request' }}` with a matching rationale comment.

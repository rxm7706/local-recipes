...
**H. CLI Operation & Configuration** — FR29 one non-interactive command → one exit code · FR30 dual `[tool.pyforge-warden]` config (pyproject+pixi, per-key precedence) · FR31 `--version`/`--help` stable contract.
...
- **Config:** dual TOML `[tool.pyforge-warden]`; waiver file `.warden-waivers.yaml` (read-only by scan; `--bypass` stanza emitted for the human to commit).
...
Greenfield project; a scaffold already exists at `src/shared/packages/pyforge-warden/` (pixi build member, cli stub, 2 smoke tests green). The first story is *complete-the-scaffold*, not *create-from-template* — lighter than a typical greenfield Story 1.1.
...
4. **Commit** the completed PRD + this readiness report (both are tracked Tier-2 planning artifacts) on branch `claude/pyforge-warden`.

...
**H. CLI Operation & Configuration** — FR29 one non-interactive command → one exit code · FR30 dual `[tool.pyforge-warden]` config (pyproject+pixi, per-key precedence, owns the hygiene→status + CVSS-threshold policy tables) · FR31 `--version`/`--help` stable contract.

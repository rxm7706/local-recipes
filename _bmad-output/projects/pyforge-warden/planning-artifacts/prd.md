...
vision:
  ...
  coreInsight: "Both jobs start from the same manifest parse; build ONE conda/pixi-native manifest front-door feeding independent per-axis assessment paths (v1 axes: hygiene via deptry source-tree AST + import->distribution mapping; security via osv-scanner versioned lockfiles + the CISA-KEV and EPSS gates; license + currency with flag-activated v1 gates, unconfigured → visible warn), emit ONE schema-validated report behind ONE exit code. (Multi-axis per the 2026-07-15 story-0.1 replan; D12 2026-07-16.)"
...
prioritizedRefinements:
  v1_must:
    ...
    - "FR5 (severity-tiered gate): exit 0/1/2; --fail-on=<severity> or max_critical/max_high/KEV; default block on critical CVE (KEV tier deferred post-v1 — REVERSED by D3/D12; historical), warn on high/med/low + all hygiene. Retires coarse --no-fail-on-*. Gate decides on report CONTENT + severity, never subprocess returncode"
...
replan:
  date: 2026-07-15
  story: "0.1"
  upstream: docs/specs/pyforge-warden.md
  note: "Spec-first replan: the spec is the sole source of truth (v1 / v1.x / vision tiering); this PRD was re-derived from it. prioritizedRefinements above are historical intake labels (pre-replan). Evidence: planning-artifacts/adversarial-review-pyforge-warden-spec-2026-07-15.md"
  rebaseline: "2026-07-16 (D12, owner-directed): v1 absorbs the axis gates (flag-activated), EPSS --min-epss, baseline & grandfathering (FR39), and the fix-PR actuator (FR40). FR space grown to FR1-FR40."
---

# Product Requirements Document - pyforge-warden

**Author:** rxm7706
**Date:** 2026-07-11 · **Replanned:** 2026-07-15 (story 0.1)

> **⚠️ Story-0.1 Replan (2026-07-15) — WINS over everything below.** This PRD was re-derived from `docs/specs/pyforge-warden.md` (the sole source of truth, tiered **v1 / v1.x / vision**). The product is a **pluggable multi-axis compliance gate**; per **D12 (2026-07-16)** v1 ships all four axes **with their gates**: hygiene + security (CISA-KEV **and** EPSS) gate by default; license + currency gates are **flag-activated** (unconfigured → visible `warn`, never a silent pass) — plus **baseline & grandfathering** (FR39) and the **fix-PR actuator** (FR40). Canonical FRs grew **FR32–FR40** (sections I–L below); **NFR-S9** added. Where any earlier text — including the 2026-07-11 reconciliation note — conflicts with the spec or this replan, **the spec wins**. Evidence base: `planning-artifacts/adversarial-review-pyforge-warden-spec-2026-07-15.md`.
...
> 3. **DEP001 (missing dependency) blocks by default**, gated on conda↔pypi name-mapping confidence (high-confidence → block, ambiguous → `warn`); DEP002–005 → `warn`. Hygiene is a **separate axis** from the CVSS gate; the hygiene→status + CVSS-threshold tables live in the FR30 `ConfigLoader`. (Refines FR18/FR8, Gap A.)
...
> **⚠️ Phase-0 review corrections (2026-07-12) — second reconciliation pass.** A full-corpus review (findings: gist `326be5f25e702e0fcce343046c70a6b2`) landed these corrections: **(a)** the exit projection is pinned — **`indeterminate` → exit 1** (exit 2 stays reserved for operational error; FR20); **(b)** J1's "warn at minimum," FR16's "clean at N%," and the offline-DB state-machine cell predated the `indeterminate` state and are corrected inline; **(c)** **FR19 is repurposed** as the warn-only coverage guardrail (post-triad, any coverage gap already exits non-zero via `indeterminate`); **(d)** persona **P8 (local developer / workstation mode)** + Journey 10 added — the `--bypass`→commit flow is local-only by design; the primary consumer remains the pipeline; **(e)** "parallel execution" in Growth means *fleet/multi-manifest* parallelism — the two engines are parallel in v1 (NFR-P-concurrency); **(f)** the 2026-07-12 ecosystem surveys (nebi + the prefix-dev org) folded in: **nebi workspaces are pixi workspaces** (scannable with zero new code; nebi-versioned envs + decision-determinism ⇒ diffable reports), the bundled map gains a **parselmouth-direct** refresh mode (regenerable without the atlas — multi-org), **py-rattler `LockFile`** serves as a *test-side* lockfile oracle (never a run-dep), **pixi-pack** + **nebi** join `pixi global` as Story-1.4 distribution-channel candidates, and **`rattler-build-parser-tests`** feeds 5.2's adversarial corpus; **(g)** implementation executes **loop-driven** — `bmad-loop` v0.8.1 orchestrating `bmad-dev-auto` (BMAD 6.10) with graduated gates (per-story-spec-approval for 1.1/1.2 → per-epic from Epic 2) and the scanner's own test suite as the deterministic verify command — see `docs/specs/bmad-loop-adoption.md` (user decision "Option B", 2026-07-12).

## Executive Summary

**pyforge-warden** (**Warden**) is a non-interactive CI/CD quality-gate CLI that runs **four axes of dependency trust** — hygiene (`deptry`), security (CVEs via Google `osv-scanner`, with the CISA-KEV + EPSS gates), license (`license-expression`, flag-activated gate), and currency (LTS/EOL tiers, flag-activated gate) — in a single schema-validated pass behind **one exit code**, for **Python libraries sourced from either PyPI or conda-forge** (D12, 2026-07-16). For PyPI-world projects both engines already work natively — deptry reads `pyproject.toml` (PEP 621 / Poetry / PDM / uv / setuptools) and `requirements.txt`; osv-scanner reads the lockfiles (`poetry.lock`, `pdm.lock`, `uv.lock`, `Pipfile.lock`, `pylock.toml`, `requirements.txt`) — so pyforge-warden **orchestrates and unifies** them. Its differentiated wedge is **extending both engines to the conda/pixi formats neither parses natively** — `environment.yml`, v0 `meta.yaml`, v1 `recipe.yaml`, and `pixi.toml` — via a manifest-resolution bridge, making conda-forge-sourced Python projects first-class alongside PyPI ones. (See § Supported Dependency Managers & Lockfiles.)
...
**Core insight:** both jobs begin at the same manifest parse. pyforge-warden builds **one conda/pixi-native manifest front-door feeding independent per-axis assessment paths** — deptry's (source-tree AST + import→distribution mapping; versions ~irrelevant), osv-scanner's (versioned lockfiles; name-only is last resort), and the D12 license/currency producers over the same resolved inventory — and emits one consolidated, schema-validated `ComplianceReport`.
...
- **Context:** Greenfield (complete intake spec + committed build scaffold at `src/shared/packages/pyforge-warden/`; no running system).
...
**PyPI path — delegate to the engines' native parsers, then unify** (pyforge-warden adds no bespoke parsing here):
...
- → pyforge-warden runs each engine on its native input and consolidates the result.

**Conda/Pixi path — the differentiated bridge (E1):** **neither** engine parses `environment.yml`, `meta.yaml` (v0), `recipe.yaml` (v1), `pixi.toml`, `pixi.lock`, or `conda-lock.yml`. pyforge-warden's manifest engine extracts the dependency set and bridges it — a synthesized `requirements.txt` for deptry, and version-pinned requirements (from `pixi.lock` / conda where present, else **name-only + marked** per OD2) for osv-scanner, since osv has **no** conda/pixi-native format.

**Stated precisely:** pyforge-warden is *not* re-implementing PyPI dependency parsing — it **extends two mature engines to the conda/pixi ecosystem they don't cover**, behind one unified gate. Non-Python osv-scanner ecosystems (npm, Go, Rust, …) and its container/artifact scanning are **out of v1 scope** (Python-first: PyPI + conda-forge).

## Success Criteria

### User Success

- A **Python developer shipping pip- or conda-sourced software (scripts, applications, components, libraries)** — whether deps come from PyPI or conda-forge — runs `pyforge-warden` once and gets a unified verdict across all four axes — hygiene + vulnerability, plus license + currency on every resolved component (gates flag-activated, D12). PyPI projects: deptry + osv consolidated (no more two CI steps + `jq`). Conda/pixi projects: **source-manifest** hygiene+vuln coverage they had none of before (no manual `requirements.txt` translation; incumbents scan only a *built* environment).
...
- Exit-code matrix 100% correct across the full projection (clean/warn/not-applicable/bypassed → 0; un-waived policy-violation → 1; **indeterminate → 1**; error → 2; SIGINT → 130) + each typed `error_kind`; **false-green = 0** on seeded fixtures; a **non-expired waiver → exit 0** (`bypassed`, `review_required`) and an **expired waiver → re-block (exit 1)**; report schema-valid in CI (100%).
...
- **Multi-axis outcomes (D12):** license/currency verdicts present for 100% of resolved components (verdict ∈ allowed/denied/unknown · supported/eol/unknown); unconfigured-axis unknowns render status `warn`, exit 0 (never `clean`); with a configured axis policy the same fixtures exit 1 (`policy-violation`/`indeterminate`); KEV- or EPSS-feed absence under an active policy → `indeterminate` on seeded fixtures; baseline suppresses only baselined finding-IDs and expired entries re-block; `--open-fix-prs` never mutates the scanned tree (asserted) and dry-run opens nothing. *(The executable form is the spec's v1 multi-axis DoD.)*
...
**PyPI path:** orchestrate deptry (native pyproject/requirements) + osv-scanner (native lockfile) → unified `ComplianceReport` + gate. **Conda/pixi path (E1 bridge):** extract deps from `environment.yml`/`meta.yaml` (v0)/`recipe.yaml` (v1)/`pixi.toml` (stdlib-lean, two-pass eval, name-only+marked, per-manifest coverage); synthesize `requirements.txt` for deptry + version-pinned reqs for osv (from `pixi.lock`/conda else name-only). **E4:** schema-validated `ComplianceReport` (`status` + severity + `schema_version` + coverage + `error_kind`) + human summary + **severity-tiered exit-gate** (FR5: 0/1/2; `--fail-on` / `max_critical`/`max_high`/KEV; default critical+KEV blocks) + typed errors (FR10) + an **auditable expiring bypass** (FR9: `--bypass` → committed waiver, default 14d / config / per-repo, `review_required`); **emit a schema-validated CycloneDX SBOM (FR8 — correct purls, coverage-marked)**. Corpus + fixture tests; conda/wheel/sdist build; respect `[tool.deptry]` ignores. **Multi-axis (2026-07-15 replan; re-baselined 2026-07-16, D12):** license (FR32) + currency (FR34) ship **with their gates, flag-activated** (FR33/FR35 — unconfigured, verdicts feed the `warn` rung, FR37; configured → `policy-violation`/`indeterminate`); KEV **and EPSS** enrichment + TOML `fail-on-kev` + CLI `--min-epss` + feed-absence semantics (FR36); **baseline & grandfathering** (FR39 — gate new findings only); the **fix-PR actuator** (FR40, opt-in `--open-fix-prs`); the versioned schema amendment (FR38, story 6.1); `warden scan --doctor` (flag, not verb); **internal JFrog distribution behind the engine version-range gate** (NFR-C1, story 6.6).

### Growth Features (Post-MVP)

*(= the spec's **v1.x** bucket as re-baselined 2026-07-16, D12 — the axis gates, EPSS, baseline ratchet, and fix PRs moved INTO v1.)* Public PyPI + conda-forge publish · channel/index-provenance axis (must read pixi config layers, not just locks) · **SARIF output** · **vendor-support backlog** · **registry perimeter** (JFrog block/allow + quarantine) · engine-swappability (`fawltydeps`, `pip-check-reqs` via `--engine`; pluggable vuln backends) · client provisioner (perimeter-scoped) · `vers` ranges · more osv-native lockfiles · better conda↔PyPI name reconciliation *(the v1 axes already run in parallel — NFR-P-concurrency; cf_atlas promotion lives in the post-v1 backlog per the spec's § Future/backlog; waivers already cover vuln findings in v1 — FR24/J4)*.

### Vision (Future)

Axes 5–6 (Sigstore/SLSA provenance; OpenSSF Scorecard maintenance) · malicious-package + **typosquat/name-squat** detection · **public-upstream ring → blocklists** · reachability · alternate-library suggestions · OpenVEX/CSAF · TUI/IDE · non-Python osv ecosystems + container/artifact scanning · default fleet supply-chain gate / the atlas's authoritative signal · the fleet **control plane**, OSPO practice, leader scorecards, and ~60-tool integration surface — full catalog: `docs/specs/pyforge-warden.md` § Vision catalog.
...
*Reveals:* E1 6-format bridge + two-pass eval; **coverage as a first-class, split (hygiene/vuln) report field**; the `indeterminate`-not-`clean` invariant on partial coverage; name-only+marked degrade. *Implies FR-NEW-A (coverage-as-requirement).*
...
**Rising action.** Each run emits a schema-validated `ComplianceReport` (`status` + `severity` + `schema_version` + **per-axis** `coverage` + per-axis `gating` bool + `error_kind`) and a **CycloneDX SBOM** of the resolved inventory. *(2026-07-15: a non-gating axis is never invisible to this consumer — its unknowns surface as `warn` in the status channel, FR37.)*
...
**Rising action → resolution.** A **warn-only** first-run mode reports everything but exits 0 (findings surface as `warn`, loud in the report), letting the team adopt without a day-one red wall and burn the backlog down deliberately. This is the pressure-release valve, distinct from the severity gate — it *is* the on-ramp. *(D12 2026-07-16: the new-findings-only **baseline & grandfathering ratchet ships v1** (FR39) — the on-ramp is FR39 + warn-only. License/currency unknowns feed `warn` by default (FR37), so first contact is honest but not red.)*
...
**Opening.** Before wiring CI, a developer trials the gate locally: install to the workstation (`pixi global install` / the local channel; air-gapped via a **pixi-pack** bundle; nebi-adopted teams via **nebi pull** — Story 5.1 docs), then `pyforge-warden scan . --warn-only` — the recommended first contact with the tool.

**Rising action.** Cold start bites first: no offline DB on the laptop → a typed error with an **actionable nudge** (provision the DB / point `--db-path` / use the explicit online mode if Story 1.4 ships one — never a silent fetch, NFR-S2). A `doctor`-style self-check (FR21's detection logic re-exposed; v1-if-cheap) answers "is this machine even scan-capable?"
...
*Reveals:* the workstation install story (5.1); cold-start provisioning UX + the online-opt-in decision (1.4); `scan --doctor` committed to v1 as a flag (D8); the waiver-authoring loop as a local-first flow.
...
- **Severity taxonomy** — **CVSS** (v3.1 / v4.0) is the **v1 gate signal**; **CVE** identifiers; **CISA KEV** gates in v1 (TOML `fail-on-kev`, default true — FR36; **not** a CLI flag); **EPSS** `--min-epss` also v1 per D12 (see Integration Requirements).
...
- **Vuln-DB provenance & currency (mandatory).** Every report **records the vuln-DB source + snapshot timestamp**; a **staleness signal is mandatory in offline mode** (DB older than a configurable N days). Per FR12/NFR-S8/C0 the stale-DB outcome is **`indeterminate` → exit 1** (a typed staleness driver; never a confident clean, never a silent 0) — the same fail-loud rule as feed absence under an active policy (FR36).
- **Determinism for forensics.** NFR-R3b twice-run byte-identical (**in opt-in `--deterministic` mode**; the default is decision-determinism) is an **audit / reproducibility** requirement in this domain (a scan cited in an incident must reproduce) — an offline, pinned DB is what makes it hold; a live DB cannot.

### Integration Requirements
- **KEV gate tier — deferral REVERSED 2026-07-15 (D3 → FR36, story 6.4).** osv-scanner does **not** natively supply CISA KEV, so we join it ourselves: a cached CISA KEV feed (US-gov public domain; offline default, opt-in online never silent) enriches each finding (`kev`, `kev_date`) and **gates in v1** (TOML `fail-on-kev`, default true — FR18 default; **TOML-only, no CLI flag** per Story 6.4). The second-data-source/staleness costs the original deferral cited are owned by story 6.4 (cache lifecycle + max-age) and FR36's feed-absence semantics (absent/stale under a KEV policy → `indeterminate`, never a silent no-op). *(EPSS: **v1** per D12 — FIRST.org cached feed, `--min-epss`, story 6.7.)*
...
- **Cardinal rule — asymmetric failure cost.** A missed CVE (false-negative) is catastrophic; a blocked build (false-positive) is costly but recoverable. Bias = **fail-loud, never false-green** (enforced by the Step-4 guards + the mandatory offline staleness warning). *(Historical note: this asymmetry once justified deferring KEV — superseded 2026-07-15/16; KEV and EPSS now gate in v1 with feed-absence → `indeterminate`, FR36.)*
...
> **[SUPERSEDED by D3 (2026-07-15) + D12 (2026-07-16) — retained as history.]** The v1 default gate blocks on **CVSS-critical AND CISA-KEV-listed** (FR18/FR36); `--min-epss` ships v1; hygiene per the DEP001 table. The original Step-5 text deferred the KEV tier post-v1 and made KEV annotation-only — that decision was reversed; see § Functional Requirements sections C/K. Original (historical): blocks on CVSS-critical, warn on high/med/low + all hygiene; KEV deferred, annotation only, never a v1 gate input. *(Supersedes FR5's original "block on critical CVE or KEV-affecting-current" wording for v1.)*
...
`pyforge-warden scan <path>` (default `.`) + global `--version` / `--help` (part of the frozen contract — CI provenance + M1 discovery). One verb; no interactive subcommands; **no prompts ever** (even `--bypass` takes `--reason` inline). Room for `report` / `explain` subcommands post-v1.

### Command Structure
- **Verb:** `scan <path>`.
- **Gate/policy:** `--fail-on=<severity>` (default `critical`+KEV, FR18), `--warn-only` (report + exit 0 — the sanctioned J6 adoption on-ramp), `--fail-under-coverage=<pct>` (default off — the only coverage-floor CLI; skipped → `indeterminate` already covers the old “require full coverage” residual under the triad), `--allow-empty` (downgrades the no-manifest guard for monorepo sweeps). *(Do **not** ship `--require-full-coverage` — Story 3.3 Never; PRD formerly advertised it.)*
- **Axis gates (D12, v1 — flag-activated):** security TOML `fail-on-kev` (default true, **no CLI flag** — Story 6.4 Design Notes) + CLI `--min-epss <0..1>`; license `--allow-licenses <SPDX,…>` / `--deny-licenses <SPDX,…>`; currency `--max-lag <n>` / `--require-lts` / `--fail-on-eol` (freshness-preconditioned). Setting any axis flag / TOML gate key flips that axis to gating (FR33/FR35/FR36).
- **Adoption & remediation (D12, v1):** `--baseline <file>` + `--baseline-emit` (FR39 — read-only, stdout stanza); `--open-fix-prs` + `--fix-prs-dry-run` (FR40 — opt-in post-verdict actuator, env credentials).
- **Self-check:** `scan --doctor` (D8 — flag on the one verb; exit 0 healthy / 2 typed error, never 1).
...
- **Config file (decided — D3):** `[tool.pyforge-warden]` in **both** `pyproject.toml` (PyPI-world home, beside `[tool.deptry]`) **and** `pixi.toml` (the conda/pixi bridge's natural home — P1/beachhead *is* the pixi user). *A dedicated `.pyforge-warden.toml` may be added post-v1 if the TOML-section approach proves limiting.*
...
- **Respect existing config:** `[tool.deptry]` ignores honored; the waiver file `.warden-waivers.yaml` is read (never written into the repo tree by a scan, NFR3).
...
- **D3 *(CLI-step decision label — distinct from the spec's D3 KEV decision)* — dual-config at v1:** keep both `pyproject.toml` + `pixi.toml` config.
...
**Core journeys supported in v1:** J1 (conda/pixi wedge), J2 (PyPI unification), J3 (fleet ops), J4 (bypass loop), J5 (machine contract), J6 (adoption on-ramp), J8 (corpus gate), J9 (verdict composition), J10 (workstation/P8). **Personas served:** P1, P2, P3, P4, M1, P7, P8.

**Must-Have Capabilities (v1):**
- **E1 — manifest bridge:** `recipe.yaml` / `meta.yaml` / `environment.yml` / `pixi.toml` (stdlib-lean, two-pass eval, split hygiene/vuln coverage, name-only+marked degrade) + PyPI delegation to the engines' native parsers.
...
- **E4 — honest report + gate:** schema `ComplianceReport` (status / severity / `schema_version` / split coverage / `error_kind`); **verdict-composition** (J9 precedence lattice); **severity gate** (FR18 — CVSS-critical + KEV default; `--min-epss` v1); **typed errors** (FR10 + FR-NEW-F expanded engine taxonomy); **CycloneDX 1.6 SBOM** (FR8, emitted `experimental`); **false-green guards** (empty-extraction, no-manifest fail-closed, offline-DB-skipped, crashed-engine).
...
- **E6 — multi-axis expansion (D12, 2026-07-16):** license axis + flag-activated gate (FR32/FR33); currency axis + flag-activated gate, freshness-preconditioned (FR34/FR35); KEV + EPSS enrichment and gates with feed-absence → `indeterminate` (FR36); unconfigured-axis `warn` visibility (FR37); the one versioned schema amendment (FR38); **baseline & grandfathering** (FR39); **fix-PR actuator** (FR40); `scan --doctor` (D8); internal JFrog distribution behind the engine version-range gate (NFR-C1).
...
**Nice-to-Have / Deferred to post-v1** *(as re-tiered by D12 — KEV, EPSS, the axis gates, baseline & grandfathering, and fix PRs moved INTO v1 and left this list):* SARIF output (`--format` value-space reserved) · cf_atlas promotion (FR-16/18) · P6 + J7 compliance-audit retrieval (enterprise) · waiver-at-scale / renewal · per-section (dev/test) severity policy · coverage-floor *default* policy (the `--fail-under-coverage` knob ships v1, default off) · deprecation lifecycle machinery · SBOM schema freeze · alternate hygiene backends (fawltydeps) · fleet/multi-manifest parallelism (engines are parallel in v1) · better conda↔PyPI reconciliation.
...
> **⚠️ Requirement-ID conventions (read first).** **FR1–FR40 in this section are canonical and binding** (FR32–FR38 added by the 2026-07-15 story-0.1 replan; FR39–FR40 + the FR33/FR35/FR36/FR37 re-baseline added 2026-07-16 per D12). Any `FRn` cited *earlier* in this document (frontmatter `prioritizedRefinements`, journeys, domain, CLI, scoping) is an **intake-spec working label** from an earlier drafting step and is **superseded** by this block. Crosswalk of the colliding labels:
>
...
> | FR-K1 / FR-L1 / FR-L2 / FR-C1 / FR-C2 *(spec working labels, 2026-07-15)* | KEV+EPSS enrichment+gates / license axis / license gate / currency axis / currency gate | **FR36 / FR32 / FR33 / FR34 / FR35** |
> | FR-B1 / FR-A1 *(spec working labels, 2026-07-16 — D12)* | baseline & grandfathering / fix-PR actuator | **FR39 / FR40** |
>
...
- **FR9-was-KEV — cut REVERSED 2026-07-15 (D3 → FR36):** the original cut ("a KEV annotation with no gate delivers no decision value while pulling in the deferred CISA-KEV data source") is answered on **both** clauses: v1 ships the annotation **with a gate** (TOML `fail-on-kev`, in the FR18 default) **and** the data-source cost is owned (feed provisioning/cache/max-age = story 6.4; absence semantics in FR36 — an absent feed under a KEV policy is `indeterminate`, never a silent no-op).
- **Requirements deferred to post-v1 (not v1 FRs — all tiered v1.x or vision in the spec's release map):** SARIF output (v1.x) · cf_atlas promotion (FR-16/18) · compliance-audit retrieval (P6/J7) · waiver-at-scale renewal · per-section severity policy · coverage-floor *default* tuning (the `--fail-under-coverage` knob ships v1, default off; `--require-full-coverage` was never shipped) · alternate hygiene backends (v1.x). *(Removed from this list by D12, 2026-07-16 — pulled into v1: the baseline & grandfathering ratchet (FR39), EPSS (FR36), the axis gates (FR33/FR35), automated fix PRs (FR40); KEV was pulled in 2026-07-15.)*
...
- **NFR-S2 (no silent egress):** the **orchestrator's own process opens no socket** (socket-guard test); all network is confined to the named engine subprocesses; air-gapped mode passes explicit `--offline`/local-DB flags (osv-scanner defaults *online* — an assertion, not a hope). KEV, **FIRST EPSS**, and endoflife.date are **cached feeds under the same posture** (offline default; opt-in online, never silent — FR36/FR34). The FR40 fix-PR actuator is the **sole exception with forge-API egress** — opt-in flag, env credentials, post-verdict, never touching the scanned tree.
...
- **NFR-S9 (bundled-data max-age — added 2026-07-15):** bundled data files (LTS registry, conda→pypi map) carry a build-time `snapshot_at`, and every verdict derived from them carries `max_age_ok` against a configurable max-age (default 180 days); a stale bundled registry can never silently report `supported` — stale + gated (FR35, v1 flag-activated) → `indeterminate` (the bundled twin of NFR-S8/FR12).
...
- **NFR-P-concurrency:** all v1 axes run **in parallel** (independent engines/producers; was "the two engines" — widened 2026-07-15); the tool holds **no shared mutable state / no global lock**; per-invocation cost is **O(project), independent of fleet size** *(absorbs the deleted standalone "fleet-scale" NFR — fleet orchestration is the CI platform's job)*.
...
## Glossary (added 2026-07-16 — reviewer gate)

- **Axis** — one assessment lane over the resolved inventory (v1: hygiene, vulnerability/security, license, currency). "**Dimension**" in older text is a synonym for axis — normalize to axis.
- **Rung** — one entry of the frozen 7-value verdict lattice (`error > policy-violation > indeterminate > warn > bypassed > clean > not-applicable`). Producers *feed* rungs; only `verdict.py` *projects*.
- **Status** — the single lattice value a run composes to; **verdict** = status + its driver; **exit code** = the separately-derived projection ({0,1,2,130}).
- **Coverage** — per-axis honesty denominators (`manifests_found/parsed`, `deps_total/assessed`, `resolution_depth`); split per axis, never blended.
- **Gating (per-axis bool)** — whether an axis's verdicts can block; hygiene/security gate by default, license/currency gate when their policy flags are configured (D12 flag-activation).
- **Finding vs component** — a finding is one actionable defect (three stable ID families); a component is one resolved dependency in the inventory.
- **Withheld / indeterminate** — a component excluded from matching for a stated reason; always visible, never silently dropped.
- **Baseline vs waiver** — both suppress by stable finding-ID with expiry; a waiver is per-finding risk acceptance (FR24), the baseline grandfathers existing debt wholesale (FR39); waiver wins where both match.

## Architecture Open Questions (consolidated)

*Re-stamped 2026-07-16 (bmad-prd reviewer gate): the architecture phase is COMPLETE — items it decided moved to "Resolved v1 assumptions" with their deciding reference; only genuinely-open items remain below. Originally the single source of truth for the architecture phase; consolidates the open-question fragments that were scattered across the Domain, CLI-Tool, and Functional-Requirements sections. Split into **resolved v1 assumptions** (recorded — do not relitigate) and **genuinely-open** decisions the architect must make. Ordered by how much of the v1 build sits on top of them.*

### Resolved v1 assumptions (recorded — do not relitigate)
- **Coverage-floor default = OFF** — `--fail-under-coverage` ships v1 but default off (protects first-run adoption). `--require-full-coverage` is **not** a shipped CLI flag.
...
- **KEV + EPSS gate in v1** (D3 reversal 2026-07-15 + D12 2026-07-16): cached CISA-KEV + FIRST-EPSS feeds (stories 6.4/6.7); absent/stale feed under an active policy → `indeterminate` (FR36). *(Replaces the original "KEV deferred, annotation-only" assumption.)*

### Resolved by the completed architecture phase (2026-07-11/12) + D12 — do not relitigate
- **[Gap A — RESOLVED] deptry-severity → verdict mapping:** hygiene computes status in the SAME lattice with a per-rule ceiling — **DEP001 blocks by default** (mapping-confidence-gated), DEP002–005 warn (FR18; architecture § GAP A; owner re-confirmed 2026-07-15). *Shipped status, precisely: story 1.3's `hygiene_rung` default has DEP001–005 all `warn` — DEP001's block activates with story 2.1's mapping-confidence gate (the decision stands; its activation is 2.1-owned).*
- **[Gap B — RESOLVED] report ↔ SBOM model:** one `ResolvedInventory`, two read-only projections (`report.py` + `sbom.py`); findings and SBOM components key off the same frozen inventory (architecture § GAP B; shipped in story 1.1).
- **[Gap C — RESOLVED] osv input + offline DB:** synthesized version-pinned `requirements.txt` from lock data; name-only → withheld-and-marked `vulnerability-indeterminate` (FR13, `pypi_identity` predicate + bundled map); offline-DB mechanism decided by the story-1.4 spike (`osv-db-offline-provisioning-decision.md`).
- **Multi-manifest selection precedence (FR1) — RESOLVED:** union coverage; every discovered manifest is scanned and reported per-manifest (reconciliation callout #5; story 1.9).
- **Waiver write-target = stdout — RESOLVED:** the `--bypass` stanza is emitted for the human to commit, never written into the repo tree by the tool (FR24 / NFR-S4; reconciles D1's read/write path; moved out of open item 8, 2026-07-16).

### Open — architect must decide (blocking items first)
1. **[superseded — see Gap A RESOLVED above]** *(AUD-WARDEN-030: open-item #1 collapsed; historical “warn-axis” recommendation was decided the other way by FR18.)*
2. **[superseded — see Gap B resolution above]** *(historical text follows)* ComplianceReport ↔ CycloneDX shared-inventory model. One artifact or two (CLI implies two: `--output` + `--sbom-output`); embedded / referenced / independent; do findings **and** SBOM components key off one **resolved-inventory** object? ("BOM component count == resolved-inventory count" implies a shared object that is never named as a first-class model.) Define it — it is the spine of E4.
3. **[superseded — see Gap C resolution above]** *(historical text follows)* osv input contract + offline-DB provisioning. What osv-scanner does with a **name-only / range** dependency (fed-and-dropped vs withheld-and-marked `vulnerability-indeterminate`, FR13); where the offline OSV DB lives; how it is provisioned; the snapshot-timestamp source; and the checksum / trust anchor (NFR-S8). *(OD2 — the #1 complexity hotspot; fully deferred today.)*
4. **[superseded — union coverage, see above]** ~~Multi-manifest selection precedence (FR1)~~.

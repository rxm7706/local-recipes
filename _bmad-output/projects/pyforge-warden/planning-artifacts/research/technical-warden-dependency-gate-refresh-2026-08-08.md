...
1. **`engines.py`'s `_engine_env()` subprocess-normalization seam** — argv-list-only, temp-file output outside the scanned tree, typed `ErrorRecord` taxonomy, bounded timeout, exit-code-as-content-vs-operational discrimination. This is the same problem class as Marshal's marshal:AD-69-style subprocess guards [citation corrected 2026-09-08: this originally named Marshal's fifth decision, which is actually "The journal is the single source of run truth"; its subprocess seam is the one cited here, "The subprocess seam is one guard, and Marshal is its first subject". The STATION was stated deliberately -- this section names Marshal and Doctor separately elsewhere -- and the section was written blind to Marshal's own research, so the number was the loose half, not the station]; Warden's version is the most battle-tested in the fleet (every deptry/osv call for 31 stories plus the strace-verified corpus).

...
- source_spec: `_bmad-output/projects/pyforge-warden/implementation-artifacts/spec-6-8-baseline-grandfathering.md`
  ...
  verified: 2026-07-30 — CONFIRMED STILL OPEN, decisively — the tree is unchanged. `planning-artifacts/architecture/architecture-pyforge-warden-2026-07-14/ARCHITECTURE-SPINE.md:265-300` still lists `waiver.py` with the exact stale comment this entry quoted ('# FR24-26 — .yaml read (safe_load) + --bypass stanza (safe_dump)', no FR39 baseline mention) and carries NO entry for `license.py`, `currency.py`, `feeds.py` or `actuator.py`. The entry's distinction also still holds: those modules DO appear elsewhere in the same file (the 'Module structure' list — license.py ×1, currency.py ×1, feeds.py ×6, actuator.py ×2), just never in the Project Structure tree. Note `:238` declares that tree 'authoritative' where the two disagree, so the stale copy is the one that wins.
...
- source_spec: `planning-artifacts/specs/spec-7-2-the-eligibility-union-carries-its-provenance.md`
  ...
  verified: 2026-09-09 — still-open, now BOUND — re-read against live code at HEAD fe4025ea90 (fleet readiness decision batch, row warden-B8). The upstream policy question this entry sits under is ANSWERED: `eligibility.py:113-121` defaults `required_authority_sources=None` to unanimous consensus over every source present in the evidence tuple, and `eligibility.py:128-131` records that shipping no CLI flag or TOML key for it was deliberate. The operator ACCEPTED that deferral 2026-09-09 and it is now a named residual on `spec-package-inventory-eligibility` (Spec moved `ready` → `in-progress`) rather than an open Spec question. THIS ROW STAYS OPEN and is unchanged in substance: an explicit caller-supplied override still leaves no trace in the returned data, so a downstream consumer holding only the `EligibilityResult` tuples cannot always re-derive status from provenance alone. Its close is now BOUND to the same story as the config surface — the config knob and the missing CLI front door are minted together, because a settable policy no CLI can set is unreachable (`grep -n eligib src/shared/packages/pyforge-warden/src/pyforge/warden/cli.py` still returns nothing). Story 7.3, named here as "the natural point to decide", shipped without taking it; the decision point moves to that combined CLI+config story.
...
### DW-FU-11-2: Whether the suite:AD-9 roster lacking a `tea` entry entirely (never provisioned via `steward provision --module tea`) should make the advisory scanner loudly refuse, distinct from today's silent fail-open when only the binary is absent from PATH.

- source_spec: `planning-artifacts/specs/spec-11-2-tea-test-review-is-a-warden-advisory-finding.md`
  summary: Whether the suite:AD-9 roster lacking a `tea` entry entirely (never provisioned via `steward provision --module tea`) should make the advisory scanner loudly refuse, distinct from today's silent fail-open when only the binary is absent from PATH.
  evidence: The lifecycle architecture spine (ARCHITECTURE-SPINE.md suite:AD-10) says "31.1 / 11.2 refuse when the suite:AD-9 roster lacks tea," and its cited template (steward Story 14.9's `--no-shims` flag) implements "refuse" as a real runtime CLI refusal, not a one-time dispatch-time check -- weighing toward a genuine runtime-behavior gap in this diff (the scanner and doctor check both treat "roster lacks tea" and "binary absent from PATH" identically, via fail-open). But Story 11.2's own Given/When/Then in epics.md states plainly "the scanner is fail-open when TEA is absent," with no roster/binary distinction -- the more specific, story-level acceptance text this spec was built from. Resolving requires an operator/architect call on which text governs; shipping either reading here without that call risks contradicting the other. If suite:AD-10's runtime-refusal reading is correct, the real gap is medium (an architecture-mandated distinction never implemented; does not affect suite:AD-4's no-competing-verdict invariant either way).
  location: src/shared/packages/pyforge-warden/src/pyforge/warden/tea_advisory.py:TeaAdvisoryScanPlugin._contribute
  origin: spec-deferred 22ec503e4880 — ingested from spec frontmatter `deferred:` (hand-driven build-auto; marshal Story 25.6)
  severity: medium (unverified)
  promoted: 2026-09-07 — ingested from spec frontmatter by scripts/deferred_work_intake.py
  status: open

  verified: 2026-09-07 — resolved — suite:AD-10's runtime-refusal reading governs: `run_tea_test_review` (`tea_advisory.py`) now checks the suite:AD-9 roster (`_bmad/custom/config.toml [modules.tea]`) FIRST and raises a new `TeaRosterMissingError` when `tea` has no roster entry at all, regardless of whether a same-named binary happens to be reachable on PATH (a leaked/ambient copy is exactly the ungoverned state suite:AD-10 refuses, not a reason to proceed). `TeaAdvisoryScanPlugin._contribute` re-raises that one exception type instead of swallowing it (every other runner/parse/binary problem still fails open, unchanged); `cli.py`'s `_run_scan` catches it around `invoke_pr_gate` and records it via `_record_error(kind=ErrorKind.CONFIG_VALIDATION, owner="tea-test-review", ...)` — the same treatment `select_scanner_plugins`'s `PluginError` already gets, a real ERROR rung that moves the exit code. This does NOT violate suite:AD-4 ("advisory lenses never gate"): suite:AD-4 governs SCORED findings about the code under scan, not an unmet tool prerequisite — a config-validation refusal is a different kind of signal, with existing precedent one call site up in the same function. A roster entry that DOES name `tea` but whose binary is merely absent from THIS process's PATH remains fail-open exactly as before (unchanged, existing tests updated to set up a roster fixture so they exercise that case specifically rather than conflating it with the no-roster case). Chose fail-closed over fail-open per this repo's own standing precedent that a compliance-adjacent tool silently reporting "fine" when it cannot actually verify anything is a real problem, not acceptable default behavior (`feedback_unauthenticated_github_probes_fail_open`) — a scanner explicitly asked to run a check whose prerequisite was never provisioned must say so loudly, not degrade indistinguishably from a transient PATH hiccup. `epics.md` Story 11.2's Given/When/Then updated to state the roster/binary distinction explicitly (it previously read "fail-open when TEA is absent" with no distinction, which is what created this conflict against suite:AD-10 in the first place); suite:AD-10 itself needed no change — its text already matched the resolution. New/updated tests: `test_roster_missing_raises_when_scanner_would_run`, `test_roster_present_but_empty_modules_table_still_refuses`, `test_roster_missing_never_calls_runner`, `test_enabled_plugin_roster_missing_raises_not_fail_open`, `test_full_scan_with_roster_missing_refuses_not_a_silent_pass` (all new); `test_fail_open_when_binary_absent`, `test_absent_binary_never_calls_runner`, `test_enabled_plugin_absent_binary_contributes_nothing` (updated to write a `[modules.tea]` roster fixture first, so they exercise "roster present, binary absent" specifically). Full suite: `pixi run -e pyforge-warden pyforge-warden-test` — 2114 passed, 11 deselected (was 2109 passed, 11 deselected before this change; +5 net new tests). `python -m pyforge.doctor.sources spec-surface` clean after `--write-baseline --spec pyforge-warden/spec-11-2-tea-test-review-is-a-warden-advisory-finding` (scoped).

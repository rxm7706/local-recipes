...
- **Severity taxonomy** — **CVSS** (v3.1 / v4.0) is the **v1 gate signal**; **CVE** identifiers; **CISA KEV** gates in v1 (`--fail-on-kev`, FR36); **EPSS** `--min-epss` also v1 per D12 (see Integration Requirements).
...
### Command Structure
- **Verb:** `scan <path>`.
- **Gate/policy:** `--fail-on=<severity>` (default `critical`+KEV, FR18), `--warn-only` (report + exit 0 — the sanctioned J6 adoption on-ramp), `--fail-under-coverage=<pct>` (default off), `--require-full-coverage` (residual role post-triad: under `--warn-only`, re-escalates skipped/partial-coverage axes to non-zero — the default path already routes skipped → `indeterminate`), `--allow-empty` (downgrades the no-manifest guard for monorepo sweeps).
- **Axis gates (D12, v1 — flag-activated):** security `--fail-on-kev` (in the default) + `--min-epss <0..1>`; license `--allow-licenses <SPDX,…>` / `--deny-licenses <SPDX,…>`; currency `--max-lag <n>` / `--require-lts` / `--fail-on-eol` (freshness-preconditioned). Setting any axis flag flips that axis to gating (FR33/FR35/FR36).
...
- **FR9-was-KEV — cut REVERSED 2026-07-15 (D3 → FR36):** the original cut ("a KEV annotation with no gate delivers no decision value while pulling in the deferred CISA-KEV data source") is answered on **both** clauses: v1 ships the annotation **with a gate** (`--fail-on-kev`, in the FR18 default) **and** the data-source cost is owned (feed provisioning/cache/max-age = story 6.4; absence semantics in FR36 — an absent feed under a KEV policy is `indeterminate`, never a silent no-op).
- **Requirements deferred to post-v1 (not v1 FRs — all tiered v1.x or vision in the spec's release map):** SARIF output (v1.x) · cf_atlas promotion (FR-16/18) · compliance-audit retrieval (P6/J7) · waiver-at-scale renewal · per-section severity policy · coverage-floor *default* tuning (the `--fail-under-coverage` / `--require-full-coverage` flags ship v1, off) · alternate hygiene backends (v1.x). *(Removed from this list by D12, 2026-07-16 — pulled into v1: the baseline & grandfathering ratchet (FR39), EPSS (FR36), the axis gates (FR33/FR35), automated fix PRs (FR40); KEV was pulled in 2026-07-15.)*

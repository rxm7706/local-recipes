---
title: 'pyforge-warden — Retrospective (2026-09-20: the fleet lint / type gate lands)'
...
evidence:
  - 'PR #1553 (2026-09-20): src/shared/packages/pyforge-warden/pyproject.toml, every .py under src/ and tests/'
  - 'pixi run -e pyforge-guild lint-types — exit 0 on the landed tree'
---

## What the gate found on its first run

- **Type baseline:** 7 modules baselined (5 codes, incl. one `call-arg`). Every entry is `[[tool.mypy.overrides]]` per module with exactly the error codes that module trips — never `ignore_errors` — so the gate is live for every other module and code. Tighten or drop an entry whenever its module is touched; the baseline is a ratchet, not a floor.
- **Lint:** no hand fixes were needed; the `call-arg` finding is a real argument-shape mismatch to fix when that module is next touched.

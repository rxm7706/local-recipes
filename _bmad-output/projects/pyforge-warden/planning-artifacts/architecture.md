...
- **Baseline & grandfathering (FR39, story 6.8 — D12; module boundary corrected 2026-07-24, implementation review):** `waiver.py` reads the committed, schema-validated `.warden-baseline.yaml` (not a separate `baseline.py` — `feeds.py`'s own shipped precedent, one module housing KEV+EPSS+endoflife rather than one file per feed, is the consolidation this mirrors. The originally planned two-file split was avoidable-but-gratuitous, not impossible: `apply_waivers`'s single matching loop needs the `BaselineEntry` type FROM the new module while that module imports matching primitives (`_is_finding_family_id`/`_is_expired`/`_parse_timestamp`) FROM `waiver.py` — resolvable one-directionally via a `TYPE_CHECKING`-guarded type-only import, since `waiver.py` already uses `from __future__ import annotations`, but only at the cost of splitting one engine's types across two files for zero behavioral gain). Suppression is keyed on the stable finding-ID grammar (the waiver key); expiry = waiver semantics; applied entries echo in the report; the gate blocks NEW findings only. Read-only (NFR-R3a); `--baseline-emit` prints, never writes.
...
- **DEP002 (unused) / DEP003 (transitive) / DEP004 (misplaced) / DEP005 (stdlib-dependency) → `warn`** by default (false-positive-prone on conda recipes that legitimately carry deps deptry can't see).
...
pyforge/warden/
  ...
  waiver.py         # .yaml read (safe_load) + --bypass stanza emit (safe_dump); + FR39 baseline & grandfathering (6.8) — one suppression engine, two file formats, no separate baseline.py (mirrors feeds.py's own consolidation)   [2026-07-16; corrected 2026-07-24]
...
- **One suppression engine (F8, 2026-07-16; module boundary corrected 2026-07-24):** matching-by-finding-ID + expiry is implemented ONCE — `waiver.py`'s `apply_waivers`, extended (never re-implemented) with a `baseline` parameter in story 6.8, both file formats and the matching core living in `waiver.py` itself (not a separate `baseline.py` — see the Baseline & grandfathering bullet above for why); the tie-break (waiver wins where both match; echoed once, via the 6.1 rung-discriminator) is decided there; pipeline position = policy evaluation, pre-verdict.

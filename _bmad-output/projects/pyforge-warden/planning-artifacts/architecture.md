...
- **Baseline & grandfathering (FR39, story 6.8 — D12; module boundary corrected 2026-07-24, implementation review):** `waiver.py` reads the committed, schema-validated `.warden-baseline.yaml` (not a separate `baseline.py` — the originally planned two-file split would need `apply_waivers`'s single matching loop to import a `BaselineEntry` type FROM the new module while that module imports matching primitives (`_is_finding_family_id`/`_is_expired`/`_parse_timestamp`) FROM `waiver.py`, a circular dependency; `feeds.py`'s own shipped precedent — one module housing KEV+EPSS+endoflife rather than one file per feed — is the consolidation this mirrors instead). Suppression is keyed on the stable finding-ID grammar (the waiver key); expiry = waiver semantics; applied entries echo in the report; the gate blocks NEW findings only. Read-only (NFR-R3a); `--baseline-emit` prints, never writes.
...
pyforge/warden/
  ...
  waiver.py         # .yaml read (safe_load) + --bypass stanza emit (safe_dump); + FR39 baseline & grandfathering (6.8) — one suppression engine, two file formats, no separate baseline.py (circular-import-avoiding, mirrors feeds.py's own consolidation)   [2026-07-16; corrected 2026-07-24]

...
| **CAP-10** | **Fact ledgers visible to Marshal retrieve.** Today `--scope` admits only `_bmad-output/projects/<slug>/`, so `presentations/<slug>/facts.yaml` is invisible. | Herald owns derive; Scribe compiles. Needs a citation rule or `scribe recall --facts`, not a scope leak. | Decks and dispatch citing the same numbers |
...
No Epic 8 story is minted for CAP-6–CAP-14. CAP-1–CAP-5 + Stories 8.3–8.6 stay the build.

...
| `count-stale` / structural drift (plan) | `PRD.md`, `epics-regenerable-factory.md` | **`bmad-correct-course`** → **`bmad-edit-prd`** / **`bmad-create-epics-and-stories`** when content is structurally stale (new epics/stories for net-new capabilities — not a number swap on `source_pin`) |

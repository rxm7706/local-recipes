...
project_name: pyforge-genesis
epicCount: 6
storyCount: 36
status: complete
---

# pyforge-genesis — Epic Breakdown
...
**Critical-path story is S-1.6 (Spike-0, Copier API fit)** — a failure there changes AD-02
or promotes a bespoke materializer, so it gates E4 entirely.
...
in E6, but NFR-R4 (guard at the primitive) is E1 by necessity.

### Architecture Decisions covered

All 15 ADs (AD-01–AD-15) flow into specific stories. The 12 conflict-prevention patterns
(P-01–P-12) are implemented throughout E1–E5 and enforced as executable tests by S-6.4.

### FR / Story Coverage Matrix

| FR Range | Capability | Owning Story/Stories |
|---|---|---|
| FR1–FR3, FR5 | Manifest as data, five classes, deferred state | S-1.4, S-1.5 |
| FR4 | Coverage check (no unclassified artifact) | S-3.5 |
| FR6 | Never-write path set declared | S-1.4, S-1.5 |
| FR7–FR13 | `genesis init` | S-4.7 |
| FR14–FR19 | `genesis adopt` detect→plan→apply, idempotent | S-4.6, S-3.6, S-6.5 |
| FR15–FR16 | Classification incl. `present-legacy` | S-3.2, S-3.4 |
| FR17 | Machine-readable plan artifact | S-3.6 |
| FR20–FR22 | Preconditions, refusals, skips | S-4.4 |
| FR23–FR28 | `genesis check` | S-4.5, S-3.1 |
| FR29 | Two-phase update | S-5.4 |
| FR30 | Referenced-dependency verification | S-5.5 |
| FR31–FR32 | Migration ordering, applied-once, seeded opt-in | S-5.3 |
| FR33–FR34 | Regenerate managed / recompute derived / replace regions | S-5.4, S-2.3, S-5.1 |
| FR35 | Never-write enforcement on update | S-1.3, S-6.4 |
| FR36 | `--force` → `run_recopy` | S-4.1, S-5.4 |
| FR37–FR42 | State file | S-4.2 |
| FR43–FR48 | Managed regions | S-2.1, S-2.2, S-2.3, S-2.4, S-2.5 |
| FR49–FR52 | Agent adapter fan-out | S-5.1 |
| FR53–FR54 | In-package templates + `--template` override | S-1.5, S-4.1 |
| FR55–FR56 | Copier public API only; `--unsafe` gate | S-4.1 |
| FR57 | Distribution (conda + wheel/sdist) | S-6.1 |
| FR58–FR59 | `--json` / `--quiet` / `--dry-run` | S-6.5 |
| FR60 | `genesis version` (both versions) | S-5.6 |
| FR61 | Distinct documented exit codes | S-1.2, S-6.5 |
| FR62 | `genesis explain <artifact>` | S-5.6 |

### NFR / Story Coverage Matrix

| NFR | Subject | Owning Story/Stories |
|---|---|---|
| NFR-R1 | No partial application | S-4.3 |
| NFR-R2 | Git is undo (clean worktree) | S-4.4 |
| NFR-R3 | No conflict markers ever | S-2.3 |
| NFR-R4 | Guard at the write primitive | S-1.3, S-6.4 |
| NFR-A1, NFR-A2 | Air-gapped, zero egress | S-6.3 |
| NFR-P1–P3 | check < 5 s, adopt --dry-run < 10 s, init < 5 min | S-6.5 |
| NFR-C1–C4 | Python ≥3.12, copier range-pin, platforms, namespace share | S-1.1, S-6.1 |
| NFR-S1 | No untrusted execution by default | S-4.1 |
| NFR-S2 | No credential handling | S-6.3 (no network stack) |
| NFR-S3 | Templates validated against manifest | S-4.1, S-4.3 |
| NFR-M1 | Manifest is the single source of truth | S-1.4, S-5.1 |
| NFR-M2 | Oracle in Genesis's own CI | S-6.2 |
| NFR-M3 | Every finding documented with a remedy | S-3.1, S-6.6 |
| NFR-O1 | Machine-readable plans and reports | S-3.6, S-6.5 |

### Success-criteria ownership

| SC | Owning story |
|---|---|
| SC-01 (master switch) | S-5.4 + S-5.7-equivalent coverage inside S-5.3/S-5.4 |
| SC-02 (empty-plan oracle) | **S-6.2** |
| SC-03 (adopt idempotent) | S-6.5 |
| SC-04 (refuse on hand-edited region) | S-4.4 |
| SC-05 (refuse on dirty worktree) | S-4.4 |
| SC-06 (offline, zero network) | S-6.3 |
| SC-07 (breaking model change absorbed) | S-5.3 |
| SC-08 (update cannot write Tier-0/2) | S-6.4 |
| SC-09 (init < 5 min) | S-6.5 |
| SC-10 (100% manifest coverage) | S-3.5 |

## Epic List

| Epic | Title | Story Count | Effort Sum |
|---|---|---|---|
| E1 | Foundation & the Write Guard | 6 | ~7 days |
| E2 | The Managed-Region Engine | 5 | ~8 days |
| E3 | Detect & Plan | 6 | ~9 days |
| E4 | Materialize & the Core Verbs | 7 | ~12 days |
| E5 | Derive, Migrate & Update | 6 | ~10 days |
| E6 | Packaging, Oracle & Hardening | 6 | ~8 days |
| **Total** | | **36** | **~54 days ≈ 11 weeks (single-builder, focused)** |

---

## Epic 1: Foundation & the Write Guard
...
### Story 1.1: Package skeleton as a pixi workspace member
...
> lands in S-6.1; this story adds only what is needed to import and test locally.

### Story 1.2: Error taxonomy and exit codes
...
**Type:** foundation • **Effort:** XS • **Deps:** S-1.1 • **FR/AD:** FR61, P-10
...
### Story 1.3: The `fs` write primitive and the never-write guard
...
**Type:** foundation • **Effort:** M • **Deps:** S-1.2 • **FR/AD/P:** FR6, FR35, AD-11,
NFR-R4, P-01
...
### Story 1.4: Manifest schema, loader, and model-version ranges
...
**Type:** foundation • **Effort:** M • **Deps:** S-1.2 • **FR/AD/P:** FR1, FR2, FR3, FR5,
...
**And** the manifest declares the `never_write[]` path set consumed by S-1.3
...
### Story 1.5: The V1 extraction manifest (the model, as data)
...
**Type:** content • **Effort:** L • **Deps:** S-1.4 • **FR/AD:** FR1, FR6, FR53, PRD
§ Extraction Manifest
...
**And** the manifest loads clean and passes the coverage check from S-3.5
**And** initial `model_version` is `1.0.0`

### Story 1.6: Spike-0 — Copier API fit (CRITICAL GATE)

As the Genesis architect,
I want the five load-bearing Copier behaviors proven on 9.17 before E4 is built,
So that a wrong assumption changes the design now rather than after seven stories depend on it.

**Type:** spike • **Effort:** S • **Deps:** S-1.1 • **FR/AD:** AD-02, AD-04, A-04, FR55
...
`correct-course` before E4 begins
**And** the spike code is discarded — it is not shipped

---

## Epic 2: The Managed-Region Engine
...
### Story 2.1: Marker grammar and the per-format registry
...
**Type:** foundation • **Effort:** S • **Deps:** S-1.4 • **FR/AD:** FR43, FR45, AD-03
...
registered format
**And** an artifact declaring a format not in the registry is a manifest load error (S-1.4)

### Story 2.2: Region parser — span discovery, nesting rejection, fence awareness
...
**Type:** foundation • **Effort:** M • **Deps:** S-2.1 • **FR/AD/P:** FR48, AD-03, P-06
...
### Story 2.3: Span substitution — the update primitive
...
**Type:** foundation • **Effort:** M • **Deps:** S-2.2, S-1.3 • **FR/AD/P:** FR44, NFR-R3,
P-06, P-01
...
### Story 2.4: Anchor resolution and region insertion
...
**Type:** feature • **Effort:** M • **Deps:** S-2.2, S-2.3 • **FR/AD:** FR46, AD-06
...
### Story 2.5: Marker deletion as a sanctioned opt-out
...
**Type:** feature • **Effort:** S • **Deps:** S-2.4, S-4.2 • **FR/AD:** FR47, AD-08
...
epic plus a renderer, so E3 completing means the product's read-side is done.

### Story 3.1: Findings model — severity, types, remedies
...
**Type:** foundation • **Effort:** S • **Deps:** S-1.2 • **FR/AD/P:** FR25, AD-04, NFR-M3, P-10
...
### Story 3.2: Repo inventory walker and artifact classification
...
**Type:** feature • **Effort:** M • **Deps:** S-1.4, S-2.2, S-3.1 • **FR/AD/P:** FR15, P-03
...
### Story 3.3: Content hashing for managed files and regions
...
**Type:** feature • **Effort:** S • **Deps:** S-3.2, S-2.2 • **FR/AD/P:** FR41, FR21, P-07
...
**And** region hashing covers the body only (consistent with S-2.1)
...
### Story 3.4: Legacy convention detection
...
**Type:** feature • **Effort:** S • **Deps:** S-3.2 • **FR/AD:** FR16, AD-09
...
### Story 3.5: Manifest coverage check
...
**Type:** feature • **Effort:** S • **Deps:** S-1.5, S-3.1 • **FR/AD:** FR4, SC-10
...
### Story 3.6: Plan and Action types, repo fingerprint, and the plan builder
...
**Type:** feature • **Effort:** M • **Deps:** S-3.2, S-3.3, S-3.4 • **FR/AD/P:** FR17, AD-07,
NFR-O1, P-04, P-05
...
## Epic 4: Materialize & the Core Verbs
...
(adopt against an empty target). **Gated on S-1.6 (Spike-0).**

### Story 4.1: Copier engine wrapper — the single seam
...
**Type:** foundation • **Effort:** M • **Deps:** S-1.6, S-1.3 • **FR/AD/P:** FR53, FR54, FR55,
...
`Worker` attribute access, no private-module imports (asserted by S-6.4's import test)
...
### Story 4.2: State schema and the atomic store
...
**Type:** foundation • **Effort:** M • **Deps:** S-1.3 • **FR/AD/P:** FR37, FR38, FR39, FR40,
...
### Story 4.3: The apply runner — transactional, guarded
...
**Type:** feature • **Effort:** M • **Deps:** S-4.1, S-4.2, S-2.3, S-3.6 • **FR/AD/P:** FR18,
...
### Story 4.4: Preconditions, refusals, and skips
...
**Type:** feature • **Effort:** S • **Deps:** S-4.3, S-3.3 • **FR/AD:** FR20, FR21, FR22,
NFR-R2, SC-04, SC-05
...
**Type:** feature • **Effort:** L • **Deps:** S-4.3, S-4.4, S-4.5, S-2.4 • **FR/AD:** FR14,
...
**Type:** feature • **Effort:** M • **Deps:** S-4.6 • **FR/AD:** FR7, FR8, FR9, FR10, FR11,
FR12, FR13
...
### Story 5.1: Neutral contract and agent-adapter fan-out
...
**Type:** feature • **Effort:** M • **Deps:** S-1.5, S-2.4 • **FR/AD:** FR49, FR50, FR51, FR52,
AD-13, NFR-M1
...
### Story 5.2: `PROJECTS.md` index and artifact-symlink derivation
...
**Type:** feature • **Effort:** S • **Deps:** S-5.1 • **FR/AD:** FR (generated-derived class),
AD-13
...
### Story 5.3: Migration registry and runner
...
**Type:** feature • **Effort:** M • **Deps:** S-4.3, S-4.2 • **FR/AD/P:** FR31, FR32, AD-12,
SC-07, P-12
...
### Story 5.4: `genesis update` — two-phase
...
**Type:** feature • **Effort:** M • **Deps:** S-5.3, S-5.1, S-2.3 • **FR/AD:** FR29, FR33,
...
**And** an attempted write to the never-write set is a hard error (FR35) — see S-6.4 for the
standing proof
...
### Story 5.5: Referenced-dependency verification and Doctor delegation
...
**Type:** feature • **Effort:** S • **Deps:** S-4.5 • **FR/AD:** FR30, PRD § Boundaries
...
### Story 5.6: `genesis explain` and `genesis version`
...
**Type:** feature • **Effort:** S • **Deps:** S-1.4, S-4.2 • **FR/AD:** FR60, FR62, D1
...
## Epic 6: Packaging, Oracle & Hardening
...
### Story 6.1: Full pixi wiring, distribution, and repo-gate compliance
...
**Type:** infra • **Effort:** M • **Deps:** S-1.1, S-5.6 • **FR/AD:** FR57, AD-14, NFR-C1,
NFR-C2, NFR-C3, D3
...
### Story 6.2: The `local-recipes` empty-plan oracle (CRITICAL)
...
**Type:** test • **Effort:** M • **Deps:** S-4.6, S-5.1, S-5.2 • **FR/AD:** SC-02, NFR-M2,
AD-10
...
### Story 6.3: Offline operation and the egress counter
...
**Type:** test • **Effort:** S • **Deps:** S-4.7, S-4.5 • **FR/AD:** NFR-A1, NFR-A2, NFR-S2,
AD-15, P-09, SC-06
...
### Story 6.4: Pattern meta-tests and the never-write proof
...
**Type:** test • **Effort:** M • **Deps:** S-4.3, S-5.4 • **FR/AD/P:** P-01–P-12, FR35, SC-08,
NFR-R4
...
**And** **P-09** is covered by S-6.3's import test
...
### Story 6.5: CLI contract, idempotence harness, and performance gates
...
**Type:** test • **Effort:** M • **Deps:** S-4.7, S-5.6 • **FR/AD:** FR58, FR59, FR61, NFR-P1,
...
(FR59)
**And** exit codes match S-1.2's taxonomy for every failure mode, asserted case by case (FR61)
...
### Story 6.6: README, adoption guide, and the finding→remedy reference
...
**Type:** docs • **Effort:** S • **Deps:** S-6.5 • **FR/AD:** NFR-M3, D1
...
```
S-1.1 ──┬─→ S-1.2 ──→ S-1.3 (GUARD) ──────────────────────────┐
        │              ↘                                       │
        │               S-1.4 ──→ S-1.5 ──→ S-3.5              │
        └─→ S-1.6 (SPIKE-0 · GATES E4)                         │
                                                               │
S-1.4 ──→ S-2.1 ──→ S-2.2 ──→ S-2.3 ──→ S-2.4 ──→ S-2.5        │
                       ↘                    ↘                  │
S-1.2 ──→ S-3.1 ──→ S-3.2 ──→ S-3.3 ──→ S-3.6 ←────────────────┘
                       ↘  ↘
                        S-3.4 ─────────↗

S-1.6 + S-1.3 ──→ S-4.1 ─┐
S-1.3 ────────→ S-4.2 ───┼─→ S-4.3 ──→ S-4.4 ──→ S-4.5 ──→ S-4.6 ──→ S-4.7
S-3.6 ───────────────────┘                                   ↘
                                                              S-2.5

S-1.5 + S-2.4 ──→ S-5.1 ──→ S-5.2
S-4.3 + S-4.2 ──→ S-5.3 ──→ S-5.4 ←── S-5.1, S-2.3
S-4.5 ──→ S-5.5
S-1.4 + S-4.2 ──→ S-5.6

S-1.1 + S-5.6 ──→ S-6.1
S-4.6 + S-5.1 + S-5.2 ──→ S-6.2 (ORACLE)
S-4.7 + S-4.5 ──→ S-6.3
S-4.3 + S-5.4 ──→ S-6.4
S-4.7 + S-5.6 ──→ S-6.5 ──→ S-6.6
```

**Critical path (single builder, no parallelism):**
S-1.1 (S) → S-1.2 (XS) → S-1.3 (M) → S-1.4 (M) → S-1.5 (L) → S-2.1 (S) → S-2.2 (M) →
S-2.3 (M) → S-2.4 (M) → S-3.2 (M) → S-3.6 (M) → S-4.1 (M) → S-4.3 (M) → S-4.5 (M) →
S-4.6 (L) → S-4.7 (M) → S-5.1 (M) → S-5.3 (M) → S-5.4 (M) → S-6.2 (M) ≈ **38 days**.
Off-critical-path work (S-1.6, S-2.5, S-3.1/3.3/3.4/3.5, S-4.2/4.4, S-5.2/5.5/5.6,
S-6.1/6.3/6.4/6.5/6.6) adds ≈ 16 days ⇒ **~54 days ≈ 11 weeks**.

**S-1.6 (Spike-0) is off the critical path in duration but gates E4 in sequence** — it must
complete before S-4.1 starts, and it is cheap (S), so it should be run in parallel with E1's
back half.
...
|---|---|
| **S-1.3** | The write guard. Everything downstream assumes it. Build it before anything writes. |
| **S-1.6** | Spike-0 — **gates E4**. A failure changes AD-02 or promotes a bespoke materializer. |
| **S-2.3** | The one algorithm that cannot be delegated to Copier; AR-1 and kill criterion K-01 live here. |
| **S-6.2** | The oracle. A non-empty plan that needs special-casing to fix triggers **K-02**. |
| **S-6.4** | Proves SC-08 — the structural guarantee that the update path cannot touch Tier-0/Tier-2. |
...
{
  "status": "complete",
  "epicsFile": "_bmad-output/projects/pyforge-genesis/planning-artifacts/epics.md",
  "epicCount": 6,
  "storyCount": 36,
  "epics": [
    {"id": "E1", "title": "Foundation & the Write Guard", "stories": 6, "effort_days": 7},
    {"id": "E2", "title": "The Managed-Region Engine", "stories": 5, "effort_days": 8},
    {"id": "E3", "title": "Detect & Plan", "stories": 6, "effort_days": 9},
    {"id": "E4", "title": "Materialize & the Core Verbs", "stories": 7, "effort_days": 12},
    {"id": "E5", "title": "Derive, Migrate & Update", "stories": 6, "effort_days": 10},
    {"id": "E6", "title": "Packaging, Oracle & Hardening", "stories": 6, "effort_days": 8}
  ],
  "criticalPath": [
    "S-1.3: fs write primitive + never-write guard",
    "S-1.5: the V1 extraction manifest",
    "S-2.3: span substitution",
    "S-2.4: anchor resolution + insertion",
    "S-3.6: Plan types + fingerprint + builder",
    "S-4.1: Copier engine wrapper",
    "S-4.3: apply runner",
    "S-4.6: genesis adopt",
    "S-5.4: genesis update",
    "S-6.2: local-recipes empty-plan oracle"
  ],
  "criticalPathDays": "~38 days (54 days total V1 with off-path work)",
  "specialStories": {
    "spike0": "S-1.6 (GATES E4)",
    "writeGuard": "S-1.3 (foundation for every write)",
    "riskiestAlgorithm": "S-2.3 (region substitution; kill criterion K-01)",
    "oracle": "S-6.2 (SC-02; kill criterion K-02)",
    "structuralGuarantee": "S-6.4 (SC-08 proof)",

...
status: approved
created: '2026-08-24'
scope: pyforge-unifying-strategy operating-model Q1–Q8 — pyforge-marshal (planning-only)
approach: Direct Adjustment
---

# Sprint Change Proposal — Operating-model obligations (pyforge-marshal)
...
**Marshal-local:** Loop/runner plugins are station hooks. A passed loop or story is not a PR quality verdict. Path B orchestration uses the Agent Canopy + station persona; Tachyon is only a production LLM endpoint the loop may be configured to call.

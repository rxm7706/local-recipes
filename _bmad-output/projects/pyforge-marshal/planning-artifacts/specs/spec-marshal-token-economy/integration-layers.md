...
|---|---|---|---|---|---|
| 0 — output compression | `recipes/caveman/` | 2.4.0, MIT (installer only) | Agent's own output (weighted heaviest) | Claude Code skill deployed per loop home by Genesis; ~65% output cut; upstream input proxy (`@caveman-ai/cli`) is BSL-1.1 and excluded | Commented-out in `pixi.toml` (pixi-candidate-currency blocker) |
| 1 — wire compression | `recipes/headroom-ai/` | 0.32.1, Apache-2.0 | Tool outputs, logs, file reads, diffs (40–95%) | `headroom wrap <cli>` / transparent proxy at the harness seam; reversible CCR store scoped to the loop home; live-zone-only so the provider cache hot zone stays byte-identical | Commented-out in `pixi.toml` (same blocker: `conda-recipe-manager==8.2.1` pin chain) |

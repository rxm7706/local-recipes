...
| # | Finding | Evidence | CAP |
|---|---|---|---|
| 9 | Shim roster still 20, one seat swapped: `bmad-checkpoint-preview` is a new shim forwarding to the new `bmad-walkthrough`; `bmad-generate-project-context` (a 6.11 shim) is neither a 6.12 shim nor a `removals.txt` entry — an update leaves its 6.11 directory orphaned | `src/bmm-skills/v6-shims/` (14) + `src/core-skills/v6-shims/` (6) at `v6.12.0`; `removals.txt` at `v6.12.0`; `.claude/skills/` 6.11 shim census (20) | CAP-8 |

...
**Post-push correction (2026-09-24, PR #1583 CI catch).** `.github/workflows/substrate-nightly.yml` as pushed called `scribe graph compile --nightly` directly to build the pair's planning-graph member -- this violates Epic 8's HARD boundary (`tests/scripts/test_scribe_nightly_trigger_docs.py`, caught by the `scripts-suite` CI lane) and is a genuine correctness bug the review pass above missed: a GitHub-hosted runner has none of the operator-local surfaces that compiler reads (checked-in `.claude/memory/`, session transcripts, the gitignored graph store), so it would have silently published an empty planning-graph. Fixed by removing that build step; the nightly pair now publishes only structure-graph and derived-context, and planning-graph is always locally rebuilt by the consumer via the existing MRS-CTX-003 fallback -- the only place it can be built correctly. This does not change the "publisher is inert" residual risk above (Actions are still disabled), but it means the *first* real run, whenever that is, does the right thing rather than the wrong one. `air-gapped-deployment.md` gained a clarifying note. Full re-verification: `tests/scripts/` 660 passed, `pyforge-marshal-test` 8600 passed.

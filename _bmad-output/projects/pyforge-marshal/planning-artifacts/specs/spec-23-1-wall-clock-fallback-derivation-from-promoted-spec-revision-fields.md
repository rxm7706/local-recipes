...
deferred:
  - summary: >-
    ...
    location: >-
      docs/dashboard/generate.py:scan_timing
    severity: medium
  - summary: >-
    ...
    location: >-
      docs/dashboard/generate.py:_sid_from_journal_key
    severity: low
---
...
- `docs/dashboard/generate.py` `scan_timing` (~L2776) — journal-only derivation today; early-`continue` when `not spans` blocks wall-clock-only lines; must merge wall-clock for done stories with zero journal coverage without putting those minutes on `velocity.bars`.
- `docs/dashboard/generate.py` `LOOP_HOMES` / `resolve_project` — journal home lookup; project dir for `planning-artifacts/specs/spec-{e}-{n}-*.md`.
...
- `docs/dashboard/data.js` -- Regenerate via `python docs/dashboard/generate.py --source git` so doctor 8.1–8.4 appear under `projects.doctor.timing.perStory`.
...
**Commands:**
- `pixi run -e local-recipes pytest .claude/skills/conda-forge-expert/tests/meta/test_dashboard_scan_timing_wall_clock.py -q` -- expected: all matrix rows PASS
- `pixi run -e local-recipes pytest .claude/skills/conda-forge-expert/tests/meta/test_dashboard_renders.py .claude/skills/conda-forge-expert/tests/meta/test_dashboard_resolve_project.py -q` -- expected: PASS
- `python docs/dashboard/generate.py --source git` -- expected: doctor timing includes 8.1–8.4; metric names ceiling bound

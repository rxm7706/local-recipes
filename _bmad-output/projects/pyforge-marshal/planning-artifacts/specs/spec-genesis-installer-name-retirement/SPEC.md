...
- **CAP-6**
  ...
  - **verified: 2026-09-12 — CLOSED.** Fixed both misses this CAP's own note named, plus one more the note never checked. `brief.md:172`'s header retitled to `## The seed installer (marshal seed)` (matching `prd.md`/`ARCHITECTURE-SPINE.md`'s own proven convention exactly), plus its 3 companion prose mentions (the "genesis-installer product brief" phrase, two "the Satellite below" cross-references in the file's own 2026-08-26 currency-reconciliation section). Then checked `specs/` (this CAP's own success criterion explicitly names it, but the prior pass never actually looked there): `spec-pyforge-marshal/SPEC.md` — marshal's own top-level kernel Spec — had SIX live section headers still standing on the compound name ("Genesis-installer constraints, folded in...", "...non-goals...", "...success signal...", "...assumptions...", "...open questions...", plus two "the folded genesis-installer questions" mentions), all renamed to "The seed installer's own ..." matching the same convention; its `companions:`/`sources:` frontmatter also cited two now-stale facts — the "Satellite: Genesis Installer PRD/Architecture" section names (both retitled 2026-08-08) and a companion entry for `epics-genesis-installer.md` that no longer exists at that live path at all (merged into `epics.md` and archived by this same Spec's own CAP-1) — both corrected. Full re-check across `planning-artifacts/{prds,architecture,specs,briefs}`: every remaining `genesis`+`satellite`/`genesis.installer` hit is now either an append-only changelog comment, a past-tense historical quote (`"This was a satellite — ..."`), the already-correctly-retitled `"Part II — the seed installer (...formerly the pyforge-genesis satellite...)"` headers in `prd.md`/`ARCHITECTURE-SPINE.md` (proof the convention took), the one already-known and separately-tracked `spec-dashboard-project-path-derivation` deferred-work item (unchanged, out of this CAP's scope by its own prior note), or this Spec's own text quoting its target pattern to describe what it retires. Zero live, un-flagged violations remain.
- **CAP-7**

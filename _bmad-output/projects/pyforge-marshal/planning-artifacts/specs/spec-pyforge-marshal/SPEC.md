...
- **CAP-252 — a blocked outcome never lands** ← spec-pyforge-marshal CAP-252 (ready 2026-09-19)
  - **intent:** A session that ends `blocked` — intent gap found, branch reverted to baseline, `status: blocked` written — produces no PR, no `done` promotion, and a journal fact carrying the spec's own reason, so the ledger tells the truth and the story is re-driven or re-minted deliberately.
  - **success:** On a fixture of doctor 27.3 (empty diff against baseline plus `status: blocked` in the worktree spec; PR #1476 landed it and promoted `27-3 → done`), the supervisor's finalize sequence stops before verify and land, journals `dispatch-blocked` with the reason, the campaign records a station block (not an advance), and the tracked ledger row never reads `done`; `has_git_progress` no longer counts a revert-to-baseline plus a status flip as progress; the pre-launch `parse_spec_status` guard treats `blocked` as not relaunchable without an operator decision; an implementation with changed paths lands exactly as today.
...
- **CAP-255 — landing evidence is intent-scoped, not just station-scoped** ← spec-pyforge-marshal CAP-255 (ready 2026-09-19)
  - **intent:** A branch that merely mentions a story key in a station-prefixed name is not that story's landing branch; only the shapes marshal itself mints for a landing mark a key merged, so a mint, fallout or fix branch can never poison a key.
  - **success:** `merged_story_keys` for `pyforge-doctor` against `origin/main` no longer contains 27.4 via `doctor/27-4-mint` (PR #1477), while every `done` row of the eight tracked ledgers with real landing evidence still classifies (the CAP-247 regression fixture extended); `parse_station_branch_name` requires the landing grammar (`dispatch/<slug>/<key>`, or the key as the whole last segment of a station branch); `_branch_belongs_to_project` gains its production caller — the retrospective scan passes real branch data instead of `branch=None` (closes DW-FU-50-4); doctor's `sources/marshal.py` consumers and `pyforge-core-test` stay green (co-governed by spec-landing-evidence-grammar / spec-pyforge-core); live history is never re-attributed.

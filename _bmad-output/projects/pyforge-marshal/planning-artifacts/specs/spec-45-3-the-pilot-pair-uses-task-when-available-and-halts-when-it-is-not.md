...
**Summary:** no commit subject on `main` names this story (hand-implemented, or landed under another story's subject); the ledger row `45-3-the-pilot-pair-uses-task-when-available-and-halts-when-it-is-not: done` is the record and `story-status` accepts it.

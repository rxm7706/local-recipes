...
sync_lineage:
  ...
  - { date: '2026-06-21', via: 'narrative re-sync (bmad-correct-course)', from: 'v8.39.0', to: 'v8.41.0', proposal: null, spec: 'docs/specs/sole-maintainer-feedstock-refresh.md', retro: null, note: 'Body-text narrative sweep from the v8.39.0 pin to live v8.41.0. v8.41.0 (MINOR, additive) added six feedstock-refresh build-shape gotchas G46–G53 (G46 stale-local-meta noarch wrong for a compiled package; G47 stale recipe-dir conda_build_config.yaml lint + variant duplicate-entry collision; G48 Rust/Go upstream ≠ heavy from-source compile; G49 per-Python compiled ≠ abi3; G50 newer-CPython C-API symbol drop → cap python matrix with match(python, ">=N"); G53 monorepo-subdir missing release-time-generated assets) plus 3 feedstock-refresh refinements; no code/test/CLI/schema change. Epic 6 gotcha-authoring story text re-synced G7–G45 → G7–G53. Atlas surface unchanged (schema v28; 42 MCP tools; 22 phases; 9 pixi envs). No epic scope change; no new stories required (gotcha additions extend the existing E6.S1g catalog story). FR-ID annotations added to Epic 14 (S1–S13 → F2.13/F2.14) and Epic 10 tool-registration rows (S5b–S5h → F3.9) for the SF1 readiness gate. source_pin re-pinned v8.39.0 → v8.41.0 (frontmatter bumped out-of-band).' }
...
**Goal**: Establish the pixi-managed Python 3.12 monorepo with the **9 factory envs** and core dependencies (the 6 lean `no-default-feature` product envs arrive with Epic 15 / Part 5 — **18 envs / 17 features** live today). No code yet — just the substrate.
...
| E1.S2 | Author `pixi.toml` core | requires-pixi >=0.76.2; declare 8 of the 9 envs (`linux`, `osx`, `win`, `build`, `grayskull`, `conda-smithy`, `local-recipes`, `vuln-db`; the 9th `gcloud` env is added in E1.S11); set `# default-env: local-recipes` directive | `pixi info` shows the declared envs; default-env resolves correctly | M |
...
**Goal**: Author SKILL.md (incl. Recipe-Authoring Gotchas G1–G53), INDEX.md, CHANGELOG.md, 17 reference docs, 9 guides, 2 quickrefs, 41 templates (13 ecosystems), and the test suite.
...
| E6.S1g | Author SKILL.md Part G: gotchas G7-G53 (the full accumulated catalog) | Extend the Recipe Authoring Gotchas section with G7 through G53 — every gotcha accumulated across the v8.x release line, e.g. G7/G10 (distribution-vs-import-name divergence), G16/G24/G26/G28/G31-G34 (feedstock-PR remediation), G38/G40/G42 (per-Python prereq fan-out + noarch flip), G39-G43 (langflow-closure external-cf-skew), G44 (.NET binary-repackage), G45 (browser-SPA viability gate + local-only fallback), G46-G53 (sole-maintainer feedstock-refresh build-shape: G46 stale-local-meta noarch, G47 stale recipe-dir CBC collision, G48 Rust/Go ≠ heavy compile, G49 per-Python ≠ abi3, G50 CPython C-API symbol drop, G53 monorepo-subdir missing generated assets). Each gotcha = a 1-paragraph pattern + case study, matching the SKILL.md gotcha style. **Depends on E6.S1f (G1-G6 section established).** | Gotchas G7-G53 all present in SKILL.md; each has a pattern statement + a concrete case study; numbering contiguous G1-G53 | L |
...
| E6.S7 | Author `reference/mcp-tools.md` | All 46 MCP tools enumerated with signature + use case + script counterpart | All 42 covered | M |
...
**Epic 6 total: 40 stories** (was 36 actual rows — the prior "35" total line undercounted by 1, a pre-existing drift now corrected; +4 this re-sync: E6.S1g gotchas G7-G53, E6.S5b recipe-yaml-reference-full, E6.S9b conda-forge-yml-reference-full, E6.S15b atlas-engineering+edge-case references; the 9th guide feedstock-platform-expansion folded into E6.S16; structural history: +5 from E6.S1 split into S1a-f, +4 from E6.S25 split into S25a-e, +1 for G45 template), complexity ~M-L average; **no XL stories remaining**.
...
| Wave 2 (Part 1 skill) | 3 | 71 | Mostly M-L (was 2 XL: SKILL.md split into 6, unit tests split into 5); +G7-G53 + full-reference/guide retrofits |
...
| E15.S3 | Six lean `no-default-feature` envs | `pyforge-{warden,atlas,doctor,scribe,herald}` + `bmad-ui` resolve without the 9 factory envs; 18 envs / 17 features total | — (this epic) |

...
input_docs:
  ...
  - project-context.md
edit_history:
  - { date: '2026-05-13', via: 'bmad-edit-prd', delta: 'v7.8.1 → v7.9.0 sync after actionable-scope audit (docs/specs/atlas-pypi-universe-split.md): schema v19 → v20, +pypi_universe side table, +pypi-only-candidates CLI/MCP, +Phase D split. PATCH bump (no FR/NFR scope shift; count-and-pin sync).' } # skill retired in 6.11
...
|---|---|
| Pixi monorepo scaffolding (18 envs / 17 features, 154 tasks — 106 in the `local-recipes` feature) | The ~1,660 recipes under `recipes/` |
| conda-forge-expert skill (Part 1, 66 Tier-1 scripts / 57 Tier-2 wrappers + docs) | Recipe-specific upstream patches |
...
| F1.7 | 106 Recipe Authoring Gotchas (G1-G107) | P0 | SKILL.md § "Recipe Authoring Gotchas" enumerates G1-G107 with cause + fix |
...
> **11-tool** server (`src/shared/packages/pyforge-atlas/src/pyforge/atlas/mcp/server.py`).
...
| F5.4 | Six lean `no-default-feature` product envs | P0 | `pyforge-{warden,atlas,doctor,scribe,herald}` + `bmad-ui` resolve independently of the 9 factory envs (18 envs / 17 features total) |
...
- **G1-G53** — Recipe Authoring Gotchas in SKILL.md
...
| F1.7 (G1-G53 gotchas) | JTBD-1.2 | JTBD-3.1 | Non-obvious failures enumerated so agents avoid them |
...
| Feature | Primary JTBD | Secondary | Why this serves the JTBD |
|---|---|---|---|
| F2.1 (SQLite schema v29) | JTBD-1.3 | — | Single source of intelligence data; `pypi_universe` side table separates PyPI directory from conda-actionable working set |

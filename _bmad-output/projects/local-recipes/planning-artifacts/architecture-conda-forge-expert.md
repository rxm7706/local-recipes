...
- Recipe Authoring Gotchas (G1–G94, all in SKILL.md; latest G91–G94 — the batch-v5 full-gate set: PEP 517 plugin host deps, CFE-block fold/duplicate, conda-recipe-manager col-0 comment crash, stale-mirror pruning — added v8.66.0)
...
Non-obvious failures that have bitten enough times to be enumerated. The catalog now spans **G1–G94**, all promoted into SKILL.md (the table below shows the founding six; G7–G94 are titled inline beneath it). Each carries a one-line symptom + fix in SKILL.md § Recipe Authoring Gotchas, which is authoritative.

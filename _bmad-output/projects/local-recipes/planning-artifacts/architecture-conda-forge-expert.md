...
- Recipe Authoring Gotchas (G1–G90, all in SKILL.md; latest G88 — the shared-protobuf-namespace-stub-collision gotcha — added v8.63.0)
...
Non-obvious failures that have bitten enough times to be enumerated. The catalog now spans **G1–G90**, all promoted into SKILL.md (the table below shows the founding six; G7–G88 are titled inline beneath it). Each carries a one-line symptom + fix in SKILL.md § Recipe Authoring Gotchas, which is authoritative.

...
|---|---|---|---|---|
| E1.S1 | Initialize repo + LICENSE + README | git init; BSD-3-Clause LICENSE; minimal README pointing at PRD; .gitignore covering pixi/rattler/output dirs | LICENSE valid; README exists; .gitignore covers `.pixi/`, `build_artifacts/`, `output/`, `.claude/data/`, `_bmad-output/.gitignore` | S |
...
| E1.S5 | Add local-recipes feature (default env composite) | Composite env: `["python", "build", "grayskull", "conda-smithy", "local-recipes"]`; activation hooks | Default `pixi run` activates the right env; activation runs without error | M |
...
| E1.S8 | Add `scripts/load-env.sh` + `scripts/bmad-switch` stubs | Parse `# default-env:` directive; bmad-switch reads/writes marker file | `scripts/bmad-switch --list` works (returns empty list pre-Wave-1-end); `load-env.sh` activates default env | S |
...
| E1.S10 | Verify-env pixi task | `pixi run verify-env` validates pixi.toml integrity + env count + default-env directive presence | Task runs without error; reports 8 envs | S |
...
| ID | Title | Scope | AC | Complexity |
|---|---|---|---|---|
| E2.S1 | Install BMAD-METHOD v6.6.0 | Install `bmad-method` PyPI package (or equivalent); generate `_bmad/config.toml` (layer 1) + `_bmad/config.user.toml` (layer 2); installs ~40 BMAD-installer skills | `_bmad/` exists with config + bmm/ + core/ + scripts/; ~40 skills under `.claude/skills/bmad-*/` | M |
| E2.S2 | Implement `_bmad/scripts/resolve_config.py` | Six-layer TOML merge; Python 3.11+ stdlib `tomllib`; active-project resolution priority (CLI > env > marker > none); merge rules (scalars override, tables deep-merge, arrays of tables key-merge) | `resolve_config.py --key agents` returns valid JSON; per-layer override tests pass | L |
...
| E2.S5 | Implement `scripts/bmad-switch` CLI | Python script; `--list` / `--current` / `--clear` / `<slug>` subcommands; slug validation (`^[a-z0-9][a-z0-9_-]*$`); writes marker file | All subcommands work; bad slug rejected; non-existent project rejected | M |
...
| E2.S9 | Add engineering-practice skills | Copy or author ~22 skills (`api-and-interface-design`, `ci-cd-and-automation`, etc.) under `.claude/skills/` | `ls .claude/skills/ | wc -l` returns ~65 total | M |
...
| ID | Title | Scope | AC | Complexity |
|---|---|---|---|---|
| E3.S1 | Author `.claude/skills/conda-forge-expert/scripts/_http.py` skeleton | Module with `make_request(url, method='GET', **kwargs)` interface; truststore integration; JFrog header injection; GitHub token detection; .netrc fallback | Function returns response object; all 4 auth methods exercised in unit tests | L |
...
| E3.S8 | Add `tests/meta/test_http_jfrog_scoping.py` | Meta-test verifying `_http.py` injects JFROG_API_KEY header unconditionally (documenting the current behavior so a future host-aware refactor breaks this test as the refactor signal) | Test runs; verifies the current header-on-every-host behavior | S |
...
**Goal**: Author SKILL.md, INDEX.md, CHANGELOG.md, 11 reference docs, 8 guides, 2 quickrefs, 30 templates, and the test suite.
...
| E6.S19 | Author 30 recipe templates (11 ecosystems) | python (noarch + compiled + maturin), rust (library + cli), go (pure + cgo), c-cpp (header-only + autotools + cmake + meson), r (cran + bioconductor), java (maven + gradle), ruby (gem), dotnet (nuget), fortran (f90), conda-forge-yml (staged-recipes + feedstock); both v0 + v1 where applicable | All 30 templates present; each valid YAML | L |

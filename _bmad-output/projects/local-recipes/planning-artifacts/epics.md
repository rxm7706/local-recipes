...
**Goal**: Establish the pixi-managed Python 3.12 monorepo with the **9 factory envs** and core dependencies (the 6 lean `no-default-feature` product envs arrive with Epic 15 / Part 5 — **17 envs / 17 features** live today). No code yet — just the substrate.
...
| E15.S3 | Six lean `no-default-feature` envs | `pyforge-{warden,atlas,doctor,scribe,herald}` + `bmad-ui` resolve without the 9 factory envs; 17 envs / 17 features total | — (this epic) |

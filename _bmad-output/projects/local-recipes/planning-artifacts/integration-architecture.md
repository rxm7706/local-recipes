...
|---|---|---|
| Part 1 (skill) | `CHANGELOG.md` TL;DR + `MANIFEST.yaml: version` (separate surfaces) | Semver: PATCH for fixes, MINOR for new gotchas/sections, MAJOR for breaking workflow changes. CHANGELOG bumps on every release; MANIFEST bumps only on portability protocol changes (currently v7.0.0; release is v8.40.0). |

...
**17 envs in two families.** The isolation is itself a contract: it is what lets `pyforge-atlas` and `pyforge-doctor` require Python **≥3.14** while warden/herald/scribe require **≥3.12** and the factory runs 3.12 — three floors that no single solve could satisfy.
...
Plus the cross-cutting items (17 pixi envs in two families, env-var inheritance, permission gates, CI gates, spec-surface governance) are setup conditions, not contracts per se, but the rebuild must establish them before any part can function.

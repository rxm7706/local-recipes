...
| DW14 | Phase G' rollup expansion — atlas currently tracks only Critical/High/KEV-affecting-current. vdb exposes additional severity-relevant signals at no extra fetch cost: Medium-affecting-current count (where ≥3 Medium aggregate to High-equivalent risk), Low-affecting-current count, EPSS score (Exploit Prediction Scoring System), CWE category mapping (RCE vs DoS vs traversal), and `withdrawn` advisory filter (currently includes withdrawn findings in counts). Plus PyPI yanked-flag (PEP 592) channel-wide (Phase R captures it for top-N candidates only). **Partially addressed by v8.6.0 spec** at `docs/specs/atlas-appthreat-deep-signals.md` (EPSS + CWE rollup + withdrawn filter + blint hardening; Medium/Low counts + yanked-flag remain open for v8.7.x). | This session's atlas-signal-scope redefinition |
| DW15 | _placeholder reserved during 2026-05-23 retro for follow-on AppThreat-ecosystem signals (EPSS / CWE / withdrawn) — superseded by the v8.6.0 spec `docs/specs/atlas-appthreat-deep-signals.md` which bundles them into Waves A-D. This row exists for retro-traceability only._ | retro-dw12-dw13-2026-05-23.md § Follow-ups |
...
| DW17 | `scan_project --pixi-lock <file>` mode via cdxgen `parsePixiLockFile` shell-out. cdxgen DOES support pixi.lock (verified 2026-05-23 against `CycloneDX/cdxgen` `lib/helpers/utils.js@9798-9920`; emits `pkg:conda/<name>@<version>-<build>?os=<subdir>` purls with URL + sha256 + license + `depends:` edges), but does NOT parse `recipe.yaml`/`meta.yaml` directly. Today `scan_project` accepts `--conda-env`, `--venv`, `--sbom-in`, manifest/lockfile types — but not `--pixi-lock`. Add a new input mode that shells out to `cdxgen -t pixi -o /tmp/bom.json <path>` and re-enters the `--sbom-in` code path with the produced BOM. Deliberately scoped as a separate follow-up (NOT bundled into v8.6.0) to keep that release's "atlas-side signals only" narrative clean; introduces a Node.js / npm runtime dep (`@cyclonedx/cdxgen`) that wasn't in v8.5.x. Effort: ~6-8 stories — install cdxgen as vuln-db env dep, scan_project flag plumb, integration test against a fixture pixi.lock, doc updates. | `docs/specs/atlas-appthreat-deep-signals.md` Appendix A § "Where cdxgen-on-pixi.lock would belong" |

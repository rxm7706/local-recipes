...
│
├── pixi.toml                                # 9 envs + ~80 tasks + 7 features (linux/osx/win/grayskull/conda-smithy/local-recipes/vuln-db; build & gcloud envs feature-light)
...
│
├── recipes/                                 # 1,602 recipe directories (OUTPUTS of the system, not part of it)
│
...
│   │   ├── SKILL.md                         # ★ primary spine: critical constraints, 10-step loop, gotchas G1–G106
...
│   │   │
│   │   ├── scripts/                         # ★★ Tier 1: canonical Python implementations (54 modules)
│   │   │   │
...
│   │   │
│   │   ├── tests/                           # 82 test files (unit + integration + meta)
│   │   │   ├── unit/                             # function-level
...
│   │
│   ├── # ── BMAD installer skills (42 bmad-* of the 65 total) ──
...
│   │
│   └── # ── Engineering practice skills (21 real + 1 stray `data/` dir = 22) ──
...
│   └── conda-forge-expert/                  # 46 thin subprocess wrappers (delegate to skill scripts/)
...
└── projects/
    ...
    │   │   ├── PRD.md / epics.md / architecture*.md / project-overview.md / ...
    │   │   ├── source-tree-analysis.md      # ← THIS FILE
    │   │   ├── project-parts.json           # machine-readable 4-part manifest
    │   │   └── change-history/              # NEW: sprint-change proposals (8 dated files)
    │   └── implementation-artifacts/
    │       ├── deferred-work.md             # cross-spec deferred items
    │       ├── spec-*.md                    # ~18 per-effort spec mirrors (cursor-sdk, phase-f, phase-k, db-gpt, ...)
    │       └── retros/                      # NEW: closeout retros (22 dated files)
    │
...
└── specs/                                   # BMAD-consumable feature specs (12; consolidated 2026-07-02)
    ...
    ├── atlas-pypi-universe-split.md         # pypi_universe side table (shipped v7.9.0)
    ├── conda-forge-expert-v8.0.md           # v8.0.0 structural-enforcement bundle
    ├── conda-forge-expert-v8.9.md           # v8.9 bundle
    ├── cfe-pr-artifact-downloader.md        # Azure DevOps PR artifact downloader (v8.14.0)
    ├── # ── packaging efforts ──
    ├── db-gpt-conda-forge.md                # 13-story DB-GPT packaging plan (lyric-* + cocoindex-class)
    ├── langflow-conda-forge.md              # langflow full prereq-closure
    ├── microsoft-conda-forge.md             # microsoft/* org audit (~10-14 recipes)
    ├── trendshift-conda-forge.md            # top GitHub-trending Python → cf
    ├── graphifyy-osx-arm64-fanout.md        # 22 tree-sitter-* platform-expansion PRs
    ├── feedstock-platform-expansion.md      # dual-goal refresh + platform-widen
    ├── feedstock-failure-remediation.md     # failing-feedstock-PR remediation loop
    ├── sole-maintainer-feedstock-refresh.md # bulk sole-maintainer feedstock refresh
    ├── # ── tooling / adjacent ──
...
```
recipes/                                     # 1,602 recipe directories (718 recipe.yaml + 1,054 meta.yaml; many carry both during v0→v1 migration)
└── <package-name>/
...
| `pixi.toml` | all | Defines 9 envs and ~80 tasks; the contract between human shell and tool surface |
...
## Counts (verified 2026-06-20)

| Asset | Count |
|---|---|
| Recipes in `recipes/` | 1,602 (718 `recipe.yaml` + 1,054 `meta.yaml`; many carry both mid-migration) |
| CFE canonical scripts | 54 (Tier 1) |
| CFE CLI wrappers | 46 (Tier 2; some scripts are internal-only and don't have wrappers) |
| MCP tools (verified by grep `@mcp.tool`) | 42 |
| Recipe templates | 41 across 13 ecosystems (12 language: c-cpp, dotnet, fortran, go, java, multi-output, nodejs, perl, python, r, ruby, rust + conda-forge-yml config-template starter); verified 2026-06-20: 39 `.yaml` + 2 `.yml` in conda-forge-yml subdirs |
| Skill tests | 82 (unit + integration + meta) |
| Installed skills total | 65 (BMAD installer + repo-specific + engineering-practice) |
| Skill reference docs | 17 |
| Skill guides | 9 |
| Skill quickrefs | 2 |
...
- Skill v8.41.0 → v8.42.1 (MINOR + PATCH, additive). Recipe-Authoring Gotchas grew to G1–G106 (+G54–G55 source-selection: `sdist > GitHub-source > wheel` + build-backend-in-host; v8.42.1 a cfe-block convention clarification). No source-tree structural change: CFE scripts/wrappers/tests, atlas phases (22), schema (v28), MCP tools (42), `reference/` (17), `guides/` (9) all unchanged — v8.42.x is SKILL.md + CHANGELOG only.

**Refresh notes (2026-06-27 vs. prior 2026-06-24 sync):**
- Skill v8.42.1 → v8.52.1 (langflow-suite closure + submission span). Recipe-Authoring Gotchas grew to **G75** (G56–G75: multi-output-win/feedstock-check, submission-flow + run_constraints reconciliation, G71 win+py3.12 reactor gap, G72 fold-sibling-into-suite, G73 monorepo-tag-ships-no-frontend → cross-platform node-build, G74 atlas-membership-staleness, G75 lean-submission-clean). No source-tree structural change: scripts/wrappers/tests, atlas phases (22), schema (v28), MCP tools (42), `reference/` (17), `guides/` (9) all unchanged — SKILL.md + CHANGELOG only.

**Refresh notes (2026-06-28 vs. prior 2026-06-27 sync):**
- Skill v8.52.1 → v8.62.0 (v0→v1 feedstock-migration retro + the conda-forge.yml-default span). Recipe-Authoring Gotchas grew to **G90** (G76–G87: langflow-win closeout, go-licenses, pnpm<11, staged-recipes-ARM, G83 conda-forge.yml-inert, G84–G87 the v0→v1 feedstock-migration set). **One structural change**: new `scripts/_cfy_template.py` — the shared renderer for the universal `conda-forge.yml` pre-seed (v8.61.0; now emitted by default on every generator path). atlas phases (22), schema (v28), MCP tools (42), `reference/` (17), `guides/` (9) all unchanged.

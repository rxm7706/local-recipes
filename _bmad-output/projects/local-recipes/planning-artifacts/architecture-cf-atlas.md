...
```
  ...
  └─────────┘  └──────┘  └──────┘  └───┘  └─────┘  └───┘  └───┘  └─────┘  └─┬─┘
            ...
            (atlas-phase,                ~46 MCP tools                      recipe-lifecycle
...
schema v29) is the **directory** of every PyPI project — separated from
...
**v8.15.0 → v8.19.0 additions (schema v29 → v26 → v27 → v28, the Phase P/F+ download-intelligence band):**
...
**v8.6.0 additions (schema v29, via v24 → v25 cleanup):**
...
CREATE TABLE pypi_universe (                -- Phase D side table (v7.9.0 / schema v29)
...
- **→ Part 3 (MCP server)**: every query CLI surfaces as an MCP tool in `conda_forge_server.py` (~46 tools total).

...
**Opening.** Before wiring CI, a developer trials the gate locally: install to the workstation (`pixi global install` / the local channel; air-gapped via a **pixi-pack** bundle; nebi-adopted teams via **nebi pull** — Story 5.1 docs), then `python-deptry-osv-scanner scan . --warn-only` — the recommended first contact with the tool.

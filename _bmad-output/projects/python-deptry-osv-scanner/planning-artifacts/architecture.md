...
referenceImplementation: src/shared/packages/python-deptry-osv-scanner/  # pixi build member; stdlib-only lib, deptry+osv-scanner conda run-deps; cli.py stub
...
project_name: 'python-deptry-osv-scanner'
user_name: 'rxm7706'
date: '2026-07-11'
---
...
**Rationale:** the stdlib-only constraint is load-bearing (lightweight fleet gate + S1 security), so no third-party CLI framework is permissible. The existing scaffold at `src/shared/packages/python-deptry-osv-scanner/` already establishes the entire foundation.
...
- **FR30 → a `ConfigLoader` component** owns the dual `[tool.python-deptry-osv-scanner]` schema (pyproject+pixi, per-key precedence, conflict-surfaced), the hygiene→status table, and the CVSS thresholds.
...
- **External contracts (versioned):** `report-schema.json` (`ComplianceReport`, `schema_version`) → stdout/`--output`; CycloneDX 1.6 SBOM → `--sbom-output`; the frozen exit-code enum `{0,1,2,130}`; the `[tool.python-deptry-osv-scanner]` config-key schema. cf_atlas (FR-16/18, post-v1) consumes the report contract.
...
**Execution model (2026-07-12 — "Option B"):** stories are driven by **`bmad-dev-auto`** sessions orchestrated by **`bmad-loop`** (`DEV → VERIFY → REVIEW → VERIFY → COMMIT`; policy at `.bmad-loop/policy.toml`), per `docs/specs/bmad-loop-adoption.md`. The deterministic `[verify]` gate is `pixi run -e python-deptry-osv-scanner python-deptry-osv-scanner-test` — so the 1.1a/1.1b contract tests + C0a/C0c gates mechanically police every later story. Gates graduate: `per-story-spec-approval` (1.1a/1.1b) → `per-epic` (E2+). The epics' Given/When/Then ACs are the contract dev-auto's spec conversion must preserve verbatim; CRITICAL escalations resolve via `bmad-loop-resolve`; the sprint feed is `sprint-status.yaml` from `bmad-sprint-planning`.

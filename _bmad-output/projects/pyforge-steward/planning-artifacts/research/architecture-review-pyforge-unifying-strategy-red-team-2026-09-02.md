...
> (`sprint-change-proposal-2026-09-02-red-team-critical.md`, Epic 40, `ready-for-dev`). R-3 … R-16 → Epics **41–43**
> (`sprint-change-proposal-2026-09-02-red-team-high.md`, fourteen stories, `ready-for-dev`). R-17 … R-25 →
> `DW-RT-2026-09-02-1..9` in the steward deferred-work ledger.
...
| S-5 | **Parity is broken at the interpreter.** The platform image materializes `python-agent-platform` (`python = "3.12.*"`, forced by langflow/dbgpt). Atlas and Doctor declare `>=3.14`; the host's own `_log_import_skip` fires because their MCP faces cannot import in that interpreter. *(Corrected 2026-09-02: the `mcp-host` sidecar exists because `langflow-base` pins `mcp <2.0` while host faces need `mcp` 2.x — MCP-SDK isolation, not an interpreter workaround; raising the interpreter does not retire it.)* Laptop (3.14 station envs) and cluster (3.12 image) run different bytecode, different C-extension builds, different `asyncio`. "Identical Pydantic models … run seamlessly" is true only of pure-Python code. | HIGH | `pixi.toml:181`, `mcp_http.py::_log_import_skip`, Dream § Fleet conventions "Python floor" row. |
...
**R-16 Right-size the interpreter story (closes S-5, D-1).** *Decided 2026-09-02: hybrid (a)+(c) — see steward 43.5 / 43.6 and Mason Epic 13; solver probes showed only two feedstock pins block 3.14.*

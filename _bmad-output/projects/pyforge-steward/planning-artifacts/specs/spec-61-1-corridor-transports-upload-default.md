...
**Summary of implemented change.** Vendor and estate lists now have an idempotent drop path. A new steward CLI duty, `steward load {inbound,outbound} --file PATH --waybill LABEL [--transport NAME] [--corridor DIR] [--json]` (the 21st duty), computes a SHA-256 `batch_sha` over the file and records the load keyed on `(direction, batch_sha, waybill)` in a new `CorridorLoad` Django model behind the `pyforge-steward[dashboard]` extra ("the existing Postgres app"). A repeat drop of the same key is a no-op — idempotent regardless of which transport (even an unknown or currently-disabled one) the repeat call names, since the existence check runs before transport validation; only the path that would create a brand-new row needs a transport that is both declared and `state: on`. Transports are declared in a new tracked `corridor/corridor.yaml` (git is the edit store, same philosophy as `catalog/catalog.yaml`): `app-upload` ships `on` (the v1 default); `shared-folder` and `email` are `state: off` slots with no runtime behavior in v1 (per the companion doc, "email is an empty slot until enabled" / "v1 does not require a mailbox parser") — a transport in v1 is a provenance label + on/off gate, not a distinct ingestion code path. Bare `steward load` (no verb) reports the declared transports read-only, touching neither Django nor the database. The base package (`corridor.py`) stays django-free at the AST level, reaching the ORM half (`dashboard/corridor_load.py`) through the same dynamic `importlib.import_module` idiom `sprint_ledger_query.sync_to_postgres` already established as the one sanctioned base→dashboard reach — now two such reaches, both pinned by `tests/meta/test_invariants.py`. No `vendor_id` and no passport/identity concept were added (Story 61.2's job on a different capability); no vendor or estate *content* is parsed or stored — only the drop's SHA-256, waybill label, direction, and transport.

**Files changed.**
- `src/shared/packages/pyforge-steward/src/pyforge/steward/corridor.py` (NEW) — config loading (`CorridorConfigError`, `TransportDecl`, `CorridorConfig`), `compute_batch_sha`, `load_extract` (existence-probe-then-validate-then-create, dynamic dashboard reach), `LoadDuty` (bare = read-only transport report; `inbound`/`outbound` = idempotent load; unified `_load_result` helper for consistent `--json`/payload shape across every branch; `_MAX_WAYBILL_LENGTH = 128` Python-side cap).
- `src/shared/packages/pyforge-steward/src/pyforge/steward/dashboard/corridor_load.py` (NEW) — `record_corridor_load(..., create_if_missing=True)`, the Django-ORM half: unconditional existence check first, `create_if_missing=False` read-only probe mode, a scoped `except IntegrityError` recovery around `.create()` that re-queries and returns `idempotent` on a genuine concurrent race (distinct from a real `DataError`), lazy `import django` never at module top.
- `src/shared/packages/pyforge-steward/src/pyforge/steward/dashboard/models.py` — added `CorridorDirection` (`TextChoices`) + `CorridorLoad` (unique on `direction`+`batch_sha`+`waybill`).
- `src/shared/packages/pyforge-steward/src/pyforge/steward/dashboard/migrations/0003_corridorload.py` (NEW) — generated via a real `makemigrations` run (not hand-written), verified zero-drift against `test_the_shipped_migration_matches_the_model`.
- `src/shared/packages/pyforge-steward/src/pyforge/steward/dashboard/admin.py` — registered read-only `CorridorLoadAdmin`, mirrors `AuditEntryAdmin`.
- `src/shared/packages/pyforge-steward/src/pyforge/steward/dashboard/__init__.py` — module-split docstring updated: "one of the sanctioned base→dashboard reaches" (no longer "the ONE"), names `corridor_load.py`'s lazy reach.
- `src/shared/packages/pyforge-steward/corridor/corridor.yaml` (NEW, tracked) — the three declared transports and their v1 states.
- `src/shared/packages/pyforge-steward/src/pyforge/steward/cli.py` — wired the 21st duty (`DUTIES`, `_HELP`, `build_parser` elif-chain + new `_add_load_subparsers`, `resolve_duty`).
- `src/shared/packages/pyforge-steward/tests/unit/test_corridor.py` (NEW, 536 lines) — full coverage: config loading, `compute_batch_sha`, `load_extract` (unknown/off transport, fresh load, identical-repeat idempotency — the I/O Matrix's required scenario, different-waybill-same-sha, repeat-with-a-different-or-off-transport-still-idempotent, `load_extract`'s own `ImportError` refusal), `LoadDuty.run` (bare, first-load-then-repeat for both directions, missing file, off transport, waybill-too-long, broken `--corridor` config, `--json` on a success and a failure branch), `record_corridor_load`'s refusal/error/race branches, `CorridorLoadAdmin`, CLI parsing.
- `src/shared/packages/pyforge-steward/tests/unit/test_cli.py` — duty count 20 → 21 (`test_there_are_exactly_twenty_one_duties`).
- `src/shared/packages/pyforge-steward/tests/unit/test_restore_duty.py` — `len(DUTIES) == 20` → `21` (caught by running the real suite, not named in the original Code Map).
- `src/shared/packages/pyforge-steward/tests/meta/test_invariants.py` — reworded the "ONE sanctioned reach" docstring claim to "one of the sanctioned...reaches" and added a mirrored AST assertion pinning `corridor.py`'s own lazy-only dashboard reach; extended `test_the_dashboard_module_split_is_pinned_not_merely_documented`'s module list with `corridor_load.py`.
- This spec — planned, reviewed, triaged, patched.

**Review findings breakdown** (21 findings from four layers; every row in `## Review Triage Log`).
- Patches applied — 9 triage entries after grouping: **medium 4** (a concurrent double-drop misreported as `status: error` instead of idempotent; `--json` honored inconsistently across `LoadDuty.run` branches with an inconsistent payload shape; transport validation running before the idempotency check, so a repeat drop with a different/off transport wrongly raised instead of no-opping; an unbounded `waybill` risking a silent SQLite-vs-PostgreSQL divergence `dashboard/audit.py` already guards against for `AuditEntry`), **low 5** (`load_extract`'s own `ImportError`→`refused` path untested; `LoadDuty`'s `CorridorConfigError` branch untested at the duty level; the idempotent branch's "originally recorded transport" behavior untested against a differing repeat transport; `CorridorLoadAdmin` never instantiated by any test; a stale `# status == "refused"` comment that also silently caught `status == "error"`). High 0.
- Rejected (5, `false`): "importlib deferred into the function body" — no named harm beyond stylistic noise; "spec status flip with no ledger update" — the dev-loop spec status and the ledger status are intentionally decoupled tracks per AGENTS.md; "admin list_display shows the full batch_sha" — matches the existing `WorkPassportAdmin` precedent in the same file; "`db_index` on `direction` is redundant" — `CorridorLoadAdmin.list_filter` queries `direction` alone, a path the composite constraint doesn't serve; "'plugins' have no plugin extension point" (self-flagged medium-confidence) — the companion doc's own words ("email is an empty slot until enabled") and this spec's Design Notes already establish that scope deliberately.
- Rejected (3, `low`, not worth the complexity): the outer duty-boundary `except Exception` catch-all untested (mirrors sibling duties' own untested catch-alls, contrived to force); `--corridor ""` silently defaulting (unlikely, fix adds a new guard); a zero-transports `corridor.yaml` producing a bare summary (the tracked file will never actually be empty, fix adds a new guard).
- No `intent_gap` or `bad_spec` entries — the Intent Alignment Auditor's report (strictly descriptive per its own brief) confirmed the diff consistently implements the CAP-139-scoped, mechanism-only reading this spec's own Design Notes already document; no unresolved ambiguity was found.

**Follow-up review recommendation: `true`** — first pass; patched entries by verdict: high 0, medium 4, low 5. Four medium entries were patched (≥ 2), so per the computation rule this is `true`. The specific unverified risk: the four medium-severity fixes — the concurrent-create race recovery in `dashboard/corridor_load.py`, the unified `--json`/payload-shape handling via `corridor.py`'s new `_load_result` helper, the reordered existence-before-transport-validation control flow in `load_extract`, and the new `_MAX_WAYBILL_LENGTH` cap — were verified by this run's own re-execution of the full station suite, the coverage gate, and direct diff inspection, but not by a fresh, independent review layer. The reordered `load_extract` flow in particular (a real behavioral change to the core idempotency logic, via the new `create_if_missing=False` probe mode) is the one a follow-up pass should re-examine most closely for any interaction this pass did not anticipate.

**Verification performed.**
- `pixi run --frozen -e pyforge-steward pyforge-steward-test` (the dispatch's configured verify, real env), both before and after the patch pass: 1467 passed / 4 skipped → **1477 passed, 4 skipped** after patches (10 new tests).
- Targeted manual pytest run over `test_corridor.py`, `test_cli.py`, `test_dashboard_admin_and_htmx.py`, `test_dashboard_audit.py`, `test_restore_duty.py`, `test_invariants.py`: **173 passed** (pre-patch pass), independently re-run by this orchestrator (not just the implementation subagent's own report) via `pixi run -e pyforge-steward python -m pytest ...` — first attempt without the pixi env showed 4 skips from a missing `django`, confirming the env matters; the pixi-env run showed the full count with zero skips.
- `PYTHONPATH=... python -m pyforge.steward.cli load` (both before and after patches): exit 0, `app-upload: on, shared-folder: off, email: off` — byte-for-byte the spec's expected output.
- `pixi run -e pyforge-steward pyforge-steward-coverage-gate`: OK both passes; `corridor.py` 86%→90%, `dashboard/corridor_load.py` 95%→91% after the patch pass added branches (both still well above the 80% floor); 24/24 touched modules ≥ 80%.
- Matrix Test Audit: the I/O Matrix's one row ("duplicate drop | same batch sha + waybill | idempotent no-op") is covered by `test_load_extract_identical_repeat_is_idempotent` and `test_load_duty_first_load_then_repeat_is_idempotent` (parametrized over both directions); both ran and passed in every verification pass above, independently re-run and re-confirmed by this orchestrator with `-k "idempotent"` (7 passed).
- Diff read in full by this orchestrator, twice (pre- and post-patch), not merely summarized from the implementation subagent's report, per step-03/04's "judge against the diff" instruction.

**Residual risks.**
- The follow-up-review risk named above (the four medium patches, particularly the reordered `load_extract` flow, unseen by a fresh review layer).
- No ledger/PR mechanics were run: `sprint-status-ledger.yaml` still reads `backlog` for `61-1-corridor-transports-upload-default` (ledger promotion via `sprint-ledger-sync` and any PR/label mechanics are a separate step, per AGENTS.md, from this single-story `bmad-build-auto` dispatch); no `pr-preflight`/shared-surface station suite was run since this branch touches only `pyforge-steward`'s own tree and no `pixi.toml`/`pyforge-core` shared surface.
- `corridor.yaml`'s `shared-folder`/`email` rows remain inert declarations with zero runtime behavior, exactly as scoped — a later story enabling either would need to add real ingestion behavior, not just flip `state: on`.
- `_MAX_WAYBILL_LENGTH` is enforced only in `LoadDuty.run` (the CLI boundary), not inside `load_extract` itself — a future direct caller of `load_extract` bypassing the CLI would not get this guard; acceptable for now since the CLI is the only caller today.

...
| bmad-eval-quality | PrivateChannel 1.3.0, BOTH noarch variants (`__unix` heeef243_0 + `__win` h2fd06db_0) — tag-sourced since 1.3.0 (joined 2026-09-05 at 0.2.0.dev0 @ 3172162f, Story 45.1; class `cli`) | `npm i -g eval-quality` (README § Install) — npm is CURRENT again (1.4.0, re-checked 2026-09-09); the long-standing 0.1.0-without-`score` staleness is resolved, so npm and GitHub now agree | bare CLI, nothing wires into `_bmad`. The commit pin is retired — the recipe follows tags (CFE G109), so the "exact commit pin is load-bearing" caveat no longer applies. **Windows: CLOSED (mason Story 14.1, 2026-09-09).** The `__win` variant was built on a GitHub Actions windows-2022 runner and uploaded, because a staged-recipes PR builds that variant and then discards it (CFE G115); both variants are confirmed in the SERVED repodata, so the `pixi.toml` pin left the unix-only target tables for the shared table and a win-64 solve now resolves `h2fd06db_0`. **Open:** the recipe is at 1.4.0 while the channel's newest is 1.3.0 — pins deliberately track the published build, so publishing 1.4.0 is what unblocks raising them |

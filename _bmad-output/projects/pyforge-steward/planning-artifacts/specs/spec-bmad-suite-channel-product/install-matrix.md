...
| bmad-eval-quality | SelfExplainML 0.2.0.dev0 @ 3172162f (joined 2026-09-05, Story 45.1; class `cli`) | `npm i -g eval-quality` (README § Install) — but npm latest is 0.1.0 without `score`; the conda recipe pins the 0.2.0 line from `main` | bare CLI, nothing wires into `_bmad`; npm STALE (0.1.0) — GitHub `main` canonical; exact commit pin is load-bearing (nine breaking schema-version bumps between v0.1.0 and main). **Windows (mason Story 14.1, 2026-09-06):** the recipe is win-ready (`if: win`/`if: unix` selectors, `call`-safe `build.bat`, `noarch_platforms: [linux_64, win_64]`) and build.number 1 re-verifies green on linux-64, but the `__win` artifact itself has not been built on Windows CI / a real Windows machine or uploaded to SelfExplainML — `pixi.toml` still pins it in the unix-only target tables (operator step, Epic 14 HARD boundary) |

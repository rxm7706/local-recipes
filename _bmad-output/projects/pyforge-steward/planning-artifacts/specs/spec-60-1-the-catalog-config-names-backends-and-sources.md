...
- 2026-09-19 (dev): `EstateListingsSource` also enforces the file-level `source:` (must be `estate-listings`, else a `config-source` finding) so a mis-sourced registry with `modules: []` is not a silent green; the per-row mismatch is `listing-source-mismatch` as specified. A module listing whose `repository` is a non-GitHub git URL renders with Claude Code's documented `{"source": "url", "url": …}` form; one with no repository at all is a `listing-no-repository` finding. Review pass 1 (2026-09-19) also added `listing-duplicate` (same `(kind, name)` from any sources), made `render`/`render --check` refuse on any collect finding (nothing written), and made `plugin: null` an empty slot legal only while `off`/`available` (the companion table's v1 default for the Claude-skill registry).

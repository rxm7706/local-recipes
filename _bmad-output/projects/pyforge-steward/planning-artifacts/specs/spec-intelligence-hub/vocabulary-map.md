...
| Nebi | pixi + conda-forge + this recipe factory + the SelfExplainML channel + the `bmad-suite` metapackage — the lineage the paper cites (pixi on conda) | No package format for Frames / Cogs / Ops / Guards; whether Nebi / Nebari are packaged anywhere we can consume is unverified (**RB-2**) |
| Frames | `CLAUDE.md`, the verified `AGENTS.md` block, `docs/dreams/`, Specs and memlogs, `.claude/memory/` team memory (Scribe), each skill's `SKILL.md`, `docs/governance/` | Inheritance (repo → station → story) is real but informal; no open protocol (**RB-1**), no field-level sharing, no feedback channel to an accountable owner beyond git review |

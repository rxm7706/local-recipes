...
and fnd:AD-10's own already-adopted convention ("cross-station prerequisites are
...
- **`Deps:` fields carry only same-station `S-<epic>.<n>` tokens (fnd:AD-10's
  ...
  exactly matching fnd:AD-10's own worked phrasing ("after steward 46.3") and
...
- **The `blocked` ledger row is added, then immediately reconciled to the
  ALREADY-TRUE state, not left mid-round-trip.** fnd:AD-10's own convention:
...
| `epics.md` Story 44.5's acceptance | Gains one new `**And**` clause describing the fnd:AD-10 in-story check against `cutover-readiness.md` P11/P12, with the trailing prose "after marshal 30.5 and 30.2" |
...
| Cutover memlog (`spec-python-foundry-cutover/.memlog.md`) | Gains, in order: (1) a `(note)` recording 44.13's new acceptance clause BEFORE the epics.md edit; (2) an `(event)`/`(note)` recording the fnd:AD-10 cross-station gate check for 44.5 (14.9/30.5/30.2 all confirmed `done`) |
...
- `_bmad-output/projects/pyforge-steward/planning-artifacts/epics.md`
  ...
  - Story 44.5's acceptance: new `**And**` line, e.g. "**And** this story
    ...
    but green (fnd:AD-10's own worked example) — after marshal 30.5 and 30.2
...
- `_bmad-output/projects/pyforge-steward/planning-artifacts/specs/spec-python-foundry-cutover/.memlog.md`
  - New `(note)` (44.13's clause, written FIRST per the AC's own ordering)
    and a following `(event)`/`(note)` (the fnd:AD-10 gate check for 44.5).
...
3. **Write the cutover memlog note (44.13's clause) BEFORE editing
   epics.md; write the fnd:AD-10 gate-check note after.**
...
- 2026-09-07: initial draft, written directly (no fork) after reading
  ...
  44.13 in full, fnd:AD-10's complete definition in the lifecycle
...
- **Blind Hunter**: independently re-verified the three-project dependency
  ...
  other epic; confirmed fnd:AD-10's own text genuinely supports the new
  acceptance clause's framing (verified "not green" ≡ "red" against
  fnd:AD-10's own worked-example wording); confirmed `cutover-readiness.md`
...
- **Intent Alignment**: independently re-evaluated, from first principles,
  ...
  (the exact cited fnd:AD-10 template) through its real
  ...
  is a correct application of fnd:AD-10's actual intent, not a rationalization
...
- **Why 44.12 is named in the Surface line but gets no text change:**
  ...
  the Surface line because it's the OTHER Epic 44 story fnd:AD-10's own
...
- **Why `44-5-move-the-estate`'s ledger row is NOT set to `blocked` even
  though the AC's own text says "plus a `blocked` ledger row":** fnd:AD-10's
  ...
  that the fnd:AD-10 check WAS performed and passed, not infer a blocked
...
Story 44.5's `Deps:` line plus one new fnd:AD-10-shaped acceptance clause
...
before the fnd:AD-10 gate-check event). `cutover-readiness.md` G2/G3/G6 all

---
title: 'pyforge-steward — Retrospective (2026-09-20: the fleet lint / type gate lands)'
...
evidence:
  - 'PR #1553 (2026-09-20): src/shared/packages/pyforge-steward/pyproject.toml, every .py under src/ and tests/'
  - 'pixi run -e pyforge-guild lint-types — exit 0 on the landed tree'
---

## What the gate found on its first run

- **Type baseline:** 11 modules baselined (7 codes). Every entry is `[[tool.mypy.overrides]]` per module with exactly the error codes that module trips — never `ignore_errors` — so the gate is live for every other module and code. Tighten or drop an entry whenever its module is touched; the baseline is a ratchet, not a floor.
- **Lint:** `bootstrap._PRE_COMMIT_CONFIG_RELATIVE_PATH` named a dotless `pre-commit-config.yaml`, so `steward setup`'s hooks step could never have found the canonical file — found by Story 66.2 landing the file.

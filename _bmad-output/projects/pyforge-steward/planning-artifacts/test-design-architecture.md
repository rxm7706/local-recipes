...
- Single operator, no multi-tenant surface (PRD §2.2) for the CLI-package core; the estate-facing epics (9, 26, 34, 41, 42) scale to multi-role dashboard viewers and multi-agent MCP callers, governed by the Canopy spine's AD-1..canopy:AD-23.
...
2. **R-6: Pod specs / image layers must never carry secret values (canopy:AD-19 unifying, AD-24 uc:AD-3)** — recommend a `tests/meta` invariant scanning the container build manifest and any Helm/OCP overlay for literal secret material, mirroring the existing `test_keys_plaintext_secret_scan.py` pattern for `keys list` output (implementation phase, owner: Dev/Steward).
...
| **R-14** | **DATA** | python-foundry cutover (Epic 44) repeats the shape of a previously-realized incident: this repo's own team memory (`feedback_...` / `project_...` entries) records pyforge-warden losing 13 of 31 story specs during an earlier cross-worktree migration, fully recovered only because session transcripts happened to survive. Epic 44's fnd:AD-21 ("the archive is the oracle") and 44.13 ("memlog fidelity") are direct responses to that lesson, but the cutover itself (44.3 "open the foundry" through 44.10 "archive local-recipes") has not yet executed. | 2 | 3 | **6** | Rehearse the restore path (fnd:AD-21) against a throwaway clone before the real cutover flag (fnd:AD-17, "cutover is a flag, not a date") flips. | Steward/Marshal | Before 44.3 executes |
...
**Mitigation Strategy:**
1. Rehearse the fnd:AD-21 "archive is the oracle" restore path against a throwaway clone before Epic 44's cutover flag (fnd:AD-17) flips for real.

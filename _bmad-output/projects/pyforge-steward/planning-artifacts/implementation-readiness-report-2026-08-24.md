...
Q5 measures parked on `spec-build-league-scorecard` (`draft`). CAP-18 is decomposed (canopy:FR-43..45,
...
| Q2 five-tier = 03 only | SPEC Never; PRD glossary + canopy:FR-37/38/39 + SM-5; Epic 29 / S-29.3 |
...
process hooks. **Not in Epics 18–30.** Bound as CAP-18 / canopy:FR-43–45.

| Bound | Where a developer reads it |
|---|---|
| Shared hook-spec + registration in `pyforge-core` | SPEC CAP-18; PRD canopy:FR-43; steward **S-32.1** |
| Warden PR-gate; scanners optional; green without Checkmarx | canopy:FR-44; Warden **Epic 9** |
| Per-station process layer; today's backend = default plugin | canopy:FR-45; steward S-32.2; atlas 18.1; mason 10.1; marshal 26.1; doctor 17.1; herald 16.1; scribe 4.1 |
...
PRD §12 covers CAP-1..18 → FR-1..canopy:FR-45 plus FR-9a/9b/21a. canopy:FR-44 is Warden Epic 9.
canopy:FR-45 is steward S-32.2 plus peer station process stories. Every FR has an epic.

FR-6 (inventory) is already delivered as `console-parity-inventory.md`; Epic 30
is the cutover, not a second inventory. Packaging FRs (FR-21, canopy:FR-33) bind to
...
| warden | 9 | Rename is S-19.1; `/compliance/` redirect. **Epic 9** = PR-gate hook specs; scanners optional plugins (canopy:FR-44). |

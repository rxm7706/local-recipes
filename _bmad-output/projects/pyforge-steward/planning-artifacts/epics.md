...
**Type:** feature • **Effort:** S • **Deps:** S-14.6 (after marshal 30.5 — cross-station: ledger `blocked` + the refusal check, AD-10) • **FR/AD:** spec-bmad-method-core-upgrade CAP-9 • spec-bmad-suite-lifecycle CAP-10 • lifecycle spine AD-5, AD-7
...
**And** the flag is refused with a named reason when any `_bmad/custom/<legacy-name>.toml` exists (trap 2) or when the marshal harness template or any rendered loop-home policy still emits the retired `bmad-dev-auto` (reported, never edited — foreign surface), so a retirement can never strand a loop home
...
**Type:** chore • **Effort:** S • **Deps:** S-14.10 • **FR/AD:** CAP-8 • AD-7 • PRD FR-8

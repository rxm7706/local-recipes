...
- source_spec: `_bmad-output/projects/pyforge-atlas/implementation-artifacts/b2-port-the-pypi-and-vulnerability-pipelines.md`
  ...
  verified: 2026-09-07 — resolved — `summarize_vdb_vulns` (`pipelines/vulnerability/nodes.py`) now takes an optional `core_packages_enumerated` input (`conda_name` -> `latest_version`, the same AD-3 cross-pipeline edge `ingest_basilisk_advisories` already uses) and scopes `vuln_kev_affecting_current` to the row(s) matching the package's CURRENT version, not the package-wide KEV sum; when the current version is unknown (input absent/empty or the package isn't in the map) it degrades to the old package-wide count rather than fabricate a scoped-but-wrong 0 (AD-13). `vuln_count`/`vuln_critical_high`/`vuln_max_cvss`/`vuln_max_epss_score` are unchanged (still package-wide by design, AC-2). Wired into the pipeline via `pipelines/vulnerability/pipeline.py`'s node inputs. New regression tests in `tests/pipelines/vulnerability/test_nodes.py`: `test_summarize_vdb_vulns_kev_affecting_current_scopes_to_current_version` (a package with a KEV hit on an old, superseded version AND a separate KEV hit on its current version reports only the current-version hit) + `test_summarize_vdb_vulns_kev_affecting_current_degrades_gracefully_without_current_version`. Full `kedro-test` suite: 1728 passed, 24 skipped, plus one pre-existing unrelated failure inherited from the branch base — `test_conda_forge_expert_not_replaced`, a git-history meta-check tripped by the orchestrator's own prior CFE-retro commit (9ef5b8cd79), confirmed present before this fix via `git stash` (identical failure on the unmodified base).
...
- source_spec: `_bmad-output/projects/pyforge-atlas/implementation-artifacts/spec-13-3-bokeh-websocket-interactivity-cap-2.md`
  ...
  verified: 2026-09-10 — **closed** (atlas Story 25.1): live-Bokeh test surface removed with `views/`; upstream hazard preserved in this ledger entry only.
...
- source_spec: `planning-artifacts/specs/spec-19-4-the-cis-two-spine-specs-exist-cap-7.md`
  ...
  verified: 2026-09-06 — still-open — semantic re-read against live code at HEAD d7faf964fb, post-refresh (Story 46.8's `steward provision --module cis --json` re-run, which refreshed all 10 installed CIS skills from the packaged revision -- the conda package is now pinned `>=0.3.2`, up from the entry's original v0.3.1 citation): `.claude/skills/bmad-cis-design-thinking/template.md:1` still renders `{{project_name}}`, and `.claude/skills/bmad-cis-design-thinking/SKILL.md` Step 4 "Load Config" still resolves only `output_folder`/`user_name`/`communication_language`/`date` -- no step resolves `project_name`. The v0.3.2 bump (which added the `resolve_customization.py --project-root` flag) did not touch this unrelated placeholder bug. Confirmed still upstream in the packaged `bmad-creative-intelligence-suite` skill, not introduced by the refresh. Stays open.
...
### DW-ATLAS-CLI-STARTUP-1: every atlas entry point pays a pandas+numpy import, making `pyforge-atlas --help` ~10x slower than its peers

- source_spec: `_bmad-output/projects/pyforge-atlas/planning-artifacts/specs/spec-pyforge-atlas/SPEC.md`
  summary: `pyforge/atlas/__init__.py:9` does `import pandas as _pd` at module scope in order to pin `future.infer_string` off (AUD-ATLAS-011 — pandas 3.0's `str` dtype uses `NaN` as its missing-value sentinel, which corrupts atlas's own DuckDB→pandas conversion). Because the pin lives in the package `__init__`, **every** atlas entry point pays the full pandas + numpy import — including `--help`, which needs neither. Measured 2026-09-14 as direct console scripts (no `pixi run`, no `docker run`, warm page cache): **`pyforge-atlas --help` 2.26s vs `warden --help` 0.23s**. `python -X importtime` attributes 290ms of a 290ms `pyforge.atlas.cli` import to pandas alone.
  evidence: Surfaced as a CI failure, not a benchmark: `guild-container (docker)` on PR #1355 reported `container-gates: cli-smoke: pyforge-atlas: timed out (> 10s)` while the other seven CLIs passed, and the same job had passed on the immediately preceding run — the 10s budget had become a coin-flip rather than a bound. `scripts/container-gates`'s own constant comment had reserved the right to per-station tuning *"absent evidence any one is structurally slower"*; this is that evidence, so the budget was raised 10 → 30 with the measurement recorded inline.
  location: src/shared/packages/pyforge-atlas/src/pyforge/atlas/__init__.py
  severity: low
  status: accepted-risk
  raised: 2026-09-14 — Owner: atlas. **The pin must not simply be deleted or deferred carelessly.** The package docstring guarantees to every importer that *"importing this package (or any of its submodules) pins pandas' process-wide"* option — that is a correctness contract, and moving the pin behind lazy initialisation reintroduces AUD-ATLAS-011 for any caller that imports pandas and uses it before atlas's first real operation. A real fix keeps the guarantee while letting `--help` avoid pandas: candidates are a console-script entry point that does not import the `pyforge.atlas` package at all for argument parsing, a PEP 562 module `__getattr__` combined with an explicit pin at every genuine entry, or narrowing the guarantee in the docstring after establishing no caller depends on it. Raising the timeout bounds the CI symptom; it does not address the 10x startup cost.

  corrected: 2026-09-14 — **This entry's diagnosis was wrong, and would have sent someone to optimise the wrong thing.** It blamed the module-scope pandas import for `pyforge-atlas --help` being ~10x its peers. Re-measured properly as direct console scripts in atlas's own env, warm cache:
    - bare interpreter `0.03s` · `import pandas` `0.62s` · `import pyforge.atlas.__main__` **`0.36s`** (pandas ≈ 0.28s of it, 77% of the *import*)
    - **`pyforge-atlas --version` `0.34s`** — fast, because Story 7.1 already intercepts `--version` *before* the Kedro imports
    - **`pyforge-atlas --help` `2.29s`**

...
- source_spec: `cfe-atlas-datapipeline-kedro-migration.md` (Story H3, § 7.1, FR-22(c))
  ...
  annotation: 2026-08-15 (Story 16.2, `_bmad-output/implementation-artifacts/spec-16-2-httpx-opener-and-rehearsal.md`) — a real httpx-backed `Opener` builder + CLI entrypoint (`src/shared/packages/pyforge-atlas/tools/lasuite_bringup.py`) now exists, plus an offline rehearsal (`tests/factory/test_lasuite_live_rehearsal.py`) proving that SAME opener drives the mock-proven create/update/idempotent-skip/resume sequence over a loopback HTTP stub. The attended DW-H3 bring-up checklist (which will run this exact script against a real Wagtail/La Suite server) is documented in that spec's Design Notes. Still ATTENDED, still not executed — `status` stays `open`; DW-H3 closes only when the real bring-up runs and passes.

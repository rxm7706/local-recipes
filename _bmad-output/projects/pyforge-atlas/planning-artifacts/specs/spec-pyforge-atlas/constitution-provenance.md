...
> (the Unity Data Stack satellite's companion, moved there intact, not
> deleted). `AD-27`/`AD-25`/`AD-26` below (were `AD-4`/`AD-2`/`AD-3` in the
> standalone document) are renumbered to match the merged
> `ARCHITECTURE-SPINE.md`. `FR-n` below is local to
...
> of the primary Spec's `FR-1`..`FR-22`. See that archived path for the
> original.
...
2026-02-20 — already overdue). Full detail: PRD § 14 (`archive/_bmad-output/projects/pyforge-atlas/planning-artifacts/prds/prd-unity-data-stack-2026-07-25/prd.md`).
...
| Environments — 12-stage SDLC | CRITICAL | FR-9, FR-58 | Adopted **as Stages**, modelled separately from Environments (AD-27) |
...
for AD-25 (Workspace Lock authoritative, PEP 751 export derived and drift-checked) and AD-26

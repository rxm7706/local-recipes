...
**Summary:** no commit subject on `main` names this story (hand-implemented, or landed under another story's subject); the ledger row `9-3-integrate-la-suite-docs-rest-api-sync: done` is the record and `story-status` accepts it.
...
- frontmatter `status` `shipped` → `done` (ledger row `9-3-integrate-la-suite-docs-rest-api-sync: done`).

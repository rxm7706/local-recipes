...
**Summary:** landed on `main` as `8f8d7a0e9c` (2026-08-15, "Story 9.2: hygiene sweep across all eight stations (CAP-8)"); also `11634075b3` (2026-08-08, "herald: Story 9.2 + 9.3 + 9.5 — success CLI wired to claim storage"). Ledger row `9-2-implement-agno-compilation-linting-and-q-a-crews: done`.
...
- frontmatter `status` `shipped` → `done` (ledger row `9-2-implement-agno-compilation-linting-and-q-a-crews: done`).

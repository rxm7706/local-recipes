...
**Summary:** landed on `main` as `e2c77635e0` (2026-08-07, "herald: Story 5.2 — conflict refusal on export push (CAP-5)"); also `9e99defb16` (2026-08-07, "marshal: Story 5.2 -- per-run detail"). Ledger row `5-2-build-the-vizro-dashboard-port-the-28-clis-to-pages: done`.
...
- frontmatter `status` `shipped` → `done` (ledger row `5-2-build-the-vizro-dashboard-port-the-28-clis-to-pages: done`).

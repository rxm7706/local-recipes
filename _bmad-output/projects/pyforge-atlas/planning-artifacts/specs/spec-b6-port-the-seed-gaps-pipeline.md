...
audit_note: |
  ...
  A2 had declared 73 entries and B6 added none, so the gate stayed green at 73. The catalog
  has grown since; the LIVE pin is `EXPECTED_TOTAL = 86` (verified 2026-07-28 against
  conf/base/catalog.yml and tests/catalog/test_catalog_resolution.py). Read every 73 below as
  "as of B6", never as current. Rewriting them to 86 would falsify the delivery record.
  Correction to the audit: it named "A2/B6"; spec-a2's only `73` is the version string
  `pixi 0.73.0`, not a catalog count — B6 is the sole site.

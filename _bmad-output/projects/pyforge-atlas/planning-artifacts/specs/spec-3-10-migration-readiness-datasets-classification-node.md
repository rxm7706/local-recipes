...
**Summary:** no commit subject on `main` names this story (hand-implemented, or landed under another story's subject); the ledger row `3-10-migration-readiness-datasets-classification-node: done` is the record and `story-status` accepts it.
...
- frontmatter `status` `shipped` → `done` (ledger row `3-10-migration-readiness-datasets-classification-node: done`).

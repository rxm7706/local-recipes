...
**Summary:** no commit subject on `main` names this story (hand-implemented, or landed under another story's subject); the ledger row `3-9-release-to-availability-velocity-columns: done` is the record and `story-status` accepts it.
...
- frontmatter `status` `shipped` → `done` (ledger row `3-9-release-to-availability-velocity-columns: done`).

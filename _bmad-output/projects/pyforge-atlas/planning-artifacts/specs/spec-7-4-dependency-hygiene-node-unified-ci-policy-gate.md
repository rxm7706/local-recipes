...
**Summary:** no commit subject on `main` names this story (hand-implemented, or landed under another story's subject); the ledger row `7-4-dependency-hygiene-node-unified-ci-policy-gate: done` is the record and `story-status` accepts it.
...
- frontmatter `status` `shipped` → `done` (ledger row `7-4-dependency-hygiene-node-unified-ci-policy-gate: done`).

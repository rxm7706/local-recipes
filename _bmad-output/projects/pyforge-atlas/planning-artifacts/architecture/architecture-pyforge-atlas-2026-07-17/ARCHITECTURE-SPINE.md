...
- **Rule:** budgets (per-node timeout/retry), validation hooks, lineage/OTel instrumentation, and profile definitions are declared in Kedro run configuration — so every entry point (Dagster-compiled job, MCP trigger, CLI) executes the identical named pipeline with identical machinery; an MCP trigger names a profile explicitly or inherits the `maintainer` default. **DEMOTED 2026-07-27 (`AUD-ATLAS-046`):** this rule previously asserted "a dataset has one writing run at a time: run admission serializes on the target dataset set (concurrent trigger … rejected/queued, never interleaved)." **That was never implemented.** The `in_process` executor in `conf/base/dagster.yml` serializes ops *within* one run and gives no cross-run or cross-process admission; there is no lock or queue in `mcp/session.py` or anywhere in the package. The shipped scope of AD-23 is the shared plane + identical machinery only. Admission is tracked as `DW-AD23-1` and **AD-23 is re-promoted to its full form only after that ships with a passing gate**.

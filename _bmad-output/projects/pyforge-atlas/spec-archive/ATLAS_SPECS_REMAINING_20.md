# ATLAS_SPECS_REMAINING_20 — B9–H4 retro-reconstructed story files
#
# NOTE: These 20 stories were NEVER written as files at build time (waves B9–H4
# ran through the in-session agent loop; .bmad-loop/runs/ never existed). The
# content below is RETRO-RECONSTRUCTED from real sources only — verbatim epics.md,
# deferred-work.md, and the actual merged git commits/diffs. It is NOT a recovered
# original. No content was invented.
...
===BEGIN C1===
# Story 4.1 (C1): Integrate `kedro-dagster` for scheduling + execution

Status: done (retro-reconstructed 2026-07-25)

> ⚠️ **Retro-reconstructed** from `epics.md` + `deferred-work.md` + merged PR `#84`. Not an original build-time story file (this wave ran through the in-session loop). Dev Agent Record / File List / Review Triage Log below are the **actual shipped git evidence**, not a live dev-agent transcript.
...
166eb42 story(C1): kedro-dagster orchestration glue + dagster-dryrun gate (FR-6)
 ...
 pixi.toml                                          |   4 +
 .../packages/pyforge-atlas/conf/base/dagster.yml   |  15 +
 .../src/pyforge/atlas/orchestration/__init__.py    |  23 ++
 .../src/pyforge/atlas/orchestration/definitions.py | 453 +++++++++++++++++++++
 .../tests/catalog/test_no_inline_io.py             |  66 ++-
 .../pyforge-atlas/tests/orchestration/__init__.py  |   1 +
 .../tests/orchestration/test_definitions_dryrun.py | 249 +++++++++++
 7 files changed, 804 insertions(+), 7 deletions(-)
...
===BEGIN G1===
# Story 8.1 (G1): Compile the intelligence layer to Pyodide / DuckDB-WASM

Status: done (retro-reconstructed 2026-07-25)

> ⚠️ **Retro-reconstructed** from `epics.md` + `deferred-work.md` + merged PR `#96`. Not an original build-time story file (this wave ran through the in-session loop). Dev Agent Record / File List / Review Triage Log below are the **actual shipped git evidence**, not a live dev-agent transcript.
...
203be0c story(G1): DuckDB-WASM in-browser read surface + wasm-smoke gate (FR-14)
 ...
 pixi.toml                                          |   9 +
 .../packages/pyforge-atlas/tests/wasm/__init__.py  |   0
 .../pyforge-atlas/tests/wasm/test_wasm_smoke.py    | 195 +++++++++++++++++++++
 src/shared/packages/pyforge-atlas/wasm/.gitignore  |   5 +
 src/shared/packages/pyforge-atlas/wasm/README.md   |  64 +++++++
 src/shared/packages/pyforge-atlas/wasm/build.py    | 142 +++++++++++++++
 .../pyforge-atlas/wasm/data/feedstock_health.csv   |   6 +
 src/shared/packages/pyforge-atlas/wasm/entry.mjs   |   9 +
 src/shared/packages/pyforge-atlas/wasm/index.html  | 109 ++++++++++++
 .../packages/pyforge-atlas/wasm/package.json       |  11 ++

...
OUT = Path(
    "/tmp/claude-0/-home-user-local-recipes/8c301a61-5446-55e6-b085-93ba3871f3dc/scratchpad/ATLAS-BMAD-SPECS-CONSOLIDATED.md"
)
...
SECTIONS = [
    # Tier 1 — intake spec
    (
        "Intake spec (Tier 1 — the binding contract)",
        "Tier 1",
        ROOT / "docs/specs/cfe-atlas-datapipeline-kedro-migration.md",
        None,
    ),
    # Tier 2 — planning
    ...
    (
        "PRD addendum",
        "Tier 2",
        PA / "prds/prd-pyforge-atlas-2026-07-17/addendum.md",
        None,
    ),
    (
        "Architecture spine",
        "Tier 2",
        PA / "architecture/architecture-pyforge-atlas-2026-07-17/ARCHITECTURE-SPINE.md",
        None,
    ),
    ...
    (
        ...
        "Tier 2",
        PA / "implementation-readiness-report-2026-07-17.md",
        None,
    ),
    (
        "Sprint-change proposal",
        "Tier 2",
        PA / "sprint-change-proposal-2026-07-17.md",
        None,
    ),
    (
        "Planning-phase closeout",
        "Tier 2",
        PA / "planning-phase-closeout-2026-07-17.md",
        None,
    ),
    # Tier 3 — story specs (local-only; only waves 0/A/B exist as files)
    (
        "Story 0.1 — legacy contextual skill",
        "Tier 3",
        IA / "0-1-generate-legacy-contextual-skill.md",
        None,
    ),
    (
        "Story A1 — scaffold Kedro/pixi project",
        "Tier 3",
        IA / "a1-scaffold-the-kedro-pixi-project-via-nebi.md",
        None,
    ),
    (
        "Story A2 — data catalog",
        "Tier 3",
        IA / "a2-define-the-data-catalog-for-all-sources-outputs.md",
        None,
    ),
    (
        "Story A3 — IncrementalParquetDataset / TTL",
        "Tier 3",
        IA / "a3-implement-incrementalparquetdataset-for-ttl-gating.md",
        None,
    ),
    (
        "Story B1 — conda-side backbone phases",
        "Tier 3",
        IA / "b1-port-the-conda-side-backbone-phases-into-kedro-nodes.md",
        None,
    ),
    (
        "Story B2 — PyPI + vulnerability pipelines",
        "Tier 3",
        IA / "b2-port-the-pypi-and-vulnerability-pipelines.md",
        None,
    ),
    (
        "Story B3 — Kedro-API-native MCP tools",
        "Tier 3",
        IA / "b3-re-expose-the-data-surface-as-kedro-api-native-mcp-tools.md",
        None,
    ),
    (
        "Story B4 — dataset parity vs legacy",
        "Tier 3",
        IA / "b4-verify-dataset-parity-against-the-legacy-orchestrator.md",
        None,
    ),
    (
        "Story B5 — external-refresh assets",
        "Tier 3",
        IA / "b5-port-the-external-refresh-assets.md",
        None,
    ),
    (
        "Story B6 — seed-gaps pipeline",
        "Tier 3",
        IA / "b6-port-the-seed-gaps-pipeline.md",
        None,
    ),
    (
        "Story B7 — universal SBOM intake",
        "Tier 3",
        IA / "b7-extend-the-universal-sbom-intake.md",
        None,
    ),
    (
        "Story B8 — Basilisk vuln ingestion",
        "Tier 3",
        IA / "b8-basilisk-conda-native-vulnerability-ingestion.md",
        None,
    ),
...
APPENDIX = [
    ...
    PA
    / "architecture/architecture-pyforge-atlas-2026-07-17/reviews/reconcile-inputs.md",
    PA
    / "architecture/architecture-pyforge-atlas-2026-07-17/reviews/review-adversarial-two-units.md",
    PA
    / "architecture/architecture-pyforge-atlas-2026-07-17/reviews/review-rubric-walker.md",
    PA
    / "architecture/architecture-pyforge-atlas-2026-07-17/reviews/review-version-verification.md",
...
H.append(">")
H.append(
    f"> Generated: {datetime.date.today().isoformat()} · binding spec version: see the intake spec's frontmatter."
)
H.append("")
H.append("---")
H.append("")
...
for title, tier, path, _ in SECTIONS:
    ...
    toc.append(
        f"{n}. [{title}](#{anchor})" + ("" if exists else " — *(missing locally)*")
    )
...
for title, tier, path, lang in SECTIONS:
    ...
    if not path.exists():
        block.append(
            "*(File not present in this checkout — see the cross-session note above.)*"
        )
        parts.append("\n".join(block))
        parts.append("\n---\n")
        continue
...
ap.append("")
ap.append(
    "Also excluded: `forge-data/` (Skill-Forge outputs for the `cf-atlas-legacy` "

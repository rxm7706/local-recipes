...
OUTDIR = Path(
    "/tmp/claude-0/-home-user-local-recipes/8c301a61-5446-55e6-b085-93ba3871f3dc/scratchpad"
)
...
STORY_PR = {
    "B9": ("#82", ["73e477f"]),
    "B10": ("#83", ["ecc161a", "520a75b"]),
    "C1": ("#84", ["166eb42"]),
    "C2": ("#85", ["d4d7372"]),
    "D1": ("#86", ["580e5ba"]),
    "D2": ("#87", ["7b6b3ca"]),
    "D3": ("#88", ["d58a4bd"]),
    "E1": ("#90", ["210b3a3", "01f8f82"]),
    "E2": ("#91", ["153a5ad"]),
    "F1": ("#92", ["13a5ce3"]),
    "F2": ("#93", ["1e122c8"]),
    "F3": ("#94", ["df58bfc", "2acfeaa"]),
    "F4": ("#95", ["fd8e1c9"]),
    "G1": ("#96", ["203be0c"]),
    "G2": ("#97", ["6146f83", "33b3fd8"]),
    "G3": ("#98", ["40b9eae"]),
    "H1": ("#99", ["fe52bbd"]),
    "H2": ("#100", ["2f4240f"]),
    "H3": ("#101", ["4e95efb"]),
...
def dw_blocks(sid):
    ...
    while i < len(dw):
        ln = dw[i]
        if (
            ln.startswith(f"## DW-{sid}-")
            or ln.startswith(f"## DW-{sid} ")
            or ln.strip() == f"## DW-{sid}"
        ):
            j = i + 1
...
d3 = [
    f"""# pyforge-atlas — B9–H4 story definitions (epics.md, verbatim)
...
d1 = [
    f"""# pyforge-atlas — B9–H4 story specs with implementation & review evidence
...
for sid in IDS:
    ...
    if body:
        d1.append(
            f"<details><summary>Primary commit body (`{shas[0]}`)</summary>\n\n```\n{body}\n```\n</details>\n"
        )
    d1.append("---\n")

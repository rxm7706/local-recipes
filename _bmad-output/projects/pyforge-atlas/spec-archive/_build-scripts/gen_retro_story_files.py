...
OUTDIR = Path(
    "/tmp/claude-0/-home-user-local-recipes/8c301a61-5446-55e6-b085-93ba3871f3dc/scratchpad/retro-story-files"
)
...
def epics_block(sid):
    start = next(
        (
            i
            for i, l in enumerate(epics)
            if l.startswith(f"### Story {sid} (") or l.startswith(f"### Story {sid}:")
        ),
        None,
    )
    if start is None:
    ...
    for j in range(start + 1, len(epics)):
        if (
            epics[j].startswith("### Story ")
            or epics[j].startswith("## Epic")
            or epics[j].startswith("## ")
        ):
            end = j
            break
    blk = epics[start:end]
...
def dw_blocks(sid):
    ...
    while i < len(dw):
        if (
            dw[i].startswith(f"## DW-{sid}-")
            or dw[i].startswith(f"## DW-{sid} ")
            or dw[i].strip() == f"## DW-{sid}"
        ):
            j = i + 1
...
def parse_story_narrative(block):
    ...
    for l in body:
        ...
        if (
            l.startswith("- **FRs:**")
            or l.startswith("- **Invariants:**")
            or l.startswith("- **Mode:**")
            or l.startswith("- **Gating")
            or l.startswith("- **Verify")
            or l.startswith("- **Depends")
        ...
        if mode == "narr":
            ...
            (meta if l.startswith("- **") else ac).append(l)
        if mode == "meta":
            meta.append(l)
    # dedupe meta lines
    seen = set()
...
concat = [
    f"""# pyforge-atlas — B9–H4 retro-reconstructed story files
...
for sid in IDS:
    ...
    out.append("")
    out.append(
        f"> ⚠️ **Retro-reconstructed** from `epics.md` + `deferred-work.md` + merged "
    ...
    out.append("")
    out.append(
        "*(Derived from the ACs — no original task breakdown was authored for this "
        "loop-run story.)*"
    )
    out.append("")
    ...
    for l in ac.splitlines():
        ...
        if ls.startswith("**Then**") or ls.startswith("**And**"):
            n += 1
            out.append(
                f"- [x] {ls.replace('**Then**', '').replace('**And**', '').strip()}"
            )
    if n == 0:
        out.append("- [x] Implement per the Acceptance Criteria above.")
    out.append("")
    ...
    out.append("")
    out.append(
        f"- [Source: _bmad-output/projects/pyforge-atlas/planning-artifacts/epics.md#Story-{sid}]"
    )
    out.append(
        f"- [Source: docs/specs/cfe-atlas-datapipeline-kedro-migration.md#§9-Story-{sid}]"
    )
    out.append(
        f"- [Architecture: _bmad-output/projects/pyforge-atlas/planning-artifacts/architecture/architecture-pyforge-atlas-2026-07-17/ARCHITECTURE-SPINE.md]"
    )
    out.append("")
    ...
    out.append("")
    out.append(
        "In-session BMAD agent loop (draft → 2× adversarial review → 1× independent "
    ...
    if revs:
        out.append(
            f"Independent/Gemini review produced follow-up fix commit(s) on PR `{pr}`:"
        )
        out.append("")
        for c in revs:
        ...
        out.append(
            f"No separate review-fix commit; findings (if any) folded into the impl "

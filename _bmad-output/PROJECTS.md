...
| `pyforge-genesis` | active | Genesis — the **constitutive** project (Charter §5): the master idea, the alignment instrument (vision deck), and the seed — Dream-first tiers, AGENTS.md family, BMAD multi-project wiring, deck family. Ships no product; the installer (`genesis init` greenfield / `genesis adopt` brownfield) is buildable work that moved to `pyforge-marshal` (`spec-genesis-installer`) in the 2026-07-28 split. Dream: `docs/dreams/pyforge-genesis.md`. |

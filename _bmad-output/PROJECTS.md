...
|------------------------|--------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ~~`local-recipes`~~ | dissolved | The former placeholder project — intake-only, where a Dream could land before it had a station. **Retired under INV-2** (`EXEMPLAR-STANDARD.md`): all 8 of its Specs moved to their owning Smith's `planning-artifacts/`, leaving zero, and `_bmad-output/projects/local-recipes/` no longer exists (verified 2026-08-15). This row is kept, struck through, only because CLAUDE.md and other docs still reference "local-recipes" as the repo/monorepo name — that name did not change, only this placeholder *project*. |
...
| `pyforge-atlas` | active | Kedro/Dagster/DuckDB migration of the cf_atlas orchestrator (~10k LOC, 23 cataloged phases) with a Vizro/Vizro-AI read surface, Boring Semantic Layer, and MCP/A2A agent interface. Original scope: 22 FRs across waves 0 + A–H, executed via bmad-loop — **waves A–H MERGED (PRs #58–#105); the original 32 stories all done, epics 0–9 done** (spec SHIPPED 2026-07-18, CFE v8.79.0). Impl artifacts reconciled 2026-07-23 after the Jul-19 truncation incident — see `projects/pyforge-atlas/_root-fallback-fork-2026-07-19/README.md`. Spec: `docs/specs/cfe-atlas-datapipeline-kedro-migration.md`. **Scope has since grown well past that original migration** (epics 10–16 added: dashboard provenance, run-admission hardening, kedro-skills adoption, the real DAG publish workflow, the trending-candidates discovery engine and its Mason handoff) — as of 2026-08-15, **55 stories / 15 epics total, 49 stories / 12 epics done**, unsupervised with 6 stories left (`pixi run -e local-recipes fleet-picture`). |
...
copied** as static content — `bmad-switch` / `bmad-loop-worktree` recreate them in whichever root
is active. `bmad-switch`'s own command surface and semantics are completely unchanged by the move;
only the root it operates against changes, and only at the AD-17 flip.

This describes a future state. As of this writing the foundry root does not exist, and the marker
and symlinks still point at `local-recipes`.

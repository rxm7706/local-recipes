...
- **Session Boot Recall:** Inspect `.claude/memory/MEMORY.md` at session start to load accumulated post-mortems, failure-mode traps, user preferences, and skill disambiguation defaults.
- **Critical Memory Traps:**
  - `feedback_replicate_every_triggered_ci_lane_locally.md`: Never rely on GitHub Actions CI to catch errors; replicate all triggered lanes locally.
  - `feedback_write_baseline_after_git_add.md`: Always stage files (`git add`) BEFORE running `--write-baseline`.
  - `feedback_parallel_agent_pr_needs_the_whole_chain.md`: Multi-agent PRs require the entire chain (Dream append, CAPs, Story, story spec, ledger row).
  - `feedback_no_courtesy_comments.md`: No AI attribution lines (`Co-Authored-By`, "Generated by AI") in commit messages or code comments.

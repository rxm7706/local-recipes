...
- **`docs/reference/atlas-spec-compendium.md`** — single-file compilation of every BMAD document that dreamed, contracted, or built the cf_atlas surface, across both eras: the pyforge-atlas Tier-0 Dream + Tier-2 build contracts (SPEC kernel, PRD, architecture spine, epics.md, all 32 story specs) and the 6 legacy `docs/specs/` builders inlined verbatim; consumer specs as pointers. Point-in-time snapshot (compilation commit in its header); the individual files remain the source of truth.

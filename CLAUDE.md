...
| `docs/specs/seed-gap-suggesters.md` | `cwe-seed-gap` + `spdx-schema-gap` — read-only suggesters proposing `cwe_categories_seed.json` / `spdx.schema.json` entries (keyword-classified `Other` CWEs; vendored-vs-upstream SPDX diff ranked by package usage); the seeds stay hand-curated, git review decides — SHIPPED 2026-07-06 (CFE v8.75.0). |
